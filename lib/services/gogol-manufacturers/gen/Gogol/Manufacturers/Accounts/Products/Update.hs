...
newManufacturersAccountsProductsUpdate name parent payload
  = ManufacturersAccountsProductsUpdate{xgafv = Core.Nothing,
                                        accessToken = Core.Nothing, callback = Core.Nothing,
                                        name = name, parent = parent, payload = payload,
                                        uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ManufacturersAccountsProductsUpdate
         where
        type Rs ManufacturersAccountsProductsUpdate = Empty
        type Scopes ManufacturersAccountsProductsUpdate =
             '[Manufacturercenter'FullControl]
        requestClient ManufacturersAccountsProductsUpdate{..}
          = go parent name xgafv accessToken callback uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy ManufacturersAccountsProductsUpdateResource)
                      Core.mempty
