...
newAction 
    ::  Action
newAction
  = Action{actor = Core.Nothing, detail = Core.Nothing,
           target = Core.Nothing, timeRange = Core.Nothing,
...
instance Core.FromJSON Action where
        parseJSON
          = Core.withObject "Action"
              (\ o ->
                 Action Core.<$>
                   (o Core..:? "actor") Core.<*> (o Core..:? "detail") Core.<*>
                     (o Core..:? "target")
...
newActionDetail 
    ::  ActionDetail
newActionDetail
  = ActionDetail{appliedLabelChange = Core.Nothing,
                 comment = Core.Nothing, create = Core.Nothing,
                 delete = Core.Nothing, dlpChange = Core.Nothing,
                 edit = Core.Nothing, move = Core.Nothing,
                 permissionChange = Core.Nothing, reference = Core.Nothing,
                 rename = Core.Nothing, restore = Core.Nothing,
                 settingsChange = Core.Nothing}
instance Core.FromJSON ActionDetail where
        parseJSON
          = Core.withObject "ActionDetail"
              (\ o ->
                 ActionDetail Core.<$>
                   (o Core..:? "appliedLabelChange") Core.<*> (o Core..:? "comment")
                     Core.<*> (o Core..:? "create")
                     Core.<*> (o Core..:? "delete")
...
instance Core.ToJSON ActionDetail where
        toJSON ActionDetail{..}
          = Core.object
              (Core.catMaybes
                 [("appliedLabelChange" Core..=) Core.<$> appliedLabelChange,
                  ("comment" Core..=) Core.<$> comment,
                  ("create" Core..=) Core.<$> create,
                  ("delete" Core..=) Core.<$> delete,
                  ("dlpChange" Core..=) Core.<$> dlpChange,
                  ("edit" Core..=) Core.<$> edit, ("move" Core..=) Core.<$> move,
                  ("permissionChange" Core..=) Core.<$> permissionChange,
...
newActor 
    ::  Actor
newActor
  = Actor{administrator = Core.Nothing, anonymous = Core.Nothing,
          impersonation = Core.Nothing, system = Core.Nothing,
          user = Core.Nothing}
instance Core.FromJSON Actor where
        parseJSON
          = Core.withObject "Actor"
              (\ o ->
                 Actor Core.<$>
                   (o Core..:? "administrator") Core.<*> (o Core..:? "anonymous")
...
instance Core.FromJSON Administrator where
        parseJSON
          = Core.withObject "Administrator" (\ o -> Core.pure Administrator)
...
instance Core.FromJSON AnonymousUser where
        parseJSON
          = Core.withObject "AnonymousUser" (\ o -> Core.pure AnonymousUser)
...
instance Core.FromJSON Anyone where
        parseJSON = Core.withObject "Anyone" (\ o -> Core.pure Anyone)
...
newApplicationReference 
    ::  ApplicationReference
newApplicationReference
  = ApplicationReference{type' = Core.Nothing}
instance Core.FromJSON ApplicationReference where
        parseJSON
          = Core.withObject "ApplicationReference"
              (\ o -> ApplicationReference Core.<$> (o Core..:? "type"))
...
instance Core.FromJSON AppliedLabelChange where
        parseJSON
          = Core.withObject "AppliedLabelChange"
              (\ o -> AppliedLabelChange Core.<$> (o Core..:? "changes"))
...
newAppliedLabelChangeDetail 
    ::  AppliedLabelChangeDetail
newAppliedLabelChangeDetail
  = AppliedLabelChangeDetail{fieldChanges = Core.Nothing,
                             label = Core.Nothing, title = Core.Nothing, types = Core.Nothing}
instance Core.FromJSON AppliedLabelChangeDetail where
        parseJSON
          = Core.withObject "AppliedLabelChangeDetail"
              (\ o ->
                 AppliedLabelChangeDetail Core.<$>
                   (o Core..:? "fieldChanges") Core.<*> (o Core..:? "label") Core.<*>
                     (o Core..:? "title")
                     Core.<*> (o Core..:? "types"))
          ...
          = Core.object
              (Core.catMaybes
                 [("fieldChanges" Core..=) Core.<$> fieldChanges,
                  ("label" Core..=) Core.<$> label, ("title" Core..=) Core.<$> title,
...
newAssignment 
    ::  Assignment
newAssignment
  = Assignment{assignedUser = Core.Nothing, subtype = Core.Nothing}
instance Core.FromJSON Assignment where
        parseJSON
          = Core.withObject "Assignment"
              (\ o ->
                 Assignment Core.<$>
                   (o Core..:? "assignedUser") Core.<*> (o Core..:? "subtype"))
...
newComment 
    ::  Comment
newComment
  = Comment{assignment = Core.Nothing, mentionedUsers = Core.Nothing,
            post = Core.Nothing, suggestion = Core.Nothing}
instance Core.FromJSON Comment where
        parseJSON
          = Core.withObject "Comment"
              (\ o ->
                 Comment Core.<$>
                   (o Core..:? "assignment") Core.<*> (o Core..:? "mentionedUsers")
                     Core.<*> (o Core..:? "post")
...
instance Core.FromJSON ConsolidationStrategy where
        parseJSON
          = Core.withObject "ConsolidationStrategy"
              (\ o ->
                 ConsolidationStrategy Core.<$>
                   (o Core..:? "legacy") Core.<*> (o Core..:? "none"))
...
newCreate 
    ::  Create
newCreate
  = Create{copy = Core.Nothing, new' = Core.Nothing,
           upload = Core.Nothing}
instance Core.FromJSON Create where
        parseJSON
          = Core.withObject "Create"
              (\ o ->
                 Create Core.<$>
                   (o Core..:? "copy") Core.<*> (o Core..:? "new") Core.<*>
                     (o Core..:? "upload"))
...
instance Core.ToJSON Create where
        toJSON Create{..}
          = Core.object
              (Core.catMaybes
                 [("copy" Core..=) Core.<$> copy, ("new" Core..=) Core.<$> new',
...
newDataLeakPreventionChange 
    ::  DataLeakPreventionChange
newDataLeakPreventionChange
  = DataLeakPreventionChange{type' = Core.Nothing}
instance Core.FromJSON DataLeakPreventionChange where
        parseJSON
          = Core.withObject "DataLeakPreventionChange"
              (\ o -> DataLeakPreventionChange Core.<$> (o Core..:? "type"))
...
instance Core.FromJSON DeletedUser where
        parseJSON
          = Core.withObject "DeletedUser" (\ o -> Core.pure DeletedUser)
...
instance Core.FromJSON Domain where
        parseJSON
          = Core.withObject "Domain"
              (\ o ->
                 Domain Core.<$>
                   (o Core..:? "legacyId") Core.<*> (o Core..:? "name"))
...
newDrive 
    ::  Drive
newDrive
  = Drive{name = Core.Nothing, root = Core.Nothing,
          title = Core.Nothing}
instance Core.FromJSON Drive where
        parseJSON
          = Core.withObject "Drive"
              (\ o ->
                 Drive Core.<$>
                   (o Core..:? "name") Core.<*> (o Core..:? "root") Core.<*>
                     (o Core..:? "title"))
...
newDriveActivity 
    ::  DriveActivity
newDriveActivity
  = DriveActivity{actions = Core.Nothing, actors = Core.Nothing,
                  primaryActionDetail = Core.Nothing, targets = Core.Nothing,
                  timeRange = Core.Nothing, timestamp = Core.Nothing}
instance Core.FromJSON DriveActivity where
        parseJSON
          = Core.withObject "DriveActivity"
              (\ o ->
                 DriveActivity Core.<$>
                   (o Core..:? "actions") Core.<*> (o Core..:? "actors") Core.<*>
                     (o Core..:? "primaryActionDetail")
                     Core.<*> (o Core..:? "targets")
                     Core.<*> (o Core..:? "timeRange")
                     Core.<*> (o Core..:? "timestamp"))
          ...
          = Core.object
              (Core.catMaybes
                 [("actions" Core..=) Core.<$> actions,
                  ...
                  ("primaryActionDetail" Core..=) Core.<$> primaryActionDetail,
...
instance Core.FromJSON DriveFile where
        parseJSON
          = Core.withObject "DriveFile" (\ o -> Core.pure DriveFile)
...
newDriveItem 
    ::  DriveItem
newDriveItem
  = DriveItem{driveFile = Core.Nothing, driveFolder = Core.Nothing,
              file = Core.Nothing, folder = Core.Nothing,
              mimeType = Core.Nothing, name = Core.Nothing, owner = Core.Nothing,
              title = Core.Nothing}
instance Core.FromJSON DriveItem where
        parseJSON
          = Core.withObject "DriveItem"
              (\ o ->
                 DriveItem Core.<$>
                   (o Core..:? "driveFile") Core.<*> (o Core..:? "driveFolder")
                     Core.<*> (o Core..:? "file")
                     Core.<*> (o Core..:? "folder")
          ...
          = Core.object
              (Core.catMaybes
                 [("driveFile" Core..=) Core.<$> driveFile,
                  ...
                  ("file" Core..=) Core.<$> file, ("folder" Core..=) Core.<$> folder,
...
newDriveItemReference 
    ::  DriveItemReference
newDriveItemReference
  = DriveItemReference{driveFile = Core.Nothing,
                       driveFolder = Core.Nothing, file = Core.Nothing,
                       folder = Core.Nothing, name = Core.Nothing, title = Core.Nothing}
instance Core.FromJSON DriveItemReference where
        parseJSON
          = Core.withObject "DriveItemReference"
              (\ o ->
                 DriveItemReference Core.<$>
                   (o Core..:? "driveFile") Core.<*> (o Core..:? "driveFolder")
                     Core.<*> (o Core..:? "file")
                     Core.<*> (o Core..:? "folder")
                     Core.<*> (o Core..:? "name")
...
newDriveReference 
    ::  DriveReference
newDriveReference
  = DriveReference{name = Core.Nothing, title = Core.Nothing}
instance Core.FromJSON DriveReference where
        parseJSON
          = Core.withObject "DriveReference"
              (\ o ->
                 DriveReference Core.<$>
...
instance Core.FromJSON Edit where
        parseJSON = Core.withObject "Edit" (\ o -> Core.pure Edit)
...
newFieldValue 
    ::  FieldValue
newFieldValue
  = FieldValue{date = Core.Nothing, integer = Core.Nothing,
               selection = Core.Nothing, selectionList = Core.Nothing,
               text = Core.Nothing, textList = Core.Nothing, user = Core.Nothing,
...
instance Core.FromJSON FieldValue where
        parseJSON
          = Core.withObject "FieldValue"
              (\ o ->
                 FieldValue Core.<$>
                   (o Core..:? "date") Core.<*> (o Core..:? "integer") Core.<*>
                     (o Core..:? "selection")
...
newFieldValueChange 
    ::  FieldValueChange
newFieldValueChange
  = FieldValueChange{displayName = Core.Nothing,
                     fieldId = Core.Nothing, newValue' = Core.Nothing,
                     oldValue = Core.Nothing}
instance Core.FromJSON FieldValueChange where
        parseJSON
          = Core.withObject "FieldValueChange"
              (\ o ->
                 FieldValueChange Core.<$>
                   (o Core..:? "displayName") Core.<*> (o Core..:? "fieldId") Core.<*>
                     (o Core..:? "newValue")
...
instance Core.FromJSON File where
        parseJSON = Core.withObject "File" (\ o -> Core.pure File)
...
newFileComment 
    ::  FileComment
newFileComment
  = FileComment{legacyCommentId = Core.Nothing,
                legacyDiscussionId = Core.Nothing, linkToDiscussion = Core.Nothing,
                parent = Core.Nothing}
instance Core.FromJSON FileComment where
        parseJSON
          = Core.withObject "FileComment"
              (\ o ->
                 FileComment Core.<$>
                   (o Core..:? "legacyCommentId") Core.<*>
                     (o Core..:? "legacyDiscussionId")
...
instance Core.ToJSON FileComment where
        toJSON FileComment{..}
          = Core.object
              (Core.catMaybes
                 [("legacyCommentId" Core..=) Core.<$> legacyCommentId,
                  ("legacyDiscussionId" Core..=) Core.<$> legacyDiscussionId,
                  ("linkToDiscussion" Core..=) Core.<$> linkToDiscussion,
...
instance Core.FromJSON Group where
        parseJSON
          = Core.withObject "Group"
              (\ o ->
                 Group Core.<$> (o Core..:? "email") Core.<*> (o Core..:? "title"))
...
instance Core.FromJSON Impersonation where
        parseJSON
          = Core.withObject "Impersonation"
              (\ o -> Impersonation Core.<$> (o Core..:? "impersonatedUser"))

instance Core.ToJSON Impersonation where
        toJSON Impersonation{..}
          = Core.object
              (Core.catMaybes
                 [("impersonatedUser" Core..=) Core.<$> impersonatedUser])
...
newKnownUser 
    ::  KnownUser
newKnownUser
  = KnownUser{isCurrentUser = Core.Nothing,
              personName = Core.Nothing}
instance Core.FromJSON KnownUser where
        parseJSON
          = Core.withObject "KnownUser"
              (\ o ->
                 KnownUser Core.<$>
                   (o Core..:? "isCurrentUser") Core.<*> (o Core..:? "personName"))
...
instance Core.FromJSON Legacy where
        parseJSON = Core.withObject "Legacy" (\ o -> Core.pure Legacy)
...
instance Core.FromJSON Move where
        parseJSON
          = Core.withObject "Move"
              (\ o ->
                 Move Core.<$>
                   (o Core..:? "addedParents") Core.<*> (o Core..:? "removedParents"))
...
instance Core.FromJSON New where
        parseJSON = Core.withObject "New" (\ o -> Core.pure New)
...
instance Core.FromJSON NoConsolidation where
        parseJSON
          = Core.withObject "NoConsolidation"
...
newOwner 
    ::  Owner
newOwner
  = Owner{domain = Core.Nothing, drive = Core.Nothing,
          teamDrive = Core.Nothing, user = Core.Nothing}
instance Core.FromJSON Owner where
        parseJSON
          = Core.withObject "Owner"
              (\ o ->
                 Owner Core.<$>
                   (o Core..:? "domain") Core.<*> (o Core..:? "drive") Core.<*>
                     (o Core..:? "teamDrive")
                     Core.<*> (o Core..:? "user"))
...
newPermission 
    ::  Permission
newPermission
  = Permission{allowDiscovery = Core.Nothing, anyone = Core.Nothing,
               domain = Core.Nothing, group' = Core.Nothing, role' = Core.Nothing,
               user = Core.Nothing}
instance Core.FromJSON Permission where
        parseJSON
          = Core.withObject "Permission"
              (\ o ->
                 Permission Core.<$>
                   (o Core..:? "allowDiscovery") Core.<*> (o Core..:? "anyone")
                     Core.<*> (o Core..:? "domain")
                     Core.<*> (o Core..:? "group")
                     Core.<*> (o Core..:? "role")
                     Core.<*> (o Core..:? "user"))
          ...
          = Core.object
              (Core.catMaybes
                 [("allowDiscovery" Core..=) Core.<$> allowDiscovery,
                  ...
                  ("domain" Core..=) Core.<$> domain,
                  ("group" Core..=) Core.<$> group', ("role" Core..=) Core.<$> role',
                  ("user" Core..=) Core.<$> user])
...
newPermissionChange 
    ::  PermissionChange
newPermissionChange
  = PermissionChange{addedPermissions = Core.Nothing,
                     removedPermissions = Core.Nothing}
instance Core.FromJSON PermissionChange where
        parseJSON
          = Core.withObject "PermissionChange"
              (\ o ->
...
instance Core.FromJSON Post' where
        parseJSON
          = Core.withObject "Post'"
              (\ o -> Post' Core.<$> (o Core..:? "subtype"))

instance Core.ToJSON Post' where
        toJSON Post'{..}
          = Core.object
              (Core.catMaybes [("subtype" Core..=) Core.<$> subtype])
...
newQueryDriveActivityRequest 
    ::  QueryDriveActivityRequest
newQueryDriveActivityRequest
  = QueryDriveActivityRequest{ancestorName = Core.Nothing,
                              consolidationStrategy = Core.Nothing, filter = Core.Nothing,
                              itemName = Core.Nothing, pageSize = Core.Nothing,
                              pageToken = Core.Nothing}
instance Core.FromJSON QueryDriveActivityRequest where
        parseJSON
          = Core.withObject "QueryDriveActivityRequest"
              (\ o ->
                 QueryDriveActivityRequest Core.<$>
                   (o Core..:? "ancestorName") Core.<*>
                     (o Core..:? "consolidationStrategy")
                     Core.<*> (o Core..:? "filter")
          ...
          = Core.object
              (Core.catMaybes
                 [("ancestorName" Core..=) Core.<$> ancestorName,
                  ("consolidationStrategy" Core..=) Core.<$> consolidationStrategy,
...
newQueryDriveActivityResponse 
    ::  QueryDriveActivityResponse
newQueryDriveActivityResponse
  = QueryDriveActivityResponse{activities = Core.Nothing,
                               nextPageToken = Core.Nothing}
instance Core.FromJSON QueryDriveActivityResponse where
        parseJSON
          = Core.withObject "QueryDriveActivityResponse"
              (\ o ->
                 QueryDriveActivityResponse Core.<$>
                   (o Core..:? "activities") Core.<*> (o Core..:? "nextPageToken"))
...
instance Core.FromJSON Rename where
        parseJSON
          = Core.withObject "Rename"
              (\ o ->
                 Rename Core.<$>
                   (o Core..:? "newTitle") Core.<*> (o Core..:? "oldTitle"))
...
newRestrictionChange 
    ::  RestrictionChange
newRestrictionChange
  = RestrictionChange{feature = Core.Nothing,
                      newRestriction' = Core.Nothing}
instance Core.FromJSON RestrictionChange where
        parseJSON
          = Core.withObject "RestrictionChange"
              (\ o ->
                 RestrictionChange Core.<$>
                   (o Core..:? "feature") Core.<*> (o Core..:? "newRestriction"))
...
newSelection 
    ::  Selection
newSelection
  = Selection{displayName = Core.Nothing, value = Core.Nothing}
instance Core.FromJSON Selection where
        parseJSON
          = Core.withObject "Selection"
              (\ o ->
                 Selection Core.<$>
...
newSettingsChange 
    ::  SettingsChange
newSettingsChange
  = SettingsChange{restrictionChanges = Core.Nothing}
instance Core.FromJSON SettingsChange where
        parseJSON
          = Core.withObject "SettingsChange"
              (\ o -> SettingsChange Core.<$> (o Core..:? "restrictionChanges"))

instance Core.ToJSON SettingsChange where
        toJSON SettingsChange{..}
          = Core.object
              (Core.catMaybes
                 [("restrictionChanges" Core..=) Core.<$> restrictionChanges])
...
-- | Information about the target of activity. For more information on how activity history is shared with users, see <https://developers.google.com/workspace/drive/activity/v2#activityhistory Activity history visibility>.
--
...
newTarget 
    ::  Target
newTarget
  = Target{drive = Core.Nothing, driveItem = Core.Nothing,
           fileComment = Core.Nothing, teamDrive = Core.Nothing}
instance Core.FromJSON Target where
        parseJSON
          = Core.withObject "Target"
              (\ o ->
                 Target Core.<$>
                   (o Core..:? "drive") Core.<*> (o Core..:? "driveItem") Core.<*>
                     (o Core..:? "fileComment")
...
newTargetReference 
    ::  TargetReference
newTargetReference
  = TargetReference{drive = Core.Nothing, driveItem = Core.Nothing,
                    teamDrive = Core.Nothing}
instance Core.FromJSON TargetReference where
        parseJSON
          = Core.withObject "TargetReference"
              (\ o ->
                 TargetReference Core.<$>
                   (o Core..:? "drive") Core.<*> (o Core..:? "driveItem") Core.<*>
                     (o Core..:? "teamDrive"))
...
newTeamDrive 
    ::  TeamDrive
newTeamDrive
  = TeamDrive{name = Core.Nothing, root = Core.Nothing,
              title = Core.Nothing}
...
instance Core.FromJSON TeamDriveReference where
        parseJSON
          = Core.withObject "TeamDriveReference"
              (\ o ->
                 TeamDriveReference Core.<$>
                   (o Core..:? "name") Core.<*> (o Core..:? "title"))
...
instance Core.FromJSON UnknownUser where
        parseJSON
          = Core.withObject "UnknownUser" (\ o -> Core.pure UnknownUser)
...
instance Core.FromJSON Upload where
        parseJSON = Core.withObject "Upload" (\ o -> Core.pure Upload)
...
newUser 
    ::  User
newUser
  = User{deletedUser = Core.Nothing, knownUser = Core.Nothing,
...
instance Core.FromJSON User where
        parseJSON
          = Core.withObject "User"
              (\ o ->
                 User Core.<$>
                   (o Core..:? "deletedUser") Core.<*> (o Core..:? "knownUser")
