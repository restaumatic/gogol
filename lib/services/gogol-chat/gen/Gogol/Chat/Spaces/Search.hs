...
--
-- Returns a list of spaces in a Google Workspace organization based on an administrator\'s search. In the request, set @use_admin_access@ to @true@. For an example, see <https://developers.google.com/workspace/chat/search-manage-admin Search for and manage spaces>. Requires <https://developers.google.com/workspace/chat/authenticate-authorize-chat-user#admin-privileges user authentication with administrator privileges> and one of the following <https://developers.google.com/workspace/chat/authenticate-authorize#chat-api-scopes authorization scopes>: - @https:\/\/www.googleapis.com\/auth\/chat.admin.spaces.readonly@ - @https:\/\/www.googleapis.com\/auth\/chat.admin.spaces@
--
-- /See:/ <https://developers.google.com/workspace/chat Google Chat API Reference> for @chat.spaces.search@.
module Gogol.Chat.Spaces.Search
    (
    -- * Resource
