...
--
-- Returns an event from a Google Chat space. The <https://developers.google.com/workspace/chat/api/reference/rest/v1/spaces.spaceEvents#SpaceEvent.FIELDS.oneof_payload event payload> contains the most recent version of the resource that changed. For example, if you request an event about a new message but the message was later updated, the server returns the updated @Message@ resource in the event payload. Note: The @permissionSettings@ field is not returned in the Space object of the Space event data for this request. Supports the following types of <https://developers.google.com/workspace/chat/authenticate-authorize authentication> with an <https://developers.google.com/workspace/chat/authenticate-authorize#chat-api-scopes authorization scope> appropriate for reading the requested data: - <https://developers.google.com/workspace/chat/authenticate-authorize-chat-app App authentication> with <https://support.google.com/a?p=chat-app-auth administrator approval> in
-- <https://developers.google.com/workspace/preview Developer Preview> with one of the following authorization scopes: - @https:\/\/www.googleapis.com\/auth\/chat.app.spaces@ - @https:\/\/www.googleapis.com\/auth\/chat.app.messages.readonly@ - @https:\/\/www.googleapis.com\/auth\/chat.app.memberships@ - <https://developers.google.com/workspace/chat/authenticate-authorize-chat-user User authentication> with one of the following authorization scopes: - @https:\/\/www.googleapis.com\/auth\/chat.spaces.readonly@ - @https:\/\/www.googleapis.com\/auth\/chat.spaces@ - @https:\/\/www.googleapis.com\/auth\/chat.messages.readonly@ - @https:\/\/www.googleapis.com\/auth\/chat.messages@ - @https:\/\/www.googleapis.com\/auth\/chat.messages.reactions.readonly@ - @https:\/\/www.googleapis.com\/auth\/chat.messages.reactions@ - @https:\/\/www.googleapis.com\/auth\/chat.memberships.readonly@ - @https:\/\/www.googleapis.com\/auth\/chat.memberships@ To get an event, the authenticated caller must be a member of the space. For an
-- example, see <https://developers.google.com/workspace/chat/get-space-event Get details about an event from a Google Chat space>.
--
-- /See:/ <https://developers.google.com/workspace/chat Google Chat API Reference> for @chat.spaces.spaceEvents.get@.
module Gogol.Chat.Spaces.SpaceEvents.Get
    (
    -- * Resource
...
instance Core.GoogleRequest ChatSpacesSpaceEventsGet where
        ...
        type Scopes ChatSpacesSpaceEventsGet =
             '[Chat'App'Memberships, Chat'App'Messages'Readonly,
               Chat'App'Spaces, Chat'Memberships, Chat'Memberships'Readonly,
