                                   ...
                                   Core.QueryParam "uploadType" Core.Text Core.:>
                                     Core.QueryParam "upload_protocol" Core.Text Core.:>
                                       Core.QueryParam "alt" Core.AltJSON Core.:>
                                         Core.Get '[Core.JSON] ResourcesListResponse
...
newDeploymentManagerResourcesList deployment project
  = DeploymentManagerResourcesList{xgafv = Core.Nothing,
                                   accessToken = Core.Nothing, callback = Core.Nothing,
                                   deployment = deployment, filter = Core.Nothing, maxResults = 500,
                                   orderBy = Core.Nothing, pageToken = Core.Nothing,
                                   project = project, uploadType = Core.Nothing,
                                   uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DeploymentManagerResourcesList where
        type Rs DeploymentManagerResourcesList = ResourcesListResponse
        type Scopes DeploymentManagerResourcesList =
             '[CloudPlatform'FullControl, CloudPlatform'ReadOnly, Ndev'Cloudman,
               Ndev'Cloudman'Readonly]
        requestClient DeploymentManagerResourcesList{..}
          = go project deployment xgafv accessToken callback filter
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DeploymentManagerResourcesListResource)
                      Core.mempty
