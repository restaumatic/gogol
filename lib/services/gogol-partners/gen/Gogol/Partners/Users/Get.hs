...
-- |
-- Module      : Gogol.Partners.Users.Get
...
-- 'PartnersUsersGet' request conforms to.
type PartnersUsersGetResource =
     "v2" Core.:>
       "users" Core.:>
           ...
           Core.QueryParam "$.xgafv" Xgafv Core.:>
             Core.QueryParam "access_token" Core.Text Core.:>
               Core.QueryParam "callback" Core.Text Core.:>
                 Core.QueryParams "requestMetadata.experimentIds" Core.Text Core.:>
                   Core.QueryParam "requestMetadata.locale" Core.Text Core.:>
                     Core.QueryParam "requestMetadata.partnersSessionId" Core.Text
                       Core.:>
                       Core.QueryParam "requestMetadata.trafficSource.trafficSourceId"
                         Core.Text
                         Core.:>
                         Core.QueryParam "requestMetadata.trafficSource.trafficSubId"
                           Core.Text
                           Core.:>
                           Core.QueryParam "requestMetadata.userOverrides.ipAddress" Core.Text
                             Core.:>
                             Core.QueryParam "requestMetadata.userOverrides.userId" Core.Text
                               Core.:>
                               Core.QueryParam "uploadType" Core.Text Core.:>
                                 Core.QueryParam "upload_protocol" Core.Text Core.:>
                                   Core.QueryParam "userView" Core.Text Core.:>
                                     Core.QueryParam "alt" Core.AltJSON Core.:>
                                       Core.Get '[Core.JSON] User

-- | Gets a user.
--
-- /See:/ 'newPartnersUsersGet' smart constructor.
data PartnersUsersGet = PartnersUsersGet
    {
      -- | V1 error format.
    ...
    , uploadProtocol :: (Core.Maybe Core.Text)
      -- | Identifier of the user. Can be set to \<code>me\<\/code> to mean the currently authenticated user.
    , userId :: Core.Text
      -- | Specifies what parts of the user information to return.
    , userView :: (Core.Maybe Core.Text)
    }
    deriving (Core.Eq, Core.Show, Core.Generic)

-- | Creates a value of 'PartnersUsersGet' with the minimum fields required to make a request.
newPartnersUsersGet 
    ::  Core.Text
       -- ^  Identifier of the user. Can be set to \<code>me\<\/code> to mean the currently authenticated user. See 'userId'.
    -> PartnersUsersGet
newPartnersUsersGet userId
  = PartnersUsersGet{xgafv = Core.Nothing,
                     ...
                     userId = userId, userView = Core.Nothing}
instance Core.GoogleRequest PartnersUsersGet where
        type Rs PartnersUsersGet = User
        type Scopes PartnersUsersGet = '[]
        requestClient PartnersUsersGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy PartnersUsersGetResource)
                      Core.mempty
