...
newPartnersCompaniesLeadsCreate companyId payload
  = PartnersCompaniesLeadsCreate{xgafv = Core.Nothing,
                                 accessToken = Core.Nothing, callback = Core.Nothing,
                                 companyId = companyId, payload = payload,
                                 uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest PartnersCompaniesLeadsCreate where
        type Rs PartnersCompaniesLeadsCreate = CreateLeadResponse
        type Scopes PartnersCompaniesLeadsCreate = '[]
        requestClient PartnersCompaniesLeadsCreate{..}
          = go companyId xgafv accessToken callback uploadType uploadProtocol
              (Core.Just Core.AltJSON)
              payload
              partnersService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy PartnersCompaniesLeadsCreateResource)
                      Core.mempty
