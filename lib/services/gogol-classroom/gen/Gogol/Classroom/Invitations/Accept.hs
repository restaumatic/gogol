...
--
-- /See:/ <https://developers.google.com/workspace/classroom/ Google Classroom API Reference> for @classroom.invitations.accept@.
module Gogol.Classroom.Invitations.Accept
    (
    -- * Resource
...
type ClassroomInvitationsAcceptResource =
     "v1" Core.:>
       "invitations" Core.:>
         Core.CaptureMode "id" "accept" Core.Text Core.:>
...
newClassroomInvitationsAccept id
  = ClassroomInvitationsAccept{xgafv = Core.Nothing,
                               accessToken = Core.Nothing, callback = Core.Nothing, id = id,
                               uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ClassroomInvitationsAccept where
        type Rs ClassroomInvitationsAccept = Empty
        type Scopes ClassroomInvitationsAccept = '[Classroom'Rosters]
        requestClient ClassroomInvitationsAccept{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ClassroomInvitationsAcceptResource)
                      Core.mempty
