...
--
-- Gets metadata for Classroom add-ons in the context of a specific post. To maintain the integrity of its own data and permissions model, an add-on should call this to validate query parameters and the requesting user\'s role whenever the add-on is opened in an <https://developers.google.com/workspace/classroom/add-ons/get-started/iframes/iframes-overview iframe>. This method returns the following error codes: * @PERMISSION_DENIED@ for access errors. * @INVALID_ARGUMENT@ if the request is malformed. * @NOT_FOUND@ if one of the identified resources does not exist.
--
-- /See:/ <https://developers.google.com/workspace/classroom/ Google Classroom API Reference> for @classroom.courses.posts.getAddOnContext@.
module Gogol.Classroom.Courses.Posts.GetAddOnContext
    (
    -- * Resource
...
data ClassroomCoursesPostsGetAddOnContext = ClassroomCoursesPostsGetAddOnContext
    ...
    , addOnToken :: (Core.Maybe Core.Text)
      -- | Optional. The identifier of the attachment. This field is required for all requests except when the user is in the <https://developers.google.com/workspace/classroom/add-ons/get-started/iframes/attachment-discovery-iframe Attachment Discovery iframe>.
...
newClassroomCoursesPostsGetAddOnContext courseId postId
  = ClassroomCoursesPostsGetAddOnContext{xgafv = Core.Nothing,
                                         accessToken = Core.Nothing, addOnToken = Core.Nothing,
                                         attachmentId = Core.Nothing, callback = Core.Nothing,
                                         courseId = courseId, itemId = Core.Nothing,
                                         postId = postId, uploadType = Core.Nothing,
                                         uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ClassroomCoursesPostsGetAddOnContext
         where
        ...
        requestClient ClassroomCoursesPostsGetAddOnContext{..}
          = go courseId postId xgafv accessToken addOnToken attachmentId
              callback
              itemId
              uploadType
              uploadProtocol
