...
--
-- /See:/ <https://developers.google.com/workspace/classroom/ Google Classroom API Reference> for @classroom.courses.topics.patch@.
module Gogol.Classroom.Courses.Topics.Patch
    (
    -- * Resource
                       ...
                       Core.QueryParam "uploadType" Core.Text Core.:>
                         Core.QueryParam "upload_protocol" Core.Text Core.:>
                           Core.QueryParam "alt" Core.AltJSON Core.:>
                             Core.ReqBody '[Core.JSON] Topic Core.:>
...
newClassroomCoursesTopicsPatch courseId id payload
  = ClassroomCoursesTopicsPatch{xgafv = Core.Nothing,
                                accessToken = Core.Nothing, callback = Core.Nothing,
                                courseId = courseId, id = id, payload = payload,
                                updateMask = Core.Nothing, uploadType = Core.Nothing,
                                uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ClassroomCoursesTopicsPatch where
        type Rs ClassroomCoursesTopicsPatch = Topic
        type Scopes ClassroomCoursesTopicsPatch = '[Classroom'Topics]
        requestClient ClassroomCoursesTopicsPatch{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ClassroomCoursesTopicsPatchResource)
                      Core.mempty
