...
--
-- /See:/ <https://developers.google.com/workspace/classroom/ Google Classroom API Reference> for @classroom.courses.teachers.delete@.
module Gogol.Classroom.Courses.Teachers.Delete
    (
    -- * Resource
...
newClassroomCoursesTeachersDelete courseId userId
  = ClassroomCoursesTeachersDelete{xgafv = Core.Nothing,
                                   accessToken = Core.Nothing, callback = Core.Nothing,
                                   courseId = courseId, uploadType = Core.Nothing,
                                   uploadProtocol = Core.Nothing, userId = userId}
instance Core.GoogleRequest ClassroomCoursesTeachersDelete where
        type Rs ClassroomCoursesTeachersDelete = Empty
        type Scopes ClassroomCoursesTeachersDelete = '[Classroom'Rosters]
        requestClient ClassroomCoursesTeachersDelete{..}
          = go courseId userId xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              classroomService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ClassroomCoursesTeachersDeleteResource)
                      Core.mempty
