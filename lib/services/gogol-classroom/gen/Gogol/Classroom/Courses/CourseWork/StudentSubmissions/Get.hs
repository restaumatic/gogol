...
--
-- /See:/ <https://developers.google.com/workspace/classroom/ Google Classroom API Reference> for @classroom.courses.courseWork.studentSubmissions.get@.
module Gogol.Classroom.Courses.CourseWork.StudentSubmissions.Get
    (
    -- * Resource
                         ...
                         Core.QueryParam "uploadType" Core.Text Core.:>
                           Core.QueryParam "upload_protocol" Core.Text Core.:>
                             Core.QueryParam "alt" Core.AltJSON Core.:>
                               Core.Get '[Core.JSON] StudentSubmission
...
newClassroomCoursesCourseWorkStudentSubmissionsGet 
    ...
    -> ClassroomCoursesCourseWorkStudentSubmissionsGet
newClassroomCoursesCourseWorkStudentSubmissionsGet courseId
  courseWorkId id
  = ClassroomCoursesCourseWorkStudentSubmissionsGet{xgafv =
                                                      Core.Nothing,
                                                    accessToken = Core.Nothing,
                                                    callback = Core.Nothing, courseId = courseId,
                                                    courseWorkId = courseWorkId, id = id,
                                                    uploadType = Core.Nothing,
                                                    uploadProtocol = Core.Nothing}
instance Core.GoogleRequest
           ClassroomCoursesCourseWorkStudentSubmissionsGet
         where
        type Rs ClassroomCoursesCourseWorkStudentSubmissionsGet =
             StudentSubmission
        type Scopes ClassroomCoursesCourseWorkStudentSubmissionsGet =
        ...
        requestClient ClassroomCoursesCourseWorkStudentSubmissionsGet{..}
          = go courseId courseWorkId id xgafv accessToken callback uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy ClassroomCoursesCourseWorkStudentSubmissionsGetResource)
                      Core.mempty
