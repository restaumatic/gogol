...
--
-- /See:/ <https://developers.google.com/games/ Google Play Games Services API Reference> for @games.recall.lastTokenFromAllDeveloperGames@.
module Gogol.Games.Recall.LastTokenFromAllDeveloperGames
    (
    -- * Resource
...
type GamesRecallLastTokenFromAllDeveloperGamesResource =
     "games" Core.:>
       "v1" Core.:>
         "recall" Core.:>
           "developerGamesLastPlayerToken" Core.:>
                     ...
                     Core.QueryParam "uploadType" Core.Text Core.:>
                       Core.QueryParam "upload_protocol" Core.Text Core.:>
                         Core.QueryParam "alt" Core.AltJSON Core.:>
                           Core.Get '[Core.JSON] RetrieveDeveloperGamesLastPlayerTokenResponse
...
newGamesRecallLastTokenFromAllDeveloperGames sessionId
  = GamesRecallLastTokenFromAllDeveloperGames{xgafv = Core.Nothing,
                                              accessToken = Core.Nothing, callback = Core.Nothing,
                                              sessionId = sessionId, uploadType = Core.Nothing,
                                              uploadProtocol = Core.Nothing}
instance Core.GoogleRequest
           GamesRecallLastTokenFromAllDeveloperGames
         where
        type Rs GamesRecallLastTokenFromAllDeveloperGames =
             RetrieveDeveloperGamesLastPlayerTokenResponse
        type Scopes GamesRecallLastTokenFromAllDeveloperGames =
             '[Androidpublisher'FullControl]
        requestClient GamesRecallLastTokenFromAllDeveloperGames{..}
          = go sessionId xgafv accessToken callback uploadType uploadProtocol
