...
--
-- /See:/ <https://developers.google.com/games/ Google Play Games Services API Reference> for @games.revisions.check@.
module Gogol.Games.Revisions.Check
    (
    -- * Resource
...
newGamesRevisionsCheck clientRevision
  = GamesRevisionsCheck{xgafv = Core.Nothing,
                        accessToken = Core.Nothing, callback = Core.Nothing,
                        clientRevision = clientRevision, uploadType = Core.Nothing,
                        uploadProtocol = Core.Nothing}
instance Core.GoogleRequest GamesRevisionsCheck where
        type Rs GamesRevisionsCheck = RevisionCheckResponse
        type Scopes GamesRevisionsCheck = '[Games'FullControl]
        requestClient GamesRevisionsCheck{..}
          = go (Core.Just clientRevision) xgafv accessToken callback
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy GamesRevisionsCheckResource)
                      Core.mempty
