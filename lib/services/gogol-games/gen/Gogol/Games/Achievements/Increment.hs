...
--
-- /See:/ <https://developers.google.com/games/ Google Play Games Services API Reference> for @games.achievements.increment@.
module Gogol.Games.Achievements.Increment
    (
    -- * Resource
                         ...
                         Core.QueryParam "uploadType" Core.Text Core.:>
                           Core.QueryParam "upload_protocol" Core.Text Core.:>
                             Core.QueryParam "alt" Core.AltJSON Core.:>
                               Core.Post '[Core.JSON] AchievementIncrementResponse
...
newGamesAchievementsIncrement 
    ...
    -> Core.Int32
       -- ^  Required. The number of steps to increment. See 'stepsToIncrement'.
    -> GamesAchievementsIncrement
newGamesAchievementsIncrement achievementId stepsToIncrement
  = GamesAchievementsIncrement{xgafv = Core.Nothing,
                               accessToken = Core.Nothing, achievementId = achievementId,
                               callback = Core.Nothing, requestId = Core.Nothing,
                               stepsToIncrement = stepsToIncrement, uploadType = Core.Nothing,
                               uploadProtocol = Core.Nothing}
instance Core.GoogleRequest GamesAchievementsIncrement where
        type Rs GamesAchievementsIncrement = AchievementIncrementResponse
        type Scopes GamesAchievementsIncrement = '[Games'FullControl]
        requestClient GamesAchievementsIncrement{..}
          = go achievementId (Core.Just stepsToIncrement) xgafv accessToken
              callback
              requestId
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy GamesAchievementsIncrementResource)
                      Core.mempty
