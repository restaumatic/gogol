...
--
-- /See:/ <https://developers.google.com/games/ Google Play Games Services API Reference> for @games.scores.submitMultiple@.
module Gogol.Games.Scores.SubmitMultiple
    (
    -- * Resource
                     ...
                     Core.QueryParam "uploadType" Core.Text Core.:>
                       Core.QueryParam "upload_protocol" Core.Text Core.:>
                         Core.QueryParam "alt" Core.AltJSON Core.:>
                           Core.ReqBody '[Core.JSON] PlayerScoreSubmissionList Core.:>
                             Core.Post '[Core.JSON] PlayerScoreListResponse
...
newGamesScoresSubmitMultiple payload
  = GamesScoresSubmitMultiple{xgafv = Core.Nothing,
                              accessToken = Core.Nothing, callback = Core.Nothing,
                              language = Core.Nothing, payload = payload,
                              uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest GamesScoresSubmitMultiple where
        type Rs GamesScoresSubmitMultiple = PlayerScoreListResponse
        type Scopes GamesScoresSubmitMultiple = '[Games'FullControl]
        requestClient GamesScoresSubmitMultiple{..}
          = go xgafv accessToken callback language uploadType uploadProtocol
              (Core.Just Core.AltJSON)
              payload
              gamesService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy GamesScoresSubmitMultipleResource)
                      Core.mempty
