...
type TracingProjectsTracesListSpansResource =
     "v2" Core.:>
       Core.CaptureMode "parent" "listSpans" Core.Text Core.:>
...
newTracingProjectsTracesListSpans parent
  = TracingProjectsTracesListSpans{xgafv = Core.Nothing,
                                   accessToken = Core.Nothing, bearerToken = Core.Nothing,
                                   callback = Core.Nothing, pageToken = Core.Nothing,
                                   parent = parent, pp = Core.True, uploadType = Core.Nothing,
                                   uploadProtocol = Core.Nothing}
instance Core.GoogleRequest TracingProjectsTracesListSpans where
        type Rs TracingProjectsTracesListSpans = ListSpansResponse
        ...
        requestClient TracingProjectsTracesListSpans{..}
          = go parent xgafv accessToken bearerToken callback pageToken
              (Core.Just pp)
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy TracingProjectsTracesListSpansResource)
                      Core.mempty
