...
-- |
-- Module      : Gogol.ReplicaPool.Internal.Product
...
newAccessConfig 
    ::  AccessConfig
newAccessConfig
  = AccessConfig{name = Core.Nothing, natIp = Core.Nothing,
                 type' = Core.Nothing}
instance Core.FromJSON AccessConfig where
        parseJSON
          = Core.withObject "AccessConfig"
              (\ o ->
                 AccessConfig Core.<$>
                   (o Core..:? "name") Core.<*> (o Core..:? "natIp") Core.<*>
                     (o Core..:? "type"))
...
instance Core.ToJSON AccessConfig where
        toJSON AccessConfig{..}
          = Core.object
              (Core.catMaybes
                 [("name" Core..=) Core.<$> name, ("natIp" Core..=) Core.<$> natIp,
                  ("type" Core..=) Core.<$> type'])
...
newAction 
    ::  Action
newAction
  = Action{commands = Core.Nothing, envVariables = Core.Nothing,
...
instance Core.FromJSON Action where
        parseJSON
          = Core.withObject "Action"
              (\ o ->
                 Action Core.<$>
                   (o Core..:? "commands") Core.<*> (o Core..:? "envVariables")
          ...
          = Core.object
              (Core.catMaybes
                 [("commands" Core..=) Core.<$> commands,
                  ...
                  ("timeoutMilliSeconds" Core..=) Core.<$> timeoutMilliSeconds])
...
instance Core.FromJSON DiskAttachment where
        parseJSON
          = Core.withObject "DiskAttachment"
              (\ o ->
                 DiskAttachment Core.<$>
                   (o Core..:? "deviceName") Core.<*> (o Core..:? "index"))
...
newEnvVariable 
    ::  EnvVariable
newEnvVariable
  = EnvVariable{hidden = Core.Nothing, name = Core.Nothing,
                value = Core.Nothing}
instance Core.FromJSON EnvVariable where
        parseJSON
          = Core.withObject "EnvVariable"
              (\ o ->
                 EnvVariable Core.<$>
                   (o Core..:? "hidden") Core.<*> (o Core..:? "name") Core.<*>
                     (o Core..:? "value"))
...
instance Core.FromJSON ExistingDisk where
        parseJSON
          = Core.withObject "ExistingDisk"
              (\ o ->
                 ExistingDisk Core.<$>
                   (o Core..:? "attachment") Core.<*> (o Core..:? "source"))
...
newHealthCheck 
    ::  HealthCheck
newHealthCheck
  = HealthCheck{checkIntervalSec = Core.Nothing,
                description = Core.Nothing, healthyThreshold = Core.Nothing,
                host = Core.Nothing, name = Core.Nothing, path = Core.Nothing,
                port = Core.Nothing, timeoutSec = Core.Nothing,
...
instance Core.FromJSON HealthCheck where
        parseJSON
          = Core.withObject "HealthCheck"
              (\ o ->
                 HealthCheck Core.<$>
                   (o Core..:? "checkIntervalSec") Core.<*> (o Core..:? "description")
...
instance Core.ToJSON HealthCheck where
        toJSON HealthCheck{..}
          = Core.object
              (Core.catMaybes
                 [("checkIntervalSec" Core..=) Core.<$> checkIntervalSec,
...
instance Core.FromJSON Label where
        parseJSON
          = Core.withObject "Label"
              (\ o ->
                 Label Core.<$> (o Core..:? "key") Core.<*> (o Core..:? "value"))
...
newMetadata 
    ::  Metadata
newMetadata
  = Metadata{fingerPrint = Core.Nothing, items = Core.Nothing}
instance Core.FromJSON Metadata where
        parseJSON
          = Core.withObject "Metadata"
              (\ o ->
                 Metadata Core.<$>
                   (o Core..:? "fingerPrint") Core.<*> (o Core..:? "items"))
...
newMetadataItem 
    ::  MetadataItem
newMetadataItem
  = MetadataItem{key = Core.Nothing, value = Core.Nothing}
instance Core.FromJSON MetadataItem where
        parseJSON
          = Core.withObject "MetadataItem"
              (\ o ->
                 MetadataItem Core.<$>
...
newNetworkInterface 
    ::  NetworkInterface
newNetworkInterface
  = NetworkInterface{accessConfigs = Core.Nothing,
                     network = Core.Nothing, networkIp = Core.Nothing}
instance Core.FromJSON NetworkInterface where
        parseJSON
          = Core.withObject "NetworkInterface"
              (\ o ->
                 NetworkInterface Core.<$>
                   (o Core..:? "accessConfigs") Core.<*> (o Core..:? "network")
...
newNewDisk 
    ::  NewDisk
newNewDisk
  = NewDisk{attachment = Core.Nothing, autoDelete = Core.Nothing,
            boot = Core.Nothing, initializeParams = Core.Nothing}
instance Core.FromJSON NewDisk where
        parseJSON
          = Core.withObject "NewDisk"
              (\ o ->
                 NewDisk Core.<$>
                   (o Core..:? "attachment") Core.<*> (o Core..:? "autoDelete")
                     Core.<*> (o Core..:? "boot")
          ...
          = Core.object
              (Core.catMaybes
                 [("attachment" Core..=) Core.<$> attachment,
                  ...
                  ("boot" Core..=) Core.<$> boot,
                  ("initializeParams" Core..=) Core.<$> initializeParams])
...
newNewDiskInitializeParams 
    ::  NewDiskInitializeParams
newNewDiskInitializeParams
  = NewDiskInitializeParams{diskSizeGb = Core.Nothing,
                            diskType = Core.Nothing, sourceImage = Core.Nothing}
instance Core.FromJSON NewDiskInitializeParams where
        parseJSON
          = Core.withObject "NewDiskInitializeParams"
              (\ o ->
                 NewDiskInitializeParams Core.<$>
...
newPool 
    ::  Pool
newPool
  = Pool{autoRestart = Core.Nothing, baseInstanceName = Core.Nothing,
         currentNumReplicas = Core.Nothing, description = Core.Nothing,
         healthChecks = Core.Nothing, initialNumReplicas = Core.Nothing,
         labels = Core.Nothing, name = Core.Nothing,
         numReplicas = Core.Nothing, resourceViews = Core.Nothing,
         selfLink = Core.Nothing, targetPool = Core.Nothing,
         targetPools = Core.Nothing, template = Core.Nothing,
         type' = Core.Nothing}
instance Core.FromJSON Pool where
        parseJSON
          = Core.withObject "Pool"
              (\ o ->
                 Pool Core.<$>
                   (o Core..:? "autoRestart") Core.<*> (o Core..:? "baseInstanceName")
          ...
          = Core.object
              (Core.catMaybes
                 [("autoRestart" Core..=) Core.<$> autoRestart,
                  ("baseInstanceName" Core..=) Core.<$> baseInstanceName,
                  ("currentNumReplicas" Core..=) Core.<$> currentNumReplicas,
                  ("description" Core..=) Core.<$> description,
                  ("healthChecks" Core..=) Core.<$> healthChecks,
                  ("initialNumReplicas" Core..=) Core.<$> initialNumReplicas,
...
newPoolsDeleteRequest 
    ::  PoolsDeleteRequest
newPoolsDeleteRequest
  = PoolsDeleteRequest{abandonInstances = Core.Nothing}
instance Core.FromJSON PoolsDeleteRequest where
        parseJSON
          = Core.withObject "PoolsDeleteRequest"
              (\ o ->
                 PoolsDeleteRequest Core.<$> (o Core..:? "abandonInstances"))

instance Core.ToJSON PoolsDeleteRequest where
        toJSON PoolsDeleteRequest{..}
          = Core.object
              (Core.catMaybes
                 [("abandonInstances" Core..=) Core.<$> abandonInstances])
...
newPoolsListResponse 
    ::  PoolsListResponse
newPoolsListResponse
  = PoolsListResponse{nextPageToken = Core.Nothing,
                      resources = Core.Nothing}
instance Core.FromJSON PoolsListResponse where
        parseJSON
          = Core.withObject "PoolsListResponse"
              (\ o ->
                 PoolsListResponse Core.<$>
...
newReplica 
    ::  Replica
newReplica
  = Replica{name = Core.Nothing, selfLink = Core.Nothing,
            status = Core.Nothing}
instance Core.FromJSON Replica where
        parseJSON
          = Core.withObject "Replica"
              (\ o ->
                 Replica Core.<$>
                   (o Core..:? "name") Core.<*> (o Core..:? "selfLink") Core.<*>
                     (o Core..:? "status"))
...
newReplicaStatus 
    ::  ReplicaStatus
newReplicaStatus
  = ReplicaStatus{details = Core.Nothing, state = Core.Nothing,
                  templateVersion = Core.Nothing, vmLink = Core.Nothing,
...
instance Core.FromJSON ReplicasDeleteRequest where
        parseJSON
          = Core.withObject "ReplicasDeleteRequest"
              (\ o ->
                 ReplicasDeleteRequest Core.<$> (o Core..:? "abandonInstance"))

instance Core.ToJSON ReplicasDeleteRequest where
        toJSON ReplicasDeleteRequest{..}
          = Core.object
              (Core.catMaybes
                 [("abandonInstance" Core..=) Core.<$> abandonInstance])
...
newReplicasListResponse 
    ::  ReplicasListResponse
newReplicasListResponse
  = ReplicasListResponse{nextPageToken = Core.Nothing,
                         resources = Core.Nothing}
instance Core.FromJSON ReplicasListResponse where
        parseJSON
          = Core.withObject "ReplicasListResponse"
              (\ o ->
                 ReplicasListResponse Core.<$>
                   (o Core..:? "nextPageToken") Core.<*> (o Core..:? "resources"))
...
newTemplate 
    ::  Template
newTemplate
  = Template{action = Core.Nothing, healthChecks = Core.Nothing,
             version = Core.Nothing, vmParams = Core.Nothing}
instance Core.FromJSON Template where
        parseJSON
          = Core.withObject "Template"
              (\ o ->
                 Template Core.<$>
                   (o Core..:? "action") Core.<*> (o Core..:? "healthChecks") Core.<*>
                     (o Core..:? "version")
...
newVmParams 
    ::  VmParams
newVmParams
  = VmParams{baseInstanceName = Core.Nothing,
             canIpForward = Core.Nothing, description = Core.Nothing,
             disksToAttach = Core.Nothing, disksToCreate = Core.Nothing,
             machineType = Core.Nothing, metadata = Core.Nothing,
             networkInterfaces = Core.Nothing, onHostMaintenance = Core.Nothing,
             serviceAccounts = Core.Nothing, tags = Core.Nothing}
instance Core.FromJSON VmParams where
        parseJSON
          = Core.withObject "VmParams"
              (\ o ->
                 VmParams Core.<$>
                   (o Core..:? "baseInstanceName") Core.<*>
                     (o Core..:? "canIpForward")
                     Core.<*> (o Core..:? "description")
