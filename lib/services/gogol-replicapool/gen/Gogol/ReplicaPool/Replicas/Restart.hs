...
newReplicaPoolReplicasRestart poolName projectName replicaName zone
  = ReplicaPoolReplicasRestart{poolName = poolName,
                               projectName = projectName, replicaName = replicaName, zone = zone}
instance Core.GoogleRequest ReplicaPoolReplicasRestart where
        type Rs ReplicaPoolReplicasRestart = Replica
        type Scopes ReplicaPoolReplicasRestart =
             '[CloudPlatform'FullControl, Ndev'Cloudman,
               Replicapool'FullControl]
        requestClient ReplicaPoolReplicasRestart{..}
          = go projectName zone poolName replicaName (Core.Just Core.AltJSON)
              replicaPoolService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ReplicaPoolReplicasRestartResource)
                      Core.mempty
