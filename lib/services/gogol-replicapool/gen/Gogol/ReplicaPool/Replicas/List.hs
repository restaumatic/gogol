                       ...
                       Core.QueryParam "maxResults" Core.Int32 Core.:>
                         Core.QueryParam "pageToken" Core.Text Core.:>
                           Core.QueryParam "alt" Core.AltJSON Core.:>
                             Core.Get '[Core.JSON] ReplicasListResponse
...
newReplicaPoolReplicasList poolName projectName zone
  = ReplicaPoolReplicasList{maxResults = 500,
                            pageToken = Core.Nothing, poolName = poolName,
                            projectName = projectName, zone = zone}
instance Core.GoogleRequest ReplicaPoolReplicasList where
        type Rs ReplicaPoolReplicasList = ReplicasListResponse
        type Scopes ReplicaPoolReplicasList =
             '[CloudPlatform'FullControl, CloudPlatform'ReadOnly, Ndev'Cloudman,
               Ndev'Cloudman'Readonly, Replicapool'FullControl,
               Replicapool'Readonly]
        requestClient ReplicaPoolReplicasList{..}
          = go projectName zone poolName (Core.Just maxResults) pageToken
