               ...
               Core.Capture "zone" Core.Text Core.:>
                 "pools" Core.:>
                   Core.QueryParam "alt" Core.AltJSON Core.:>
                     Core.ReqBody '[Core.JSON] Pool Core.:> Core.Post '[Core.JSON] Pool
...
newReplicaPoolPoolsInsert payload projectName zone
  = ReplicaPoolPoolsInsert{payload = payload,
...
instance Core.GoogleRequest ReplicaPoolPoolsInsert where
        ...
        type Scopes ReplicaPoolPoolsInsert =
             '[CloudPlatform'FullControl, Ndev'Cloudman,
               Replicapool'FullControl]
        requestClient ReplicaPoolPoolsInsert{..}
          = go projectName zone (Core.Just Core.AltJSON) payload
              replicaPoolService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ReplicaPoolPoolsInsertResource)
                      Core.mempty
