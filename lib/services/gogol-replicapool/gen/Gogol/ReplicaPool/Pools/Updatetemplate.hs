...
newReplicaPoolPoolsUpdatetemplate payload poolName projectName zone
  = ReplicaPoolPoolsUpdatetemplate{payload = payload,
                                   poolName = poolName, projectName = projectName, zone = zone}
instance Core.GoogleRequest ReplicaPoolPoolsUpdatetemplate where
        type Rs ReplicaPoolPoolsUpdatetemplate = ()
        type Scopes ReplicaPoolPoolsUpdatetemplate =
             '[CloudPlatform'FullControl, Ndev'Cloudman,
               Replicapool'FullControl]
        requestClient ReplicaPoolPoolsUpdatetemplate{..}
          = go projectName zone poolName (Core.Just Core.AltJSON) payload
              replicaPoolService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ReplicaPoolPoolsUpdatetemplateResource)
                      Core.mempty
