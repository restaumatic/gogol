               ...
               Core.Capture "zone" Core.Text Core.:>
                 "pools" Core.:>
                   Core.Capture "poolName" Core.Text Core.:>
                     "resize" Core.:>
                       Core.QueryParam "numReplicas" Core.Int32 Core.:>
                         Core.QueryParam "alt" Core.AltJSON Core.:>
                           Core.Post '[Core.JSON] Pool
...
newReplicaPoolPoolsResize poolName projectName zone
  = ReplicaPoolPoolsResize{numReplicas = Core.Nothing,
...
instance Core.GoogleRequest ReplicaPoolPoolsResize where
        ...
        type Scopes ReplicaPoolPoolsResize =
             '[CloudPlatform'FullControl, Ndev'Cloudman,
               Replicapool'FullControl]
        requestClient ReplicaPoolPoolsResize{..}
          = go projectName zone poolName numReplicas (Core.Just Core.AltJSON)
              replicaPoolService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ReplicaPoolPoolsResizeResource)
                      Core.mempty
