               ...
               Core.QueryParam "uploadType" Core.Text Core.:>
                 Core.QueryParam "upload_protocol" Core.Text Core.:>
                   Core.QueryParam "alt" Core.AltJSON Core.:>
                     Core.ReqBody '[Core.JSON] UpdateInstanceConfigRequest Core.:>
...
newSpannerProjectsInstanceConfigsPatch 
    :: 
                                       Core.Text
       -- ^  A unique identifier for the instance configuration. Values are of the form @projects\/\/instanceConfigs\/a-z*@. User instance configuration must start with @custom-@. See 'name'.
    -> UpdateInstanceConfigRequest
       -- ^  Multipart request metadata. See 'payload'.
    -> SpannerProjectsInstanceConfigsPatch
newSpannerProjectsInstanceConfigsPatch name payload
  = SpannerProjectsInstanceConfigsPatch{xgafv = Core.Nothing,
                                        accessToken = Core.Nothing, callback = Core.Nothing,
                                        name = name, payload = payload, uploadType = Core.Nothing,
                                        uploadProtocol = Core.Nothing}
instance Core.GoogleRequest SpannerProjectsInstanceConfigsPatch
         where
        type Rs SpannerProjectsInstanceConfigsPatch = Operation
        ...
        requestClient SpannerProjectsInstanceConfigsPatch{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy SpannerProjectsInstanceConfigsPatchResource)
                      Core.mempty
