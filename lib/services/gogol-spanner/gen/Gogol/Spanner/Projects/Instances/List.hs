             ...
             Core.QueryParam "access_token" Core.Text Core.:>
               Core.QueryParam "callback" Core.Text Core.:>
                 Core.QueryParam "filter" Core.Text Core.:>
                   Core.QueryParam "instanceDeadline" Core.DateTime Core.:>
...
newSpannerProjectsInstancesList parent
  = SpannerProjectsInstancesList{xgafv = Core.Nothing,
                                 accessToken = Core.Nothing, callback = Core.Nothing,
                                 filter = Core.Nothing, instanceDeadline = Core.Nothing,
                                 pageSize = Core.Nothing, pageToken = Core.Nothing, parent = parent,
                                 uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest SpannerProjectsInstancesList where
        type Rs SpannerProjectsInstancesList = ListInstancesResponse
        type Scopes SpannerProjectsInstancesList =
             '[CloudPlatform'FullControl, Spanner'Admin]
        requestClient SpannerProjectsInstancesList{..}
          = go parent xgafv accessToken callback filter instanceDeadline
              pageSize
              pageToken
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy SpannerProjectsInstancesListResource)
                      Core.mempty
