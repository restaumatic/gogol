...
newSpannerProjectsInstancesGetIamPolicy payload resource
  = SpannerProjectsInstancesGetIamPolicy{xgafv = Core.Nothing,
                                         accessToken = Core.Nothing, callback = Core.Nothing,
                                         payload = payload, resource = resource,
                                         uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest SpannerProjectsInstancesGetIamPolicy
         where
        type Rs SpannerProjectsInstancesGetIamPolicy = Policy
        type Scopes SpannerProjectsInstancesGetIamPolicy =
             '[CloudPlatform'FullControl, Spanner'Admin]
        requestClient SpannerProjectsInstancesGetIamPolicy{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy SpannerProjectsInstancesGetIamPolicyResource)
                      Core.mempty
