...
newSpannerProjectsInstancesGet name
  = SpannerProjectsInstancesGet{xgafv = Core.Nothing,
                                accessToken = Core.Nothing, callback = Core.Nothing,
                                fieldMask = Core.Nothing, name = name, uploadType = Core.Nothing,
                                uploadProtocol = Core.Nothing}
instance Core.GoogleRequest SpannerProjectsInstancesGet where
        type Rs SpannerProjectsInstancesGet = Instance
        type Scopes SpannerProjectsInstancesGet =
             '[CloudPlatform'FullControl, Spanner'Admin]
        requestClient SpannerProjectsInstancesGet{..}
          = go name xgafv accessToken callback fieldMask uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              spannerService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy SpannerProjectsInstancesGetResource)
                      Core.mempty
