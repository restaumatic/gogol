...
module Gogol.Spanner.Internal.Product
  (
    ...
    -- * PartitionEndRecord
    PartitionEndRecord (..),
    newPartitionEndRecord,

    -- * PartitionEventRecord
    PartitionEventRecord (..),
    newPartitionEventRecord,
...
newAdaptMessageResponse 
    ::  AdaptMessageResponse
newAdaptMessageResponse
  = AdaptMessageResponse{last = Core.Nothing, payload = Core.Nothing,
          ...
          = Core.withObject "AdaptMessageResponse"
              (\ o ->
                 AdaptMessageResponse Core.<$>
                   (o Core..:? "last") Core.<*> (o Core..:? "payload") Core.<*>
                     (o Core..:? "stateUpdates"))
...
-- | Spanner Change Streams enable customers to capture and stream out changes to their Spanner databases in real-time. A change stream can be created with option partition/mode=\'IMMUTABLE/KEY/RANGE\' or partition/mode=\'MUTABLE/KEY/RANGE\'. This message is only used in Change Streams created with the option partition/mode=\'MUTABLE/KEY/RANGE\'. Spanner automatically creates a special Table-Valued Function (TVF) along with each Change Streams. The function provides access to the change stream\'s records. The function is named READ/ (where is the name of the change stream), and it returns a table with only one column called ChangeRecord.
--
-- /See:/ 'newChangeStreamRecord' smart constructor.
data ChangeStreamRecord = ChangeStreamRecord
    {
      -- | Data change record describing a data change for a change stream partition.
      dataChangeRecord :: (Core.Maybe DataChangeRecord)
      -- | Heartbeat record describing a heartbeat for a change stream partition.
    , heartbeatRecord :: (Core.Maybe HeartbeatRecord)
      -- | Partition end record describing a terminated change stream partition.
    , partitionEndRecord :: (Core.Maybe PartitionEndRecord)
      -- | Partition event record describing key range changes for a change stream partition.
    , partitionEventRecord :: (Core.Maybe PartitionEventRecord)
      -- | Partition start record describing a new change stream partition.
    , partitionStartRecord :: (Core.Maybe PartitionStartRecord)
    }
    deriving (Core.Eq, Core.Show, Core.Generic)

-- | Creates a value of 'ChangeStreamRecord' with the minimum fields required to make a request.
newChangeStreamRecord 
    ::  ChangeStreamRecord
newChangeStreamRecord
  = ChangeStreamRecord{dataChangeRecord = Core.Nothing,
                       heartbeatRecord = Core.Nothing, partitionEndRecord = Core.Nothing,
                       partitionEventRecord = Core.Nothing,
                       partitionStartRecord = Core.Nothing}
instance Core.FromJSON ChangeStreamRecord where
        parseJSON
          = Core.withObject "ChangeStreamRecord"
              (\ o ->
                 ChangeStreamRecord Core.<$>
                   (o Core..:? "dataChangeRecord") Core.<*>
                     (o Core..:? "heartbeatRecord")
                     Core.<*> (o Core..:? "partitionEndRecord")
                     Core.<*> (o Core..:? "partitionEventRecord")
                     Core.<*> (o Core..:? "partitionStartRecord"))

instance Core.ToJSON ChangeStreamRecord where
        toJSON ChangeStreamRecord{..}
          = Core.object
              (Core.catMaybes
                 [("dataChangeRecord" Core..=) Core.<$> dataChangeRecord,
                  ("heartbeatRecord" Core..=) Core.<$> heartbeatRecord,
                  ("partitionEndRecord" Core..=) Core.<$> partitionEndRecord,
                  ("partitionEventRecord" Core..=) Core.<$> partitionEventRecord,
                  ("partitionStartRecord" Core..=) Core.<$> partitionStartRecord])
...
data ColumnMetadata = ColumnMetadata
    {
      ...
      isPrimaryKey :: (Core.Maybe Core.Bool)
      -- | Name of the column.
    , name :: (Core.Maybe Core.Text)
      -- | Ordinal position of the column based on the original table definition in the schema starting with a value of 1.
    , ordinalPosition :: (Core.Maybe Core.Int64)
      -- | Type of the column.
    , type' :: (Core.Maybe Type)
    }
    deriving (Core.Eq, Core.Show, Core.Generic)

-- | Creates a value of 'ColumnMetadata' with the minimum fields required to make a request.
newColumnMetadata 
    ::  ColumnMetadata
newColumnMetadata
  = ColumnMetadata{isPrimaryKey = Core.Nothing, name = Core.Nothing,
                   ordinalPosition = Core.Nothing, type' = Core.Nothing}
instance Core.FromJSON ColumnMetadata where
        parseJSON
          = Core.withObject "ColumnMetadata"
              (\ o ->
                 ColumnMetadata Core.<$>
                   (o Core..:? "isPrimaryKey") Core.<*> (o Core..:? "name") Core.<*>
                     (o Core..:? "ordinalPosition" Core.<&> Core.fmap Core.fromAsText)
                     Core.<*> (o Core..:? "type"))

instance Core.ToJSON ColumnMetadata where
        toJSON ColumnMetadata{..}
          = Core.object
              (Core.catMaybes
                 [("isPrimaryKey" Core..=) Core.<$> isPrimaryKey,
                  ("name" Core..=) Core.<$> name,
                  ("ordinalPosition" Core..=) Core.. Core.AsText Core.<$>
                    ordinalPosition,
                  ("type" Core..=) Core.<$> type'])
...
data CommitResponse = CommitResponse
    ...
    , snapshotTimestamp :: (Core.Maybe Core.DateTime)
    }
    deriving (Core.Eq, Core.Show, Core.Generic)
...
newCommitResponse
  = CommitResponse{commitStats = Core.Nothing,
                   ...
                   snapshotTimestamp = Core.Nothing}
          ...
          = Core.object
              (Core.catMaybes
                 [("commitStats" Core..=) Core.<$> commitStats,
                  ("commitTimestamp" Core..=) Core.<$> commitTimestamp,
                  ("precommitToken" Core..=) Core.<$> precommitToken,
                  ("snapshotTimestamp" Core..=) Core.<$> snapshotTimestamp])
...
--
-- /See:/ 'newDataChangeRecord' smart constructor.
data DataChangeRecord = DataChangeRecord
    {
      -- | Provides metadata describing the columns associated with the mods listed below.
      columnMetadata :: (Core.Maybe [ColumnMetadata])
      -- | Indicates the timestamp in which the change was committed. DataChangeRecord.commit/timestamps, PartitionStartRecord.start/timestamps, PartitionEventRecord.commit/timestamps, and PartitionEndRecord.end/timestamps can have the same value in the same partition.
    , commitTimestamp :: (Core.Maybe Core.DateTime)
    ...
    , isLastRecordInTransactionInPartition :: (Core.Maybe Core.Bool)
    ...
    , isSystemTransaction :: (Core.Maybe Core.Bool)
      -- | Describes the type of change.
    , modType :: (Core.Maybe DataChangeRecord_ModType)
    ...
    , numberOfPartitionsInTransaction :: (Core.Maybe Core.Int32)
    ...
    , numberOfRecordsInTransaction :: (Core.Maybe Core.Int32)
      -- | Record sequence numbers are unique and monotonically increasing (but not necessarily contiguous) for a specific timestamp across record types in the same partition. To guarantee ordered processing, the reader should process records (of potentially different types) in record/sequence order for a specific timestamp in the same partition. The record sequence number ordering across partitions is only meaningful in the context of a specific transaction. Record sequence numbers are unique across partitions for a specific transaction. Sort the DataChangeRecords for the same server/transaction/id by record/sequence to reconstruct the ordering of the changes within the transaction.
    , recordSequence :: (Core.Maybe Core.Text)
      -- | Provides a globally unique string that represents the transaction in which the change was committed. Multiple transactions can have the same commit timestamp, but each transaction has a unique server/transaction/id.
    , serverTransactionId :: (Core.Maybe Core.Text)
    ...
    , valueCaptureType :: (Core.Maybe DataChangeRecord_ValueCaptureType)
    }
    deriving (Core.Eq, Core.Show, Core.Generic)

-- | Creates a value of 'DataChangeRecord' with the minimum fields required to make a request.
newDataChangeRecord 
    ::  DataChangeRecord
newDataChangeRecord
  = DataChangeRecord{columnMetadata = Core.Nothing,
                     commitTimestamp = Core.Nothing,
                     isLastRecordInTransactionInPartition = Core.Nothing,
                     isSystemTransaction = Core.Nothing, modType = Core.Nothing,
                     mods = Core.Nothing,
                     numberOfPartitionsInTransaction = Core.Nothing,
                     numberOfRecordsInTransaction = Core.Nothing,
                     recordSequence = Core.Nothing, serverTransactionId = Core.Nothing,
                     table = Core.Nothing, transactionTag = Core.Nothing,
                     valueCaptureType = Core.Nothing}
instance Core.FromJSON DataChangeRecord where
        parseJSON
          = Core.withObject "DataChangeRecord"
              (\ o ->
                 DataChangeRecord Core.<$>
                   (o Core..:? "columnMetadata") Core.<*>
                     (o Core..:? "commitTimestamp")
                     Core.<*> (o Core..:? "isLastRecordInTransactionInPartition")
                     Core.<*> (o Core..:? "isSystemTransaction")
                     Core.<*> (o Core..:? "modType")
                     Core.<*> (o Core..:? "mods")
                     Core.<*> (o Core..:? "numberOfPartitionsInTransaction")
                     Core.<*> (o Core..:? "numberOfRecordsInTransaction")
                     Core.<*> (o Core..:? "recordSequence")
                     Core.<*> (o Core..:? "serverTransactionId")
                     Core.<*> (o Core..:? "table")
                     Core.<*> (o Core..:? "transactionTag")
                     Core.<*> (o Core..:? "valueCaptureType"))

instance Core.ToJSON DataChangeRecord where
        toJSON DataChangeRecord{..}
          = Core.object
              (Core.catMaybes
                 [("columnMetadata" Core..=) Core.<$> columnMetadata,
                  ("commitTimestamp" Core..=) Core.<$> commitTimestamp,
                  ("isLastRecordInTransactionInPartition" Core..=) Core.<$>
                    isLastRecordInTransactionInPartition,
                  ("isSystemTransaction" Core..=) Core.<$> isSystemTransaction,
                  ("modType" Core..=) Core.<$> modType,
                  ("mods" Core..=) Core.<$> mods,
                  ("numberOfPartitionsInTransaction" Core..=) Core.<$>
                    numberOfPartitionsInTransaction,
                  ("numberOfRecordsInTransaction" Core..=) Core.<$>
                    numberOfRecordsInTransaction,
                  ("recordSequence" Core..=) Core.<$> recordSequence,
                  ("serverTransactionId" Core..=) Core.<$> serverTransactionId,
                  ("table" Core..=) Core.<$> table,
                  ("transactionTag" Core..=) Core.<$> transactionTag,
                  ("valueCaptureType" Core..=) Core.<$> valueCaptureType])
...
--
-- /See:/ 'newHeartbeatRecord' smart constructor.
newtype HeartbeatRecord = HeartbeatRecord
    {
...
-- | Creates a value of 'HeartbeatRecord' with the minimum fields required to make a request.
newHeartbeatRecord 
    ::  HeartbeatRecord
newHeartbeatRecord = HeartbeatRecord{timestamp = Core.Nothing}
instance Core.FromJSON HeartbeatRecord where
        parseJSON
          = Core.withObject "HeartbeatRecord"
              (\ o -> HeartbeatRecord Core.<$> (o Core..:? "timestamp"))

instance Core.ToJSON HeartbeatRecord where
        toJSON HeartbeatRecord{..}
          = Core.object
              (Core.catMaybes [("timestamp" Core..=) Core.<$> timestamp])
...
--
-- /See:/ 'newMod' smart constructor.
data Mod = Mod
    {
      -- | Returns the value of the primary key of the modified row.
      keys :: (Core.Maybe [ModValue])
      -- | Returns the new values after the change for the modified columns. Always empty for DELETE.
    , newValues' :: (Core.Maybe [ModValue])
      -- | Returns the old values before the change for the modified columns. Always empty for INSERT, or if old values are not being captured specified by value/capture/type.
    , oldValues :: (Core.Maybe [ModValue])
    }
    deriving (Core.Eq, Core.Show, Core.Generic)

-- | Creates a value of 'Mod' with the minimum fields required to make a request.
newMod 
    ::  Mod
newMod
  = Mod{keys = Core.Nothing, newValues' = Core.Nothing,
        oldValues = Core.Nothing}
instance Core.FromJSON Mod where
        parseJSON
          = Core.withObject "Mod"
              (\ o ->
                 Mod Core.<$>
                   (o Core..:? "keys") Core.<*> (o Core..:? "newValues") Core.<*>
                     (o Core..:? "oldValues"))

instance Core.ToJSON Mod where
        toJSON Mod{..}
          = Core.object
              (Core.catMaybes
                 [("keys" Core..=) Core.<$> keys,
                  ("newValues" Core..=) Core.<$> newValues',
                  ("oldValues" Core..=) Core.<$> oldValues])


-- | Returns the value and associated metadata for a particular field of the Mod.
--
-- /See:/ 'newModValue' smart constructor.
data ModValue = ModValue
    {
      ...
      columnMetadataIndex :: (Core.Maybe Core.Int32)
      -- | The value of the column.
    , value :: (Core.Maybe Core.Value)
    }
    deriving (Core.Eq, Core.Show, Core.Generic)

-- | Creates a value of 'ModValue' with the minimum fields required to make a request.
newModValue 
    ::  ModValue
newModValue
  = ModValue{columnMetadataIndex = Core.Nothing,
             value = Core.Nothing}
instance Core.FromJSON ModValue where
        parseJSON
          = Core.withObject "ModValue"
              (\ o ->
                 ModValue Core.<$>
                   (o Core..:? "columnMetadataIndex") Core.<*> (o Core..:? "value"))

instance Core.ToJSON ModValue where
        toJSON ModValue{..}
          = Core.object
              (Core.catMaybes
                 [("columnMetadataIndex" Core..=) Core.<$> columnMetadataIndex,
                  ("value" Core..=) Core.<$> value])


-- | Describes move-in of the key ranges into the change stream partition identified by partition/token. To maintain processing the changes for a particular key in timestamp order, the query processing the change stream partition identified by partition/token should not advance beyond the partition event record commit timestamp until the queries processing the source change stream partitions have processed all change stream records with timestamps \<= the partition event record commit timestamp.
--
-- /See:/ 'newMoveInEvent' smart constructor.
newtype MoveInEvent = MoveInEvent
    {
      ...
      sourcePartitionToken :: (Core.Maybe Core.Text)
    }
    deriving (Core.Eq, Core.Show, Core.Generic)

-- | Creates a value of 'MoveInEvent' with the minimum fields required to make a request.
newMoveInEvent 
    ::  MoveInEvent
newMoveInEvent = MoveInEvent{sourcePartitionToken = Core.Nothing}
instance Core.FromJSON MoveInEvent where
        parseJSON
          = Core.withObject "MoveInEvent"
              (\ o -> MoveInEvent Core.<$> (o Core..:? "sourcePartitionToken"))

instance Core.ToJSON MoveInEvent where
        toJSON MoveInEvent{..}
          = Core.object
              (Core.catMaybes
                 [("sourcePartitionToken" Core..=) Core.<$> sourcePartitionToken])
...
-- | Describes move-out of the key ranges out of the change stream partition identified by partition/token. To maintain processing the changes for a particular key in timestamp order, the query processing the MoveOutEvent in the partition identified by partition/token should inform the queries processing the destination partitions that they can unblock and proceed processing records past the commit_timestamp.
--
-- /See:/ 'newMoveOutEvent' smart constructor.
newtype MoveOutEvent = MoveOutEvent
    {
      ...
      destinationPartitionToken :: (Core.Maybe Core.Text)
    }
    deriving (Core.Eq, Core.Show, Core.Generic)

-- | Creates a value of 'MoveOutEvent' with the minimum fields required to make a request.
newMoveOutEvent 
    ::  MoveOutEvent
newMoveOutEvent
  = MoveOutEvent{destinationPartitionToken = Core.Nothing}
instance Core.FromJSON MoveOutEvent where
        parseJSON
          = Core.withObject "MoveOutEvent"
              (\ o ->
                 MoveOutEvent Core.<$> (o Core..:? "destinationPartitionToken"))

instance Core.ToJSON MoveOutEvent where
        toJSON MoveOutEvent{..}
          = Core.object
              (Core.catMaybes
                 [("destinationPartitionToken" Core..=) Core.<$>
...
--
-- /See:/ 'newPartitionEndRecord' smart constructor.
data PartitionEndRecord = PartitionEndRecord
    {
      -- | End timestamp at which the change stream partition is terminated. All changes generated by this partition will have timestamps \<= end/timestamp. DataChangeRecord.commit/timestamps, PartitionStartRecord.start/timestamps, PartitionEventRecord.commit/timestamps, and PartitionEndRecord.end_timestamps can have the same value in the same partition. PartitionEndRecord is the last record returned for a partition.
      endTimestamp :: (Core.Maybe Core.DateTime)
    ...
    , partitionToken :: (Core.Maybe Core.Text)
...
-- | Creates a value of 'PartitionEndRecord' with the minimum fields required to make a request.
newPartitionEndRecord 
    ::  PartitionEndRecord
newPartitionEndRecord
  = PartitionEndRecord{endTimestamp = Core.Nothing,
                       partitionToken = Core.Nothing, recordSequence = Core.Nothing}
instance Core.FromJSON PartitionEndRecord where
        parseJSON
          = Core.withObject "PartitionEndRecord"
              (\ o ->
                 PartitionEndRecord Core.<$>
                   (o Core..:? "endTimestamp") Core.<*> (o Core..:? "partitionToken")
                     Core.<*> (o Core..:? "recordSequence"))

instance Core.ToJSON PartitionEndRecord where
        toJSON PartitionEndRecord{..}
          = Core.object
              (Core.catMaybes
...
-- | A partition event record describes key range changes for a change stream partition. The changes to a row defined by its primary key can be captured in one change stream partition for a specific time range, and then be captured in a different change stream partition for a different time range. This movement of key ranges across change stream partitions is a reflection of activities, such as Spanner\'s dynamic splitting and load balancing, etc. Processing this event is needed if users want to guarantee processing of the changes for any key in timestamp order. If time ordered processing of changes for a primary key is not needed, this event can be ignored. To guarantee time ordered processing for each primary key, if the event describes move-ins, the reader of this partition needs to wait until the readers of the source partitions have processed all records with timestamps \<= this PartitionEventRecord.commit_timestamp, before advancing beyond this PartitionEventRecord. If the event describes move-outs, the
-- reader can notify the readers of the destination partitions that they can continue processing.
--
-- /See:/ 'newPartitionEventRecord' smart constructor.
data PartitionEventRecord = PartitionEventRecord
    {
      -- | Indicates the commit timestamp at which the key range change occurred. DataChangeRecord.commit/timestamps, PartitionStartRecord.start/timestamps, PartitionEventRecord.commit/timestamps, and PartitionEndRecord.end/timestamps can have the same value in the same partition.
      commitTimestamp :: (Core.Maybe Core.DateTime)
      -- | Set when one or more key ranges are moved into the change stream partition identified by partition/token. Example: Two key ranges are moved into partition (P1) from partition (P2) and partition (P3) in a single transaction at timestamp T. The PartitionEventRecord returned in P1 will reflect the move as: PartitionEventRecord { commit/timestamp: T partition/token: \"P1\" move/in/events { source/partition/token: \"P2\" } move/in/events { source/partition/token: \"P3\" } } The PartitionEventRecord returned in P2 will reflect the move as: PartitionEventRecord { commit/timestamp: T partition/token: \"P2\" move/out/events { destination/partition/token: \"P1\" } } The PartitionEventRecord returned in P3 will reflect the move as: PartitionEventRecord { commit/timestamp: T partition/token: \"P3\" move/out/events { destination/partition_token: \"P1\" } }
    , moveInEvents :: (Core.Maybe [MoveInEvent])
      -- | Set when one or more key ranges are moved out of the change stream partition identified by partition/token. Example: Two key ranges are moved out of partition (P1) to partition (P2) and partition (P3) in a single transaction at timestamp T. The PartitionEventRecord returned in P1 will reflect the move as: PartitionEventRecord { commit/timestamp: T partition/token: \"P1\" move/out/events { destination/partition/token: \"P2\" } move/out/events { destination/partition/token: \"P3\" } } The PartitionEventRecord returned in P2 will reflect the move as: PartitionEventRecord { commit/timestamp: T partition/token: \"P2\" move/in/events { source/partition/token: \"P1\" } } The PartitionEventRecord returned in P3 will reflect the move as: PartitionEventRecord { commit/timestamp: T partition/token: \"P3\" move/in/events { source/partition_token: \"P1\" } }
    , moveOutEvents :: (Core.Maybe [MoveOutEvent])
...
-- | Creates a value of 'PartitionEventRecord' with the minimum fields required to make a request.
newPartitionEventRecord 
    ::  PartitionEventRecord
newPartitionEventRecord
  = PartitionEventRecord{commitTimestamp = Core.Nothing,
                         moveInEvents = Core.Nothing, moveOutEvents = Core.Nothing,
                         partitionToken = Core.Nothing, recordSequence = Core.Nothing}
instance Core.FromJSON PartitionEventRecord where
        parseJSON
          = Core.withObject "PartitionEventRecord"
              (\ o ->
                 PartitionEventRecord Core.<$>
                   (o Core..:? "commitTimestamp") Core.<*> (o Core..:? "moveInEvents")
                     Core.<*> (o Core..:? "moveOutEvents")
                     Core.<*> (o Core..:? "partitionToken")
                     Core.<*> (o Core..:? "recordSequence"))

instance Core.ToJSON PartitionEventRecord where
        toJSON PartitionEventRecord{..}
          = Core.object
              (Core.catMaybes
                 [("commitTimestamp" Core..=) Core.<$> commitTimestamp,
                  ("moveInEvents" Core..=) Core.<$> moveInEvents,
                  ("moveOutEvents" Core..=) Core.<$> moveOutEvents,
                  ("partitionToken" Core..=) Core.<$> partitionToken,
                  ("recordSequence" Core..=) Core.<$> recordSequence])
...
--
-- /See:/ 'newPartitionStartRecord' smart constructor.
data PartitionStartRecord = PartitionStartRecord
    {
      -- | Unique partition identifiers to be used in queries.
      partitionTokens :: (Core.Maybe [Core.Text])
      -- | Record sequence numbers are unique and monotonically increasing (but not necessarily contiguous) for a specific timestamp across record types in the same partition. To guarantee ordered processing, the reader should process records (of potentially different types) in record_sequence order for a specific timestamp in the same partition.
    , recordSequence :: (Core.Maybe Core.Text)
      -- | Start timestamp at which the partitions should be queried to return change stream records with timestamps >= start/timestamp. DataChangeRecord.commit/timestamps, PartitionStartRecord.start/timestamps, PartitionEventRecord.commit/timestamps, and PartitionEndRecord.end_timestamps can have the same value in the same partition.
    , startTimestamp :: (Core.Maybe Core.DateTime)
    }
    deriving (Core.Eq, Core.Show, Core.Generic)

-- | Creates a value of 'PartitionStartRecord' with the minimum fields required to make a request.
newPartitionStartRecord 
    ::  PartitionStartRecord
newPartitionStartRecord
  = PartitionStartRecord{partitionTokens = Core.Nothing,
                         recordSequence = Core.Nothing, startTimestamp = Core.Nothing}
instance Core.FromJSON PartitionStartRecord where
        parseJSON
          = Core.withObject "PartitionStartRecord"
              (\ o ->
                 PartitionStartRecord Core.<$>
                   (o Core..:? "partitionTokens") Core.<*>
                     (o Core..:? "recordSequence")
                     Core.<*> (o Core..:? "startTimestamp"))

instance Core.ToJSON PartitionStartRecord where
        toJSON PartitionStartRecord{..}
          = Core.object
              (Core.catMaybes
                 [("partitionTokens" Core..=) Core.<$> partitionTokens,
                  ("recordSequence" Core..=) Core.<$> recordSequence,
