...
--
-- /See:/ 'newSignSshPublicKeyRequest' smart constructor.
data SignSshPublicKeyRequest = SignSshPublicKeyRequest
    {
      ...
      appEngineInstance :: (Core.Maybe Core.Text)
      -- | The Compute instance to sign the SSH public key for. Expected format: projects\/{project}\/zones\/{zone}\/instances\/{numeric/instance/id}
    , computeInstance :: (Core.Maybe Core.Text)
...
-- | Creates a value of 'SignSshPublicKeyRequest' with the minimum fields required to make a request.
newSignSshPublicKeyRequest 
    ::  SignSshPublicKeyRequest
newSignSshPublicKeyRequest
  = SignSshPublicKeyRequest{appEngineInstance = Core.Nothing,
                            computeInstance = Core.Nothing, serviceAccount = Core.Nothing,
                            sshPublicKey = Core.Nothing}
instance Core.FromJSON SignSshPublicKeyRequest where
        parseJSON
          = Core.withObject "SignSshPublicKeyRequest"
              (\ o ->
                 SignSshPublicKeyRequest Core.<$>
                   (o Core..:? "appEngineInstance") Core.<*>
                     (o Core..:? "computeInstance")
                     Core.<*> (o Core..:? "serviceAccount")
                     Core.<*> (o Core..:? "sshPublicKey"))

instance Core.ToJSON SignSshPublicKeyRequest where
        toJSON SignSshPublicKeyRequest{..}
          = Core.object
              (Core.catMaybes
                 [("appEngineInstance" Core..=) Core.<$> appEngineInstance,
                  ("computeInstance" Core..=) Core.<$> computeInstance,
...
--
-- /See:/ 'newSignSshPublicKeyResponse' smart constructor.
newtype SignSshPublicKeyResponse = SignSshPublicKeyResponse
    {
      -- | The signed SSH public key to use in the SSH handshake.
      signedSshPublicKey :: (Core.Maybe Core.Text)
    }
    deriving (Core.Eq, Core.Show, Core.Generic)

-- | Creates a value of 'SignSshPublicKeyResponse' with the minimum fields required to make a request.
newSignSshPublicKeyResponse 
    ::  SignSshPublicKeyResponse
newSignSshPublicKeyResponse
  = SignSshPublicKeyResponse{signedSshPublicKey = Core.Nothing}
instance Core.FromJSON SignSshPublicKeyResponse where
        parseJSON
          = Core.withObject "SignSshPublicKeyResponse"
              (\ o ->
                 SignSshPublicKeyResponse Core.<$>
                   (o Core..:? "signedSshPublicKey"))

instance Core.ToJSON SignSshPublicKeyResponse where
        toJSON SignSshPublicKeyResponse{..}
          = Core.object
              (Core.catMaybes
                 [("signedSshPublicKey" Core..=) Core.<$> signedSshPublicKey])
