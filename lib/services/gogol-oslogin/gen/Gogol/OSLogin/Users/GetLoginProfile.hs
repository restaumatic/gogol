             ...
             Core.QueryParam "access_token" Core.Text Core.:>
               Core.QueryParam "callback" Core.Text Core.:>
                 Core.QueryParam "projectId" Core.Text Core.:>
                   Core.QueryParam "systemId" Core.Text Core.:>
                     Core.QueryParam "uploadType" Core.Text Core.:>
                       Core.QueryParam "upload_protocol" Core.Text Core.:>
                         Core.QueryParam "alt" Core.AltJSON Core.:>
                           Core.Get '[Core.JSON] LoginProfile
...
newOSLoginUsersGetLoginProfile name
  = OSLoginUsersGetLoginProfile{xgafv = Core.Nothing,
                                accessToken = Core.Nothing, callback = Core.Nothing, name = name,
                                projectId = Core.Nothing, systemId = Core.Nothing,
                                uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest OSLoginUsersGetLoginProfile where
        type Rs OSLoginUsersGetLoginProfile = LoginProfile
        type Scopes OSLoginUsersGetLoginProfile =
             '[CloudPlatform'FullControl, CloudPlatform'ReadOnly,
               Compute'FullControl, Compute'Readonly]
        requestClient OSLoginUsersGetLoginProfile{..}
          = go name xgafv accessToken callback projectId systemId uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              oSLoginService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy OSLoginUsersGetLoginProfileResource)
                      Core.mempty
