           ...
           Core.Capture "packageName" Core.Text Core.:>
             "generatedApks" Core.:>
               Core.Capture "versionCode" Core.Int32 Core.:>
                 "downloads" Core.:>
                   Core.CaptureMode "downloadId" "download" Core.Text Core.:>
...
newAndroidPublisherGeneratedapksDownload 
    ...
    -> AndroidPublisherGeneratedapksDownload
newAndroidPublisherGeneratedapksDownload downloadId packageName
  versionCode
  = AndroidPublisherGeneratedapksDownload{xgafv = Core.Nothing,
                                          accessToken = Core.Nothing, callback = Core.Nothing,
                                          downloadId = downloadId, packageName = packageName,
                                          uploadType = Core.Nothing, uploadProtocol = Core.Nothing,
                                          versionCode = versionCode}
instance Core.GoogleRequest AndroidPublisherGeneratedapksDownload
         where
        type Rs AndroidPublisherGeneratedapksDownload = ()
        type Scopes AndroidPublisherGeneratedapksDownload =
             '[Androidpublisher'FullControl]
        requestClient AndroidPublisherGeneratedapksDownload{..}
          = go packageName versionCode downloadId xgafv accessToken callback
              uploadType
              uploadProtocol
...
instance Core.GoogleRequest
         ...
         where
        type Rs (Core.MediaDownload AndroidPublisherGeneratedapksDownload)
             = Core.Stream
        type Scopes
               (Core.MediaDownload AndroidPublisherGeneratedapksDownload)
