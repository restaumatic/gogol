       ...
       "v1" Core.:>
         "activities" Core.:>
           Core.Capture "activityId" Core.Text Core.:>
             "people" Core.:>
               Core.Capture "collection" PeopleListByActivityCollection Core.:>
...
newPlusDomainsPeopleListByActivity activityId collection
  = PlusDomainsPeopleListByActivity{activityId = activityId,
                                    collection = collection, maxResults = 20,
                                    pageToken = Core.Nothing}
instance Core.GoogleRequest PlusDomainsPeopleListByActivity where
        type Rs PlusDomainsPeopleListByActivity = PeopleFeed
        type Scopes PlusDomainsPeopleListByActivity =
             '[Plus'Login, Plus'Stream'Read]
        requestClient PlusDomainsPeopleListByActivity{..}
          = go activityId collection (Core.Just maxResults) pageToken
              (Core.Just Core.AltJSON)
              plusDomainsService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy PlusDomainsPeopleListByActivityResource)
                      Core.mempty
