       ...
       "v1" Core.:>
         "people" Core.:>
           Core.Capture "userId" Core.Text Core.:>
             "people" Core.:>
               Core.Capture "collection" PeopleListCollection Core.:>
...
newPlusDomainsPeopleList collection userId
  = PlusDomainsPeopleList{collection = collection, maxResults = 100,
                          orderBy = Core.Nothing, pageToken = Core.Nothing, userId = userId}
instance Core.GoogleRequest PlusDomainsPeopleList where
        type Rs PlusDomainsPeopleList = PeopleFeed
        type Scopes PlusDomainsPeopleList =
             '[Plus'Circles'Read, Plus'Login, Plus'Me]
        requestClient PlusDomainsPeopleList{..}
          = go userId collection (Core.Just maxResults) orderBy pageToken
              (Core.Just Core.AltJSON)
              plusDomainsService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy PlusDomainsPeopleListResource)
                      Core.mempty
