...
-- |
-- Module      : Gogol.FireStore.Projects.Databases.Operations.Get
...
newFireStoreProjectsDatabasesOperationsGet name
  = FireStoreProjectsDatabasesOperationsGet{xgafv = Core.Nothing,
                                            accessToken = Core.Nothing, callback = Core.Nothing,
                                            name = name, uploadType = Core.Nothing,
                                            uploadProtocol = Core.Nothing}
instance Core.GoogleRequest FireStoreProjectsDatabasesOperationsGet
         where
        type Rs FireStoreProjectsDatabasesOperationsGet =
             GoogleLongrunningOperation
        type Scopes FireStoreProjectsDatabasesOperationsGet =
             '[CloudPlatform'FullControl, Datastore'FullControl]
        requestClient FireStoreProjectsDatabasesOperationsGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy FireStoreProjectsDatabasesOperationsGetResource)
                      Core.mempty
