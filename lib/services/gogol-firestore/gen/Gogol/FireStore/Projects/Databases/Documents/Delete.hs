         ...
         Core.QueryParam "$.xgafv" Xgafv Core.:>
           Core.QueryParam "access_token" Core.Text Core.:>
             Core.QueryParam "callback" Core.Text Core.:>
               Core.QueryParam "currentDocument.exists" Core.Bool Core.:>
                 Core.QueryParam "currentDocument.updateTime" Core.DateTime Core.:>
...
newFireStoreProjectsDatabasesDocumentsDelete name
  = FireStoreProjectsDatabasesDocumentsDelete{xgafv = Core.Nothing,
                                              accessToken = Core.Nothing, callback = Core.Nothing,
                                              currentDocumentExists = Core.Nothing,
                                              currentDocumentUpdateTime = Core.Nothing, name = name,
                                              uploadType = Core.Nothing,
                                              uploadProtocol = Core.Nothing}
instance Core.GoogleRequest
           FireStoreProjectsDatabasesDocumentsDelete
         where
        type Rs FireStoreProjectsDatabasesDocumentsDelete = Empty
        type Scopes FireStoreProjectsDatabasesDocumentsDelete =
             '[CloudPlatform'FullControl, Datastore'FullControl]
        requestClient FireStoreProjectsDatabasesDocumentsDelete{..}
          = go name xgafv accessToken callback currentDocumentExists
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy FireStoreProjectsDatabasesDocumentsDeleteResource)
                      Core.mempty
