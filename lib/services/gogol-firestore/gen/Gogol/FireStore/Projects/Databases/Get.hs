               ...
               Core.QueryParam "uploadType" Core.Text Core.:>
                 Core.QueryParam "upload_protocol" Core.Text Core.:>
                   Core.QueryParam "alt" Core.AltJSON Core.:>
                     Core.Get '[Core.JSON] GoogleFirestoreAdminV1Database
...
newFireStoreProjectsDatabasesGet name
  = FireStoreProjectsDatabasesGet{xgafv = Core.Nothing,
                                  accessToken = Core.Nothing, callback = Core.Nothing, name = name,
                                  uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest FireStoreProjectsDatabasesGet where
        type Rs FireStoreProjectsDatabasesGet =
             GoogleFirestoreAdminV1Database
        type Scopes FireStoreProjectsDatabasesGet =
        ...
        requestClient FireStoreProjectsDatabasesGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy FireStoreProjectsDatabasesGetResource)
                      Core.mempty
