             ...
             "deals" Core.:>
               "delete" Core.:>
                 Core.QueryParam "alt" Core.AltJSON Core.:>
                   Core.ReqBody '[Core.JSON] DeleteOrderDealsRequest Core.:>
...
newAdExchangeBuyerMarketplacedealsDelete payload proposalId
  = AdExchangeBuyerMarketplacedealsDelete{payload = payload,
                                          proposalId = proposalId}
instance Core.GoogleRequest AdExchangeBuyerMarketplacedealsDelete
         where
        type Rs AdExchangeBuyerMarketplacedealsDelete =
             DeleteOrderDealsResponse
        type Scopes AdExchangeBuyerMarketplacedealsDelete =
             '[Adexchange'Buyer]
        requestClient AdExchangeBuyerMarketplacedealsDelete{..}
          = go proposalId (Core.Just Core.AltJSON) payload
              adExchangeBuyerService
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy AdExchangeBuyerMarketplacedealsDeleteResource)
                      Core.mempty
