             ...
             "deals" Core.:>
               "update" Core.:>
                 Core.QueryParam "alt" Core.AltJSON Core.:>
                   Core.ReqBody '[Core.JSON] EditAllOrderDealsRequest Core.:>
                     Core.Post '[Core.JSON] EditAllOrderDealsResponse
...
newAdExchangeBuyerMarketplacedealsUpdate payload proposalId
  = AdExchangeBuyerMarketplacedealsUpdate{payload = payload,
                                          proposalId = proposalId}
instance Core.GoogleRequest AdExchangeBuyerMarketplacedealsUpdate
         where
        type Rs AdExchangeBuyerMarketplacedealsUpdate =
             EditAllOrderDealsResponse
        type Scopes AdExchangeBuyerMarketplacedealsUpdate =
             '[Adexchange'Buyer]
        requestClient AdExchangeBuyerMarketplacedealsUpdate{..}
          = go proposalId (Core.Just Core.AltJSON) payload
              adExchangeBuyerService
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy AdExchangeBuyerMarketplacedealsUpdateResource)
                      Core.mempty
