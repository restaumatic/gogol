           ...
           "search" Core.:>
             Core.QueryParam "pqlQuery" Core.Text Core.:>
               Core.QueryParam "alt" Core.AltJSON Core.:>
                 Core.Get '[Core.JSON] GetOffersResponse
...
instance Core.GoogleRequest AdExchangeBuyerProductsSearch where
        type Rs AdExchangeBuyerProductsSearch = GetOffersResponse
        type Scopes AdExchangeBuyerProductsSearch = '[Adexchange'Buyer]
        requestClient AdExchangeBuyerProductsSearch{..}
          = go pqlQuery (Core.Just Core.AltJSON) adExchangeBuyerService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy AdExchangeBuyerProductsSearchResource)
                      Core.mempty
