         ...
         "creatives" Core.:>
           Core.QueryParams "accountId" Core.Int32 Core.:>
             Core.QueryParams "buyerCreativeId" Core.Text Core.:>
               Core.QueryParam "dealsStatusFilter" CreativesListDealsStatusFilter
                 Core.:>
...
newAdExchangeBuyerCreativesList 
    ::  AdExchangeBuyerCreativesList
newAdExchangeBuyerCreativesList
  = AdExchangeBuyerCreativesList{accountId = Core.Nothing,
                                 buyerCreativeId = Core.Nothing, dealsStatusFilter = Core.Nothing,
                                 maxResults = Core.Nothing, openAuctionStatusFilter = Core.Nothing,
                                 pageToken = Core.Nothing}
instance Core.GoogleRequest AdExchangeBuyerCreativesList where
        type Rs AdExchangeBuyerCreativesList = CreativesList
        type Scopes AdExchangeBuyerCreativesList = '[Adexchange'Buyer]
        requestClient AdExchangeBuyerCreativesList{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy AdExchangeBuyerCreativesListResource)
                      Core.mempty
