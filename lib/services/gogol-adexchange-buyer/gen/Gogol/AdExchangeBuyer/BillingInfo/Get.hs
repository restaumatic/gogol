...
instance Core.GoogleRequest AdExchangeBuyerBillingInfoGet where
        type Rs AdExchangeBuyerBillingInfoGet = BillingInfo
        type Scopes AdExchangeBuyerBillingInfoGet = '[Adexchange'Buyer]
        requestClient AdExchangeBuyerBillingInfoGet{..}
          = go accountId (Core.Just Core.AltJSON) adExchangeBuyerService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy AdExchangeBuyerBillingInfoGetResource)
                      Core.mempty
