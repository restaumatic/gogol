         ...
         "proposals" Core.:>
           Core.Capture "proposalId" Core.Text Core.:>
             Core.Capture "revisionNumber" Core.Int64 Core.:>
               Core.Capture "updateAction" ProposalsUpdateUpdateAction Core.:>
...
newAdExchangeBuyerProposalsUpdate 
    ...
    -> AdExchangeBuyerProposalsUpdate
newAdExchangeBuyerProposalsUpdate payload proposalId revisionNumber
  updateAction
  = AdExchangeBuyerProposalsUpdate{payload = payload,
                                   proposalId = proposalId, revisionNumber = revisionNumber,
                                   updateAction = updateAction}
instance Core.GoogleRequest AdExchangeBuyerProposalsUpdate where
        type Rs AdExchangeBuyerProposalsUpdate = Proposal
        type Scopes AdExchangeBuyerProposalsUpdate = '[Adexchange'Buyer]
        requestClient AdExchangeBuyerProposalsUpdate{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy AdExchangeBuyerProposalsUpdateResource)
                      Core.mempty
