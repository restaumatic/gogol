...
newAdExchangeBuyerPretargetingConfigInsert accountId payload
  = AdExchangeBuyerPretargetingConfigInsert{accountId = accountId,
                                            payload = payload}
instance Core.GoogleRequest AdExchangeBuyerPretargetingConfigInsert
         where
        type Rs AdExchangeBuyerPretargetingConfigInsert =
             PretargetingConfig
        type Scopes AdExchangeBuyerPretargetingConfigInsert =
             '[Adexchange'Buyer]
        requestClient AdExchangeBuyerPretargetingConfigInsert{..}
          = go accountId (Core.Just Core.AltJSON) payload
              adExchangeBuyerService
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy AdExchangeBuyerPretargetingConfigInsertResource)
                      Core.mempty
