...
--
-- List all \"Other contacts\", that is contacts that are not in a contact group. \"Other contacts\" are typically auto created contacts from interactions. Sync tokens expire 7 days after the full sync. A request with an expired sync token will get an error with an <https://cloud.google.com/apis/design/errors#error_info google.rpc.ErrorInfo> with reason \"EXPIRED/SYNC/TOKEN\". In the case of such an error clients should make a full sync request without a @sync_token@. The first page of a full sync request has an additional quota. If the quota is exceeded, a 429 error will be returned. This quota is fixed and can not be increased. When the @sync_token@ is specified, resources deleted since the last sync will be returned as a person with @PersonMetadata.deleted@ set to true. When the @page_token@ or @sync_token@ is specified, all other request parameters must match the first call. Writes may have a propagation delay of several minutes for sync requests. Incremental syncs are not intended for read-after-write use
-- cases. See example usage at </people/v1/other-contacts#list_the_users_other_contacts_that_have_changed List the user\'s other contacts that have changed>.
--
                 ...
                 Core.QueryParam "pageToken" Core.Text Core.:>
                   Core.QueryParam "readMask" Core.FieldMask Core.:>
                     Core.QueryParam "requestSyncToken" Core.Bool Core.:>
                       Core.QueryParams "sources" OtherContactsListSources Core.:>
                         Core.QueryParam "syncToken" Core.Text Core.:>
                           Core.QueryParam "uploadType" Core.Text Core.:>
                             Core.QueryParam "upload_protocol" Core.Text Core.:>
                               Core.QueryParam "alt" Core.AltJSON Core.:>
                                 Core.Get '[Core.JSON] ListOtherContactsResponse
...
newPeopleOtherContactsList 
    ::  PeopleOtherContactsList
newPeopleOtherContactsList
  = PeopleOtherContactsList{xgafv = Core.Nothing,
                            accessToken = Core.Nothing, callback = Core.Nothing,
                            pageSize = Core.Nothing, pageToken = Core.Nothing,
                            readMask = Core.Nothing, requestSyncToken = Core.Nothing,
                            sources = Core.Nothing, syncToken = Core.Nothing,
                            uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest PeopleOtherContactsList where
        type Rs PeopleOtherContactsList = ListOtherContactsResponse
        type Scopes PeopleOtherContactsList = '[Contacts'Other'Readonly]
        requestClient PeopleOtherContactsList{..}
          = go xgafv accessToken callback pageSize pageToken readMask
              requestSyncToken
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy PeopleOtherContactsListResource)
                      Core.mempty
