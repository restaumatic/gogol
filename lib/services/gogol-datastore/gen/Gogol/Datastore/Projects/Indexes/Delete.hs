...
newDatastoreProjectsIndexesDelete indexId projectId
  = DatastoreProjectsIndexesDelete{xgafv = Core.Nothing,
                                   accessToken = Core.Nothing, callback = Core.Nothing,
                                   indexId = indexId, projectId = projectId,
                                   uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DatastoreProjectsIndexesDelete where
        type Rs DatastoreProjectsIndexesDelete = GoogleLongrunningOperation
        type Scopes DatastoreProjectsIndexesDelete =
             '[CloudPlatform'FullControl, Datastore'FullControl]
        requestClient DatastoreProjectsIndexesDelete{..}
          = go projectId indexId xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              datastoreService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DatastoreProjectsIndexesDeleteResource)
                      Core.mempty
