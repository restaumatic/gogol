...
newDatastoreProjectsIndexesGet indexId projectId
  = DatastoreProjectsIndexesGet{xgafv = Core.Nothing,
                                accessToken = Core.Nothing, callback = Core.Nothing,
                                indexId = indexId, projectId = projectId,
                                uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DatastoreProjectsIndexesGet where
        type Rs DatastoreProjectsIndexesGet = GoogleDatastoreAdminV1Index
        ...
        requestClient DatastoreProjectsIndexesGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DatastoreProjectsIndexesGetResource)
                      Core.mempty
