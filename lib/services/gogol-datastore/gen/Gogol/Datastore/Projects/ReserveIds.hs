...
type DatastoreProjectsReserveIdsResource =
     "v1" Core.:>
       "projects" Core.:>
         Core.CaptureMode "projectId" "reserveIds" Core.Text Core.:>
                 ...
                 Core.QueryParam "uploadType" Core.Text Core.:>
                   Core.QueryParam "upload_protocol" Core.Text Core.:>
                     Core.QueryParam "alt" Core.AltJSON Core.:>
                       Core.ReqBody '[Core.JSON] ReserveIdsRequest Core.:>
                         Core.Post '[Core.JSON] ReserveIdsResponse
...
newDatastoreProjectsReserveIds payload projectId
  = DatastoreProjectsReserveIds{xgafv = Core.Nothing,
                                accessToken = Core.Nothing, callback = Core.Nothing,
                                payload = payload, projectId = projectId,
                                uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DatastoreProjectsReserveIds where
        type Rs DatastoreProjectsReserveIds = ReserveIdsResponse
        type Scopes DatastoreProjectsReserveIds =
        ...
        requestClient DatastoreProjectsReserveIds{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DatastoreProjectsReserveIdsResource)
                      Core.mempty
