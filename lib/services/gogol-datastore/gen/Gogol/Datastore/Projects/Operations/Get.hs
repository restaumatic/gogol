...
newDatastoreProjectsOperationsGet name
  = DatastoreProjectsOperationsGet{xgafv = Core.Nothing,
                                   accessToken = Core.Nothing, callback = Core.Nothing, name = name,
                                   uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DatastoreProjectsOperationsGet where
        type Rs DatastoreProjectsOperationsGet = GoogleLongrunningOperation
        ...
        requestClient DatastoreProjectsOperationsGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DatastoreProjectsOperationsGetResource)
                      Core.mempty
