...
--
-- /See:/ <https://cloud.google.com/sensitive-data-protection/docs/ Sensitive Data Protection (DLP) Reference> for @dlp.organizations.deidentifyTemplates.get@.
module Gogol.DLP.Organizations.DeidentifyTemplates.Get
    (
    -- * Resource
...
newDLPOrganizationsDeidentifyTemplatesGet name
  = DLPOrganizationsDeidentifyTemplatesGet{xgafv = Core.Nothing,
                                           accessToken = Core.Nothing, callback = Core.Nothing,
                                           name = name, uploadType = Core.Nothing,
                                           uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DLPOrganizationsDeidentifyTemplatesGet
         where
        type Rs DLPOrganizationsDeidentifyTemplatesGet =
             GooglePrivacyDlpV2DeidentifyTemplate
        type Scopes DLPOrganizationsDeidentifyTemplatesGet =
             '[CloudPlatform'FullControl]
        requestClient DLPOrganizationsDeidentifyTemplatesGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy DLPOrganizationsDeidentifyTemplatesGetResource)
                      Core.mempty
