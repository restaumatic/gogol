...
--
-- /See:/ <https://cloud.google.com/sensitive-data-protection/docs/ Sensitive Data Protection (DLP) Reference> for @dlp.organizations.inspectTemplates.patch@.
module Gogol.DLP.Organizations.InspectTemplates.Patch
    (
    -- * Resource
...
newDLPOrganizationsInspectTemplatesPatch name payload
  = DLPOrganizationsInspectTemplatesPatch{xgafv = Core.Nothing,
                                          accessToken = Core.Nothing, callback = Core.Nothing,
                                          name = name, payload = payload, uploadType = Core.Nothing,
                                          uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DLPOrganizationsInspectTemplatesPatch
         where
        type Rs DLPOrganizationsInspectTemplatesPatch =
             GooglePrivacyDlpV2InspectTemplate
        type Scopes DLPOrganizationsInspectTemplatesPatch =
             '[CloudPlatform'FullControl]
        requestClient DLPOrganizationsInspectTemplatesPatch{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy DLPOrganizationsInspectTemplatesPatchResource)
                      Core.mempty
