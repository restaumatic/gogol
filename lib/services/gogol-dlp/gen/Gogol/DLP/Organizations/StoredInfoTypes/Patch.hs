...
--
-- /See:/ <https://cloud.google.com/sensitive-data-protection/docs/ Sensitive Data Protection (DLP) Reference> for @dlp.organizations.storedInfoTypes.patch@.
module Gogol.DLP.Organizations.StoredInfoTypes.Patch
    (
    -- * Resource
...
newDLPOrganizationsStoredInfoTypesPatch name payload
  = DLPOrganizationsStoredInfoTypesPatch{xgafv = Core.Nothing,
                                         accessToken = Core.Nothing, callback = Core.Nothing,
                                         name = name, payload = payload, uploadType = Core.Nothing,
                                         uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DLPOrganizationsStoredInfoTypesPatch
         where
        type Rs DLPOrganizationsStoredInfoTypesPatch =
             GooglePrivacyDlpV2StoredInfoType
        type Scopes DLPOrganizationsStoredInfoTypesPatch =
             '[CloudPlatform'FullControl]
        requestClient DLPOrganizationsStoredInfoTypesPatch{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy DLPOrganizationsStoredInfoTypesPatchResource)
                      Core.mempty
