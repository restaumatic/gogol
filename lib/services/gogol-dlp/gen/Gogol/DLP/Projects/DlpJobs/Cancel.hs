...
--
-- /See:/ <https://cloud.google.com/sensitive-data-protection/docs/ Sensitive Data Protection (DLP) Reference> for @dlp.projects.dlpJobs.cancel@.
module Gogol.DLP.Projects.DlpJobs.Cancel
    (
    -- * Resource
               ...
               Core.QueryParam "uploadType" Core.Text Core.:>
                 Core.QueryParam "upload_protocol" Core.Text Core.:>
                   Core.QueryParam "alt" Core.AltJSON Core.:>
                     Core.ReqBody '[Core.JSON] GooglePrivacyDlpV2CancelDlpJobRequest
...
newDLPProjectsDlpJobsCancel name payload
  = DLPProjectsDlpJobsCancel{xgafv = Core.Nothing,
...
instance Core.GoogleRequest DLPProjectsDlpJobsCancel where
        type Rs DLPProjectsDlpJobsCancel = GoogleProtobufEmpty
        type Scopes DLPProjectsDlpJobsCancel = '[CloudPlatform'FullControl]
        requestClient DLPProjectsDlpJobsCancel{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DLPProjectsDlpJobsCancelResource)
                      Core.mempty
