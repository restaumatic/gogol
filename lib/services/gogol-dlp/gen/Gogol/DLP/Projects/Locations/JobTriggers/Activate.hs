...
--
-- /See:/ <https://cloud.google.com/sensitive-data-protection/docs/ Sensitive Data Protection (DLP) Reference> for @dlp.projects.locations.jobTriggers.activate@.
module Gogol.DLP.Projects.Locations.JobTriggers.Activate
    (
    -- * Resource
...
newDLPProjectsLocationsJobTriggersActivate name payload
  = DLPProjectsLocationsJobTriggersActivate{xgafv = Core.Nothing,
                                            accessToken = Core.Nothing, callback = Core.Nothing,
                                            name = name, payload = payload,
                                            uploadType = Core.Nothing,
                                            uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DLPProjectsLocationsJobTriggersActivate
         where
        type Rs DLPProjectsLocationsJobTriggersActivate =
             GooglePrivacyDlpV2DlpJob
        type Scopes DLPProjectsLocationsJobTriggersActivate =
             '[CloudPlatform'FullControl]
        requestClient DLPProjectsLocationsJobTriggersActivate{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy DLPProjectsLocationsJobTriggersActivateResource)
                      Core.mempty
