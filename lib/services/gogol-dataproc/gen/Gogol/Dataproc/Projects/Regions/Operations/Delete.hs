...
newDataprocProjectsRegionsOperationsDelete name
  = DataprocProjectsRegionsOperationsDelete{xgafv = Core.Nothing,
                                            accessToken = Core.Nothing, callback = Core.Nothing,
                                            name = name, uploadType = Core.Nothing,
                                            uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DataprocProjectsRegionsOperationsDelete
         where
        type Rs DataprocProjectsRegionsOperationsDelete = Empty
        type Scopes DataprocProjectsRegionsOperationsDelete =
             '[CloudPlatform'FullControl]
        requestClient DataprocProjectsRegionsOperationsDelete{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy DataprocProjectsRegionsOperationsDeleteResource)
                      Core.mempty
