...
--
-- /See:/ <https://developers.google.com/games/ Google Play Games Services Management API Reference> for @gamesManagement.events.reset@.
module Gogol.GamesManagement.Events.Reset
    (
    -- * Resource
...
newGamesManagementEventsReset eventId
  = GamesManagementEventsReset{xgafv = Core.Nothing,
                               accessToken = Core.Nothing, callback = Core.Nothing,
                               eventId = eventId, uploadType = Core.Nothing,
                               uploadProtocol = Core.Nothing}
instance Core.GoogleRequest GamesManagementEventsReset where
        type Rs GamesManagementEventsReset = ()
        type Scopes GamesManagementEventsReset = '[Games'FullControl]
        requestClient GamesManagementEventsReset{..}
          = go eventId xgafv accessToken callback uploadType uploadProtocol
              (Core.Just Core.AltJSON)
              gamesManagementService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy GamesManagementEventsResetResource)
                      Core.mempty
