...
--
-- /See:/ <https://developers.google.com/games/ Google Play Games Services Management API Reference> for @gamesManagement.players.unhide@.
module Gogol.GamesManagement.Players.Unhide
    (
    -- * Resource
...
newGamesManagementPlayersUnhide applicationId playerId
  = GamesManagementPlayersUnhide{xgafv = Core.Nothing,
                                 accessToken = Core.Nothing, applicationId = applicationId,
                                 callback = Core.Nothing, playerId = playerId,
                                 uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest GamesManagementPlayersUnhide where
        type Rs GamesManagementPlayersUnhide = ()
        type Scopes GamesManagementPlayersUnhide = '[Games'FullControl]
        requestClient GamesManagementPlayersUnhide{..}
          = go applicationId playerId xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              gamesManagementService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy GamesManagementPlayersUnhideResource)
                      Core.mempty
