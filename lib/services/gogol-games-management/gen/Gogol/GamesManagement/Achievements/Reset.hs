...
--
-- /See:/ <https://developers.google.com/games/ Google Play Games Services Management API Reference> for @gamesManagement.achievements.reset@.
module Gogol.GamesManagement.Achievements.Reset
    (
    -- * Resource
...
newGamesManagementAchievementsReset achievementId
  = GamesManagementAchievementsReset{xgafv = Core.Nothing,
                                     accessToken = Core.Nothing, achievementId = achievementId,
                                     callback = Core.Nothing, uploadType = Core.Nothing,
                                     uploadProtocol = Core.Nothing}
instance Core.GoogleRequest GamesManagementAchievementsReset where
        type Rs GamesManagementAchievementsReset = AchievementResetResponse
        type Scopes GamesManagementAchievementsReset = '[Games'FullControl]
        requestClient GamesManagementAchievementsReset{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy GamesManagementAchievementsResetResource)
                      Core.mempty
