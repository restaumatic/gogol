...
newStreetViewPublishPhotoDelete photoId
  = StreetViewPublishPhotoDelete{xgafv = Core.Nothing,
                                 accessToken = Core.Nothing, callback = Core.Nothing,
                                 photoId = photoId, uploadType = Core.Nothing,
                                 uploadProtocol = Core.Nothing}
instance Core.GoogleRequest StreetViewPublishPhotoDelete where
        type Rs StreetViewPublishPhotoDelete = Empty
        type Scopes StreetViewPublishPhotoDelete =
             '[Streetviewpublish'FullControl]
        requestClient StreetViewPublishPhotoDelete{..}
          = go photoId xgafv accessToken callback uploadType uploadProtocol
              (Core.Just Core.AltJSON)
              streetViewPublishService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy StreetViewPublishPhotoDeleteResource)
                      Core.mempty
