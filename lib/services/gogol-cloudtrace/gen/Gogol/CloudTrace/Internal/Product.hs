...
instance Core.FromJSON Annotation where
        parseJSON
          = Core.withObject "Annotation"
              (\ o ->
                 Annotation Core.<$>
                   (o Core..:? "attributes") Core.<*> (o Core..:? "description"))
...
newAttributeValue 
    ::  AttributeValue
newAttributeValue
  = AttributeValue{boolValue = Core.Nothing, intValue = Core.Nothing,
...
data Attributes = Attributes
    {
      -- | A set of attributes. Each attribute\'s key can be up to 128 bytes long. The value can be a string up to 256 bytes, a signed 64-bit integer, or the boolean values @true@ or @false@. For example: \"\/instance/id\": { \"string/value\": { \"value\": \"my-instance\" } } \"\/http\/request/bytes\": { \"int/value\": 300 } \"example.com\/myattribute\": { \"bool_value\": false }
...
newAttributes 
    ::  Attributes
newAttributes
  = Attributes{attributeMap = Core.Nothing,
...
instance Core.FromJSON Attributes_AttributeMap where
        parseJSON
          = Core.withObject "Attributes_AttributeMap"
              (\ o -> Attributes_AttributeMap Core.<$> (Core.parseJSONObject o))

instance Core.ToJSON Attributes_AttributeMap where
        toJSON Attributes_AttributeMap{..} = Core.toJSON additional
...
newLink 
    ::  Link
newLink
  = Link{attributes = Core.Nothing, spanId = Core.Nothing,
         traceId = Core.Nothing, type' = Core.Nothing}
instance Core.FromJSON Link where
        parseJSON
          = Core.withObject "Link"
              (\ o ->
                 Link Core.<$>
                   (o Core..:? "attributes") Core.<*> (o Core..:? "spanId") Core.<*>
                     (o Core..:? "traceId")
                     Core.<*> (o Core..:? "type"))
...
newLinks 
    ::  Links
newLinks
  = Links{droppedLinksCount = Core.Nothing, link = Core.Nothing}
instance Core.FromJSON Links where
        parseJSON
          = Core.withObject "Links"
              (\ o ->
                 Links Core.<$>
                   (o Core..:? "droppedLinksCount") Core.<*> (o Core..:? "link"))

instance Core.ToJSON Links where
        toJSON Links{..}
          = Core.object
              (Core.catMaybes
                 [("droppedLinksCount" Core..=) Core.<$> droppedLinksCount,
                  ("link" Core..=) Core.<$> link])
...
instance Core.ToJSON MessageEvent where
        toJSON MessageEvent{..}
          = Core.object
              (Core.catMaybes
                 [("compressedSizeBytes" Core..=) Core.. Core.AsText Core.<$>
                    compressedSizeBytes,
                  ("id" Core..=) Core.. Core.AsText Core.<$> id,
                  ("type" Core..=) Core.<$> type',
                  ("uncompressedSizeBytes" Core..=) Core.. Core.AsText Core.<$>
...
instance Core.FromJSON Module where
        parseJSON
          = Core.withObject "Module"
              (\ o ->
                 Module Core.<$>
                   (o Core..:? "buildId") Core.<*> (o Core..:? "module"))
...
newSpan 
    ::  Span
newSpan
  = Span{attributes = Core.Nothing, childSpanCount = Core.Nothing,
         displayName = Core.Nothing, endTime = Core.Nothing,
         links = Core.Nothing, name = Core.Nothing,
         parentSpanId = Core.Nothing,
         sameProcessAsParentSpan = Core.Nothing, spanId = Core.Nothing,
         spanKind = Core.Nothing, stackTrace = Core.Nothing,
...
instance Core.FromJSON Span where
        parseJSON
          = Core.withObject "Span"
              (\ o ->
                 Span Core.<$>
                   (o Core..:? "attributes") Core.<*> (o Core..:? "childSpanCount")
          ...
          = Core.object
              (Core.catMaybes
                 [("attributes" Core..=) Core.<$> attributes,
                  ...
                  ("links" Core..=) Core.<$> links, ("name" Core..=) Core.<$> name,
                  ("parentSpanId" Core..=) Core.<$> parentSpanId,
                  ("sameProcessAsParentSpan" Core..=) Core.<$>
...
newStackFrame 
    ::  StackFrame
newStackFrame
  = StackFrame{columnNumber = Core.Nothing, fileName = Core.Nothing,
               functionName = Core.Nothing, lineNumber = Core.Nothing,
               loadModule = Core.Nothing, originalFunctionName = Core.Nothing,
...
instance Core.FromJSON StackFrame where
        parseJSON
          = Core.withObject "StackFrame"
              (\ o ->
                 StackFrame Core.<$>
                   (o Core..:? "columnNumber" Core.<&> Core.fmap Core.fromAsText)
...
instance Core.ToJSON StackFrame where
        toJSON StackFrame{..}
          = Core.object
              (Core.catMaybes
                 [("columnNumber" Core..=) Core.. Core.AsText Core.<$> columnNumber,
                  ...
                  ("originalFunctionName" Core..=) Core.<$> originalFunctionName,
...
newStackFrames 
    ::  StackFrames
newStackFrames
  = StackFrames{droppedFramesCount = Core.Nothing,
                frame = Core.Nothing}
instance Core.FromJSON StackFrames where
        parseJSON
          = Core.withObject "StackFrames"
              (\ o ->
                 StackFrames Core.<$>
                   (o Core..:? "droppedFramesCount") Core.<*> (o Core..:? "frame"))

instance Core.ToJSON StackFrames where
        toJSON StackFrames{..}
          = Core.object
              (Core.catMaybes
                 [("droppedFramesCount" Core..=) Core.<$> droppedFramesCount,
...
newStackTrace 
    ::  StackTrace
newStackTrace
  = StackTrace{stackFrames = Core.Nothing,
               stackTraceHashId = Core.Nothing}
              ...
              (\ o ->
                 StackTrace Core.<$>
                   (o Core..:? "stackFrames") Core.<*>
                     (o Core..:? "stackTraceHashId" Core.<&> Core.fmap Core.fromAsText))
          ...
          = Core.object
              (Core.catMaybes
                 [("stackFrames" Core..=) Core.<$> stackFrames,
                  ("stackTraceHashId" Core..=) Core.. Core.AsText Core.<$>
                    stackTraceHashId])
...
newTimeEvent 
    ::  TimeEvent
newTimeEvent
  = TimeEvent{annotation = Core.Nothing, messageEvent = Core.Nothing,
              time = Core.Nothing}
instance Core.FromJSON TimeEvent where
        parseJSON
          = Core.withObject "TimeEvent"
              (\ o ->
                 TimeEvent Core.<$>
                   (o Core..:? "annotation") Core.<*> (o Core..:? "messageEvent")
                     Core.<*> (o Core..:? "time"))
...
newTimeEvents 
    ::  TimeEvents
newTimeEvents
  = TimeEvents{droppedAnnotationsCount = Core.Nothing,
               droppedMessageEventsCount = Core.Nothing, timeEvent = Core.Nothing}
              ...
              (\ o ->
                 TimeEvents Core.<$>
                   (o Core..:? "droppedAnnotationsCount") Core.<*>
                     (o Core..:? "droppedMessageEventsCount")
          ...
          = Core.object
              (Core.catMaybes
                 [("droppedAnnotationsCount" Core..=) Core.<$>
                  ...
                  ("droppedMessageEventsCount" Core..=) Core.<$>
...
newTruncatableString 
    ::  TruncatableString
newTruncatableString
  = TruncatableString{truncatedByteCount = Core.Nothing,
                      value = Core.Nothing}
          ...
          = Core.withObject "TruncatableString"
              (\ o ->
                 TruncatableString Core.<$>
                   (o Core..:? "truncatedByteCount") Core.<*> (o Core..:? "value"))

instance Core.ToJSON TruncatableString where
        toJSON TruncatableString{..}
          = Core.object
              (Core.catMaybes
                 [("truncatedByteCount" Core..=) Core.<$> truncatedByteCount,
