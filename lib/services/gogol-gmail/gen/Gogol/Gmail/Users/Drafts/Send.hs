...
--
-- /See:/ <https://developers.google.com/workspace/gmail/api/ Gmail API Reference> for @gmail.users.drafts.send@.
module Gogol.Gmail.Users.Drafts.Send
    (
    -- * Resource
                       ...
                       Core.QueryParam "uploadType" Core.Text Core.:>
                         Core.QueryParam "upload_protocol" Core.Text Core.:>
                           Core.QueryParam "alt" Core.AltJSON Core.:>
                             Core.ReqBody '[Core.JSON] Draft Core.:>
...
newGmailUsersDraftsSend payload
  = GmailUsersDraftsSend{xgafv = Core.Nothing,
...
instance Core.GoogleRequest GmailUsersDraftsSend where
        ...
        type Scopes GmailUsersDraftsSend =
        ...
        requestClient GmailUsersDraftsSend{..}
          ...
          where go Core.:<|> _
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy GmailUsersDraftsSendResource)
                      Core.mempty

instance Core.GoogleRequest (Core.MediaUpload GmailUsersDraftsSend)
         where
        type Rs (Core.MediaUpload GmailUsersDraftsSend) = Message
        type Scopes (Core.MediaUpload GmailUsersDraftsSend) =
             Core.Scopes GmailUsersDraftsSend
        requestClient (Core.MediaUpload GmailUsersDraftsSend{..} body)
          = go userId xgafv accessToken callback uploadType uploadProtocol
              (Core.Just Core.AltJSON)
