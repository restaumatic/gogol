...
-- Creates a new label.
--
-- /See:/ <https://developers.google.com/workspace/gmail/api/ Gmail API Reference> for @gmail.users.labels.create@.
module Gogol.Gmail.Users.Labels.Create
    (
    -- * Resource
...
newGmailUsersLabelsCreate payload
  = GmailUsersLabelsCreate{xgafv = Core.Nothing,
...
instance Core.GoogleRequest GmailUsersLabelsCreate where
        ...
        requestClient GmailUsersLabelsCreate{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy GmailUsersLabelsCreateResource)
                      Core.mempty
