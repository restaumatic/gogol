...
--
-- /See:/ <https://developers.google.com/workspace/gmail/api/ Gmail API Reference> for @gmail.users.labels.delete@.
module Gogol.Gmail.Users.Labels.Delete
    (
    -- * Resource
...
newGmailUsersLabelsDelete id
  = GmailUsersLabelsDelete{xgafv = Core.Nothing,
...
instance Core.GoogleRequest GmailUsersLabelsDelete where
        ...
        type Scopes GmailUsersLabelsDelete =
        ...
        requestClient GmailUsersLabelsDelete{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy GmailUsersLabelsDeleteResource)
                      Core.mempty
