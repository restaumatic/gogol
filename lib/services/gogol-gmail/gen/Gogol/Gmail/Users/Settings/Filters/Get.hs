...
-- Gets a filter.
--
-- /See:/ <https://developers.google.com/workspace/gmail/api/ Gmail API Reference> for @gmail.users.settings.filters.get@.
module Gogol.Gmail.Users.Settings.Filters.Get
    (
    -- * Resource
...
newGmailUsersSettingsFiltersGet id
  = GmailUsersSettingsFiltersGet{xgafv = Core.Nothing,
                                 accessToken = Core.Nothing, callback = Core.Nothing, id = id,
                                 uploadType = Core.Nothing, uploadProtocol = Core.Nothing,
                                 userId = "me"}
instance Core.GoogleRequest GmailUsersSettingsFiltersGet where
        type Rs GmailUsersSettingsFiltersGet = Filter
        type Scopes GmailUsersSettingsFiltersGet =
        ...
        requestClient GmailUsersSettingsFiltersGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy GmailUsersSettingsFiltersGetResource)
                      Core.mempty
