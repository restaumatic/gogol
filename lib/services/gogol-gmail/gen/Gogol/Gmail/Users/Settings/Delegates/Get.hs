...
--
-- /See:/ <https://developers.google.com/workspace/gmail/api/ Gmail API Reference> for @gmail.users.settings.delegates.get@.
module Gogol.Gmail.Users.Settings.Delegates.Get
    (
    -- * Resource
...
newGmailUsersSettingsDelegatesGet delegateEmail
  = GmailUsersSettingsDelegatesGet{xgafv = Core.Nothing,
                                   accessToken = Core.Nothing, callback = Core.Nothing,
                                   delegateEmail = delegateEmail, uploadType = Core.Nothing,
                                   uploadProtocol = Core.Nothing, userId = "me"}
instance Core.GoogleRequest GmailUsersSettingsDelegatesGet where
        type Rs GmailUsersSettingsDelegatesGet = Delegate
        type Scopes GmailUsersSettingsDelegatesGet =
             '[Gmail'FullControl, Gmail'Modify, Gmail'Readonly,
               Gmail'Settings'Basic]
        requestClient GmailUsersSettingsDelegatesGet{..}
          = go userId delegateEmail xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              gmailService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy GmailUsersSettingsDelegatesGetResource)
                      Core.mempty
