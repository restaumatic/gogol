...
--
-- Deletes a client-side encryption key pair permanently and immediately. You can only permanently delete key pairs that have been turned off for more than 30 days. To turn off a key pair, use the DisableCseKeyPair method. Gmail can\'t restore or decrypt any messages that were encrypted by an obliterated key. Authenticated users and Google Workspace administrators lose access to reading the encrypted messages. For administrators managing identities and keypairs for users in their organization, requests require authorization with a <https://developers.google.com/identity/protocols/OAuth2ServiceAccount service account> that has <https://developers.google.com/identity/protocols/OAuth2ServiceAccount#delegatingauthority domain-wide delegation authority> to impersonate users with the @https:\/\/www.googleapis.com\/auth\/gmail.settings.basic@ scope. For users managing their own identities and keypairs, requests require <https://support.google.com/a/answer/14153163 hardware key encryption> turned on and configured.
--
-- /See:/ <https://developers.google.com/workspace/gmail/api/ Gmail API Reference> for @gmail.users.settings.cse.keypairs.obliterate@.
module Gogol.Gmail.Users.Settings.Cse.Keypairs.Obliterate
    (
    -- * Resource
         ...
         "users" Core.:>
           Core.Capture "userId" Core.Text Core.:>
             "settings" Core.:>
               "cse" Core.:>
                 "keypairs" Core.:>
                   Core.CaptureMode "keyPairId" "obliterate" Core.Text Core.:>
                           ...
                           Core.QueryParam "uploadType" Core.Text Core.:>
                             Core.QueryParam "upload_protocol" Core.Text Core.:>
                               Core.QueryParam "alt" Core.AltJSON Core.:>
                                 Core.ReqBody '[Core.JSON] ObliterateCseKeyPairRequest Core.:>
                                   Core.Post '[Core.JSON] ()
...
newGmailUsersSettingsCseKeypairsObliterate keyPairId payload
  = GmailUsersSettingsCseKeypairsObliterate{xgafv = Core.Nothing,
                                            accessToken = Core.Nothing, callback = Core.Nothing,
                                            keyPairId = keyPairId, payload = payload,
                                            uploadType = Core.Nothing,
                                            uploadProtocol = Core.Nothing, userId = "me"}
instance Core.GoogleRequest GmailUsersSettingsCseKeypairsObliterate
         where
        ...
        requestClient GmailUsersSettingsCseKeypairsObliterate{..}
          = go userId keyPairId xgafv accessToken callback uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy GmailUsersSettingsCseKeypairsObliterateResource)
                      Core.mempty
