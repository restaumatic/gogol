...
--
-- Turns on a client-side encryption key pair that was turned off. The key pair becomes active again for any associated client-side encryption identities. For administrators managing identities and keypairs for users in their organization, requests require authorization with a <https://developers.google.com/identity/protocols/OAuth2ServiceAccount service account> that has <https://developers.google.com/identity/protocols/OAuth2ServiceAccount#delegatingauthority domain-wide delegation authority> to impersonate users with the @https:\/\/www.googleapis.com\/auth\/gmail.settings.basic@ scope. For users managing their own identities and keypairs, requests require <https://support.google.com/a/answer/14153163 hardware key encryption> turned on and configured.
--
-- /See:/ <https://developers.google.com/workspace/gmail/api/ Gmail API Reference> for @gmail.users.settings.cse.keypairs.enable@.
module Gogol.Gmail.Users.Settings.Cse.Keypairs.Enable
    (
    -- * Resource
         ...
         "users" Core.:>
           Core.Capture "userId" Core.Text Core.:>
             "settings" Core.:>
               "cse" Core.:>
                 "keypairs" Core.:>
                   Core.CaptureMode "keyPairId" "enable" Core.Text Core.:>
                           ...
                           Core.QueryParam "uploadType" Core.Text Core.:>
                             Core.QueryParam "upload_protocol" Core.Text Core.:>
                               Core.QueryParam "alt" Core.AltJSON Core.:>
                                 Core.ReqBody '[Core.JSON] EnableCseKeyPairRequest Core.:>
...
newGmailUsersSettingsCseKeypairsEnable keyPairId payload
  = GmailUsersSettingsCseKeypairsEnable{xgafv = Core.Nothing,
                                        accessToken = Core.Nothing, callback = Core.Nothing,
                                        keyPairId = keyPairId, payload = payload,
                                        uploadType = Core.Nothing, uploadProtocol = Core.Nothing,
                                        userId = "me"}
instance Core.GoogleRequest GmailUsersSettingsCseKeypairsEnable
         where
        type Rs GmailUsersSettingsCseKeypairsEnable = CseKeyPair
        type Scopes GmailUsersSettingsCseKeypairsEnable =
