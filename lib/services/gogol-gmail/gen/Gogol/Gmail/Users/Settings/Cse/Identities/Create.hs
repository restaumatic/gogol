...
--
-- Creates and configures a client-side encryption identity that\'s authorized to send mail from the user account. Google publishes the S\/MIME certificate to a shared domain-wide directory so that people within a Google Workspace organization can encrypt and send mail to the identity. For administrators managing identities and keypairs for users in their organization, requests require authorization with a <https://developers.google.com/identity/protocols/OAuth2ServiceAccount service account> that has <https://developers.google.com/identity/protocols/OAuth2ServiceAccount#delegatingauthority domain-wide delegation authority> to impersonate users with the @https:\/\/www.googleapis.com\/auth\/gmail.settings.basic@ scope. For users managing their own identities and keypairs, requests require <https://support.google.com/a/answer/14153163 hardware key encryption> turned on and configured.
--
-- /See:/ <https://developers.google.com/workspace/gmail/api/ Gmail API Reference> for @gmail.users.settings.cse.identities.create@.
module Gogol.Gmail.Users.Settings.Cse.Identities.Create
    (
    -- * Resource
                         ...
                         Core.QueryParam "uploadType" Core.Text Core.:>
                           Core.QueryParam "upload_protocol" Core.Text Core.:>
                             Core.QueryParam "alt" Core.AltJSON Core.:>
                               Core.ReqBody '[Core.JSON] CseIdentity Core.:>
                                 Core.Post '[Core.JSON] CseIdentity
...
newGmailUsersSettingsCseIdentitiesCreate payload
  = GmailUsersSettingsCseIdentitiesCreate{xgafv = Core.Nothing,
                                          accessToken = Core.Nothing, callback = Core.Nothing,
                                          payload = payload, uploadType = Core.Nothing,
                                          uploadProtocol = Core.Nothing, userId = "me"}
instance Core.GoogleRequest GmailUsersSettingsCseIdentitiesCreate
         where
        type Rs GmailUsersSettingsCseIdentitiesCreate = CseIdentity
        type Scopes GmailUsersSettingsCseIdentitiesCreate =
