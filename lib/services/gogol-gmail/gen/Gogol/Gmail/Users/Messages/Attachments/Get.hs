...
--
-- /See:/ <https://developers.google.com/workspace/gmail/api/ Gmail API Reference> for @gmail.users.messages.attachments.get@.
module Gogol.Gmail.Users.Messages.Attachments.Get
    (
    -- * Resource
...
newGmailUsersMessagesAttachmentsGet id messageId
  = GmailUsersMessagesAttachmentsGet{xgafv = Core.Nothing,
                                     accessToken = Core.Nothing, callback = Core.Nothing, id = id,
                                     messageId = messageId, uploadType = Core.Nothing,
                                     uploadProtocol = Core.Nothing, userId = "me"}
instance Core.GoogleRequest GmailUsersMessagesAttachmentsGet where
        type Rs GmailUsersMessagesAttachmentsGet = MessagePartBody
        type Scopes GmailUsersMessagesAttachmentsGet =
             '[Gmail'FullControl, Gmail'Addons'Current'Message'Action,
               Gmail'Addons'Current'Message'Readonly, Gmail'Modify,
               Gmail'Readonly]
        requestClient GmailUsersMessagesAttachmentsGet{..}
          = go userId messageId id xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              gmailService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy GmailUsersMessagesAttachmentsGetResource)
                      Core.mempty
