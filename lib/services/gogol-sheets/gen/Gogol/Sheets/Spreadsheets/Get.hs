...
--
-- Returns the spreadsheet at the given ID. The caller must specify the spreadsheet ID. By default, data within grids is not returned. You can include grid data in one of 2 ways: * Specify a <https://developers.google.com/workspace/sheets/api/guides/field-masks field mask> listing your desired fields using the @fields@ URL parameter in HTTP * Set the includeGridData URL parameter to true. If a field mask is set, the @includeGridData@ parameter is ignored For large spreadsheets, as a best practice, retrieve only the specific spreadsheet fields that you want. To retrieve only subsets of spreadsheet data, use the ranges URL parameter. Ranges are specified using <https://developers.google.com/workspace/sheets/api/guides/concepts#cell A1 notation>. You can define a single cell (for example, @A1@) or multiple cells (for example, @A1:D5@). You can also get cells from other sheets within the same spreadsheet (for example, @Sheet2!A1:C4@) or retrieve multiple ranges at once (for example,
...
--
-- /See:/ <https://developers.google.com/workspace/sheets/ Google Sheets API Reference> for @sheets.spreadsheets.get@.
module Gogol.Sheets.Spreadsheets.Get
    (
    -- * Resource
           ...
           Core.QueryParam "$.xgafv" Xgafv Core.:>
             Core.QueryParam "access_token" Core.Text Core.:>
               Core.QueryParam "callback" Core.Text Core.:>
                 Core.QueryParam "excludeTablesInBandedRanges" Core.Bool Core.:>
...
newSheetsSpreadsheetsGet spreadsheetId
  = SheetsSpreadsheetsGet{xgafv = Core.Nothing,
                          accessToken = Core.Nothing, callback = Core.Nothing,
                          excludeTablesInBandedRanges = Core.Nothing,
                          includeGridData = Core.Nothing, ranges = Core.Nothing,
