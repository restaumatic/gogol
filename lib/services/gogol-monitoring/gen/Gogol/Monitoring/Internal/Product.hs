...
module Gogol.Monitoring.Internal.Product
  (
    ...
    -- * LogMetadata_ExtractedLabels
    LogMetadata_ExtractedLabels (..),
    newLogMetadata_ExtractedLabels,
    ...
    -- * PolicySnapshot_UserLabels
    PolicySnapshot_UserLabels (..),
    newPolicySnapshot_UserLabels,
...
data Alert = Alert
    ...
    , metric :: (Core.Maybe Metric)
      -- | Identifier. The name of the alert.The format is: projects\/[PROJECT/ID/OR/NUMBER]\/alerts\/[ALERT/ID] The [ALERT_ID] is a system-assigned unique identifier for the alert.
    , name :: (Core.Maybe Core.Text)
      -- | The time when the alert was opened.
    , openTime :: (Core.Maybe Core.DateTime)
      -- | The snapshot of the alert policy that generated this alert.
    , policy :: (Core.Maybe PolicySnapshot)
...
newAlert 
    ::  Alert
newAlert
  = Alert{closeTime = Core.Nothing, log = Core.Nothing,
          metadata = Core.Nothing, metric = Core.Nothing,
          name = Core.Nothing, openTime = Core.Nothing,
          policy = Core.Nothing, resource = Core.Nothing,
          state = Core.Nothing}
instance Core.FromJSON Alert where
        parseJSON
          = Core.withObject "Alert"
              (\ o ->
                 Alert Core.<$>
                   (o Core..:? "closeTime") Core.<*> (o Core..:? "log") Core.<*>
                     (o Core..:? "metadata")
                     Core.<*> (o Core..:? "metric")
                     Core.<*> (o Core..:? "name")
...
--
-- /See:/ 'newLogMetadata' smart constructor.
newtype LogMetadata = LogMetadata
    {
      -- | The labels extracted from the log.
      extractedLabels :: (Core.Maybe LogMetadata_ExtractedLabels)
    }
    deriving (Core.Eq, Core.Show, Core.Generic)

-- | Creates a value of 'LogMetadata' with the minimum fields required to make a request.
newLogMetadata 
    ::  LogMetadata
newLogMetadata = LogMetadata{extractedLabels = Core.Nothing}
instance Core.FromJSON LogMetadata where
        parseJSON
          = Core.withObject "LogMetadata"
              (\ o -> LogMetadata Core.<$> (o Core..:? "extractedLabels"))

instance Core.ToJSON LogMetadata where
        toJSON LogMetadata{..}
          = Core.object
              (Core.catMaybes
                 [("extractedLabels" Core..=) Core.<$> extractedLabels])


-- | The labels extracted from the log.
--
-- /See:/ 'newLogMetadata_ExtractedLabels' smart constructor.
newtype LogMetadata_ExtractedLabels = LogMetadata_ExtractedLabels
    {
...
-- | Creates a value of 'LogMetadata_ExtractedLabels' with the minimum fields required to make a request.
newLogMetadata_ExtractedLabels 
    :: 
                               Core.HashMap Core.Text Core.Text
       -- ^  See 'additional'.
    -> LogMetadata_ExtractedLabels
newLogMetadata_ExtractedLabels additional
  = LogMetadata_ExtractedLabels{additional = additional}
instance Core.FromJSON LogMetadata_ExtractedLabels where
        parseJSON
          = Core.withObject "LogMetadata_ExtractedLabels"
              (\ o ->
                 LogMetadata_ExtractedLabels Core.<$> (Core.parseJSONObject o))

instance Core.ToJSON LogMetadata_ExtractedLabels where
        toJSON LogMetadata_ExtractedLabels{..} = Core.toJSON additional
...
--
-- /See:/ 'newPolicySnapshot' smart constructor.
data PolicySnapshot = PolicySnapshot
    {
      -- | The display name of the alert policy.
      displayName :: (Core.Maybe Core.Text)
      -- | The name of the alert policy resource. In the form of \"projects\/PROJECT/ID/OR/NUMBER\/alertPolicies\/ALERT/POLICY_ID\".
    , name :: (Core.Maybe Core.Text)
      -- | The severity of the alert policy.
    , severity :: (Core.Maybe PolicySnapshot_Severity)
      -- | The user labels for the alert policy.
    , userLabels :: (Core.Maybe PolicySnapshot_UserLabels)
    }
    deriving (Core.Eq, Core.Show, Core.Generic)

-- | Creates a value of 'PolicySnapshot' with the minimum fields required to make a request.
newPolicySnapshot 
    ::  PolicySnapshot
newPolicySnapshot
  = PolicySnapshot{displayName = Core.Nothing, name = Core.Nothing,
                   severity = Core.Nothing, userLabels = Core.Nothing}
instance Core.FromJSON PolicySnapshot where
        parseJSON
          = Core.withObject "PolicySnapshot"
              (\ o ->
                 PolicySnapshot Core.<$>
...
instance Core.ToJSON PolicySnapshot where
...
-- | The user labels for the alert policy.
--
-- /See:/ 'newPolicySnapshot_UserLabels' smart constructor.
newtype PolicySnapshot_UserLabels = PolicySnapshot_UserLabels
    {
...
-- | Creates a value of 'PolicySnapshot_UserLabels' with the minimum fields required to make a request.
newPolicySnapshot_UserLabels 
    :: 
                             Core.HashMap Core.Text Core.Text
       -- ^  See 'additional'.
    -> PolicySnapshot_UserLabels
newPolicySnapshot_UserLabels additional
  = PolicySnapshot_UserLabels{additional = additional}
instance Core.FromJSON PolicySnapshot_UserLabels where
        parseJSON
          = Core.withObject "PolicySnapshot_UserLabels"
              (\ o ->
                 PolicySnapshot_UserLabels Core.<$> (Core.parseJSONObject o))

instance Core.ToJSON PolicySnapshot_UserLabels where
        toJSON PolicySnapshot_UserLabels{..} = Core.toJSON additional
