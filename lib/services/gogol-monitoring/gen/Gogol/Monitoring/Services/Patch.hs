...
newMonitoringServicesPatch 
    :: 
                           Core.Text
       -- ^  Identifier. Resource name for this Service. The format is: projects\/[PROJECT/ID/OR/NUMBER]\/services\/[SERVICE/ID] See 'name'.
    -> Service
       -- ^  Multipart request metadata. See 'payload'.
    -> MonitoringServicesPatch
newMonitoringServicesPatch name payload
  = MonitoringServicesPatch{xgafv = Core.Nothing,
...
instance Core.GoogleRequest MonitoringServicesPatch where
        ...
        requestClient MonitoringServicesPatch{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy MonitoringServicesPatchResource)
                      Core.mempty
