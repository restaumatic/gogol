...
newBigQueryRoutinesSetIamPolicy payload resource
  = BigQueryRoutinesSetIamPolicy{xgafv = Core.Nothing,
                                 accessToken = Core.Nothing, callback = Core.Nothing,
                                 payload = payload, resource = resource, uploadType = Core.Nothing,
                                 uploadProtocol = Core.Nothing}
instance Core.GoogleRequest BigQueryRoutinesSetIamPolicy where
        ...
        requestClient BigQueryRoutinesSetIamPolicy{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy BigQueryRoutinesSetIamPolicyResource)
                      Core.mempty
