...
newBigQueryJobsGet jobId projectId
  = BigQueryJobsGet{xgafv = Core.Nothing, accessToken = Core.Nothing,
                    callback = Core.Nothing, jobId = jobId, location = Core.Nothing,
...
instance Core.GoogleRequest BigQueryJobsGet where
        ...
        type Scopes BigQueryJobsGet =
             '[Bigquery'FullControl, CloudPlatform'FullControl,
               CloudPlatform'ReadOnly]
        requestClient BigQueryJobsGet{..}
          = go projectId jobId xgafv accessToken callback location uploadType
              uploadProtocol
