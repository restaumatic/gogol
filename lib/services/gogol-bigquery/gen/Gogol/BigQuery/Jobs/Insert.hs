...
newBigQueryJobsInsert payload projectId
  = BigQueryJobsInsert{xgafv = Core.Nothing,
...
instance Core.GoogleRequest (Core.MediaUpload BigQueryJobsInsert)
         where
        ...
        requestClient (Core.MediaUpload BigQueryJobsInsert{..} body)
          = go projectId xgafv accessToken callback uploadType uploadProtocol
              (Core.Just Core.AltJSON)
