...
data DigitalAssetLinksStatementsList = DigitalAssetLinksStatementsList
    ...
    , relation :: (Core.Maybe Core.Text)
      -- | Whether to return any relation/extensions payloads specified in the source digital asset links statements. If this is set to @false@ (default), relation/extensions specified will not be returned, even if they are specified in the DAL statement file. If set to @true@, the API will propagate relation_extensions associated with each statement\'s relation type, if specified in the DAL statement file.
...
newDigitalAssetLinksStatementsList 
    :: 
                                   DigitalAssetLinksStatementsList
newDigitalAssetLinksStatementsList
  = DigitalAssetLinksStatementsList{xgafv = Core.Nothing,
                                    ...
                                    sourceWebSite = Core.Nothing, uploadType = Core.Nothing,
                                    uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DigitalAssetLinksStatementsList where
        type Rs DigitalAssetLinksStatementsList = ListResponse
        ...
        requestClient DigitalAssetLinksStatementsList{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DigitalAssetLinksStatementsListResource)
                      Core.mempty
