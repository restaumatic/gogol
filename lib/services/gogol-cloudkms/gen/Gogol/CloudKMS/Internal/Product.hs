...
module Gogol.CloudKMS.Internal.Product
  (
    ...
    -- * DecapsulateRequest
    DecapsulateRequest (..),
    newDecapsulateRequest,

    -- * DecapsulateResponse
    DecapsulateResponse (..),
    newDecapsulateResponse,
...
--
-- /See:/ 'newDecapsulateRequest' smart constructor.
data DecapsulateRequest = DecapsulateRequest
    {
...
-- | Creates a value of 'DecapsulateRequest' with the minimum fields required to make a request.
newDecapsulateRequest 
    ::  DecapsulateRequest
newDecapsulateRequest
  = DecapsulateRequest{ciphertext = Core.Nothing,
                       ciphertextCrc32c = Core.Nothing}
instance Core.FromJSON DecapsulateRequest where
        parseJSON
          = Core.withObject "DecapsulateRequest"
              (\ o ->
                 DecapsulateRequest Core.<$>
                   (o Core..:? "ciphertext") Core.<*>
                     (o Core..:? "ciphertextCrc32c" Core.<&> Core.fmap Core.fromAsText))

instance Core.ToJSON DecapsulateRequest where
        toJSON DecapsulateRequest{..}
          = Core.object
              (Core.catMaybes
                 [("ciphertext" Core..=) Core.<$> ciphertext,
                  ("ciphertextCrc32c" Core..=) Core.. Core.AsText Core.<$>
...
--
-- /See:/ 'newDecapsulateResponse' smart constructor.
data DecapsulateResponse = DecapsulateResponse
    {
    ...
    , protectionLevel :: (Core.Maybe DecapsulateResponse_ProtectionLevel)
      -- | The decapsulated shared_secret originally encapsulated with the matching public key.
    , sharedSecret :: (Core.Maybe Core.Base64)
      -- | Integrity verification field. A CRC32C checksum of the returned DecapsulateResponse.shared/secret. An integrity check of DecapsulateResponse.shared/secret can be performed by computing the CRC32C checksum of DecapsulateResponse.shared_secret and comparing your results to this field. Discard the response in case of non-matching checksum values, and perform a limited number of retries. A persistent mismatch may indicate an issue in your computation of the CRC32C checksum. Note: receiving this response message indicates that KeyManagementService is able to successfully decrypt the ciphertext. Note: This field is defined as int64 for reasons of compatibility across different languages. However, it is a non-negative integer, which will never exceed 2^32-1, and can be safely downconverted to uint32 in languages that support this type.
    , sharedSecretCrc32c :: (Core.Maybe Core.Int64)
      -- | Integrity verification field. A flag indicating whether DecapsulateRequest.ciphertext/crc32c was received by KeyManagementService and used for the integrity verification of the ciphertext. A false value of this field indicates either that DecapsulateRequest.ciphertext/crc32c was left unset or that it was not delivered to KeyManagementService. If you\'ve set DecapsulateRequest.ciphertext_crc32c but this field is still false, discard the response and perform a limited number of retries.
    , verifiedCiphertextCrc32c :: (Core.Maybe Core.Bool)
    }
    deriving (Core.Eq, Core.Show, Core.Generic)

-- | Creates a value of 'DecapsulateResponse' with the minimum fields required to make a request.
newDecapsulateResponse 
    ::  DecapsulateResponse
newDecapsulateResponse
  = DecapsulateResponse{name = Core.Nothing,
                        protectionLevel = Core.Nothing, sharedSecret = Core.Nothing,
                        sharedSecretCrc32c = Core.Nothing,
                        verifiedCiphertextCrc32c = Core.Nothing}
instance Core.FromJSON DecapsulateResponse where
        parseJSON
          = Core.withObject "DecapsulateResponse"
              (\ o ->
                 DecapsulateResponse Core.<$>
                   (o Core..:? "name") Core.<*> (o Core..:? "protectionLevel")
                     Core.<*> (o Core..:? "sharedSecret")
                     Core.<*>
                     (o Core..:? "sharedSecretCrc32c" Core.<&>
                        Core.fmap Core.fromAsText)
                     Core.<*> (o Core..:? "verifiedCiphertextCrc32c"))

instance Core.ToJSON DecapsulateResponse where
        toJSON DecapsulateResponse{..}
          = Core.object
              (Core.catMaybes
                 [("name" Core..=) Core.<$> name,
                  ("protectionLevel" Core..=) Core.<$> protectionLevel,
                  ("sharedSecret" Core..=) Core.<$> sharedSecret,
                  ("sharedSecretCrc32c" Core..=) Core.. Core.AsText Core.<$>
                    sharedSecretCrc32c,
                  ("verifiedCiphertextCrc32c" Core..=) Core.<$>
                    verifiedCiphertextCrc32c])
