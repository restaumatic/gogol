...
type CloudKMSProjectsLocationsGenerateRandomBytesResource =
     "v1" Core.:>
       Core.CaptureMode "location" "generateRandomBytes" Core.Text Core.:>
               ...
               Core.QueryParam "uploadType" Core.Text Core.:>
                 Core.QueryParam "upload_protocol" Core.Text Core.:>
                   Core.QueryParam "alt" Core.AltJSON Core.:>
                     Core.ReqBody '[Core.JSON] GenerateRandomBytesRequest Core.:>
                       Core.Post '[Core.JSON] GenerateRandomBytesResponse
...
newCloudKMSProjectsLocationsGenerateRandomBytes location payload
  = CloudKMSProjectsLocationsGenerateRandomBytes{xgafv =
                                                   Core.Nothing,
                                                 accessToken = Core.Nothing,
                                                 callback = Core.Nothing, location = location,
                                                 payload = payload, uploadType = Core.Nothing,
                                                 uploadProtocol = Core.Nothing}
instance Core.GoogleRequest
           CloudKMSProjectsLocationsGenerateRandomBytes
         where
        type Rs CloudKMSProjectsLocationsGenerateRandomBytes =
             GenerateRandomBytesResponse
        type Scopes CloudKMSProjectsLocationsGenerateRandomBytes =
             '[CloudPlatform'FullControl, Cloudkms'FullControl]
        requestClient CloudKMSProjectsLocationsGenerateRandomBytes{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy CloudKMSProjectsLocationsGenerateRandomBytesResource)
                      Core.mempty
