...
instance Core.GoogleRequest
           AnalyticsUserDeletionUserDeletionRequestUpsert
         where
        type Rs AnalyticsUserDeletionUserDeletionRequestUpsert =
             UserDeletionRequest
        type Scopes AnalyticsUserDeletionUserDeletionRequestUpsert =
             '[Analytics'User'Deletion]
        requestClient AnalyticsUserDeletionUserDeletionRequestUpsert{..}
          = go (Core.Just Core.AltJSON) payload analyticsService
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy AnalyticsUserDeletionUserDeletionRequestUpsertResource)
                      Core.mempty
