...
newAnalyticsManagementAccountUserLinksList accountId
  = AnalyticsManagementAccountUserLinksList{accountId = accountId,
                                            maxResults = Core.Nothing, startIndex = Core.Nothing}
instance Core.GoogleRequest AnalyticsManagementAccountUserLinksList
         where
        type Rs AnalyticsManagementAccountUserLinksList = EntityUserLinks
        type Scopes AnalyticsManagementAccountUserLinksList =
             '[Analytics'Manage'Users, Analytics'Manage'Users'Readonly]
        requestClient AnalyticsManagementAccountUserLinksList{..}
          = go accountId maxResults startIndex (Core.Just Core.AltJSON)
              analyticsService
          where go
                  = Core.buildClient
                      (Core.Proxy ::
