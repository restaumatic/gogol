                     ...
                     Core.QueryParam "max-results" Core.Int32 Core.:>
                       Core.QueryParam "start-index" Core.Int32 Core.:>
                         Core.QueryParam "alt" Core.AltJSON Core.:>
                           Core.Get '[Core.JSON] CustomDataSources
...
newAnalyticsManagementCustomDataSourcesList accountId webPropertyId
  = AnalyticsManagementCustomDataSourcesList{accountId = accountId,
                                             maxResults = Core.Nothing, startIndex = Core.Nothing,
                                             webPropertyId = webPropertyId}
instance Core.GoogleRequest
           AnalyticsManagementCustomDataSourcesList
         where
        type Rs AnalyticsManagementCustomDataSourcesList =
             CustomDataSources
        type Scopes AnalyticsManagementCustomDataSourcesList =
             '[Analytics'FullControl, Analytics'Edit, Analytics'Readonly]
        requestClient AnalyticsManagementCustomDataSourcesList{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy AnalyticsManagementCustomDataSourcesListResource)
                      Core.mempty
