...
newAnalyticsManagementAccountsList 
    :: 
                                   AnalyticsManagementAccountsList
newAnalyticsManagementAccountsList
  = AnalyticsManagementAccountsList{maxResults = Core.Nothing,
                                    startIndex = Core.Nothing}
instance Core.GoogleRequest AnalyticsManagementAccountsList where
        type Rs AnalyticsManagementAccountsList = Accounts
        type Scopes AnalyticsManagementAccountsList =
        ...
        requestClient AnalyticsManagementAccountsList{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy AnalyticsManagementAccountsListResource)
                      Core.mempty
