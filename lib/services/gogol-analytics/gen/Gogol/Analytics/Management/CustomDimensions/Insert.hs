...
newAnalyticsManagementCustomDimensionsInsert 
    ...
    -> AnalyticsManagementCustomDimensionsInsert
newAnalyticsManagementCustomDimensionsInsert accountId payload
  webPropertyId
  = AnalyticsManagementCustomDimensionsInsert{accountId = accountId,
                                              payload = payload, webPropertyId = webPropertyId}
instance Core.GoogleRequest
           AnalyticsManagementCustomDimensionsInsert
         where
        type Rs AnalyticsManagementCustomDimensionsInsert = CustomDimension
        type Scopes AnalyticsManagementCustomDimensionsInsert =
             '[Analytics'Edit]
        requestClient AnalyticsManagementCustomDimensionsInsert{..}
          = go accountId webPropertyId (Core.Just Core.AltJSON) payload
              analyticsService
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy AnalyticsManagementCustomDimensionsInsertResource)
                      Core.mempty
