...
newAnalyticsManagementCustomMetricsInsert 
    ...
    -> AnalyticsManagementCustomMetricsInsert
newAnalyticsManagementCustomMetricsInsert accountId payload
  webPropertyId
  = AnalyticsManagementCustomMetricsInsert{accountId = accountId,
                                           payload = payload, webPropertyId = webPropertyId}
instance Core.GoogleRequest AnalyticsManagementCustomMetricsInsert
         where
        type Rs AnalyticsManagementCustomMetricsInsert = CustomMetric
        type Scopes AnalyticsManagementCustomMetricsInsert =
             '[Analytics'Edit]
        requestClient AnalyticsManagementCustomMetricsInsert{..}
          = go accountId webPropertyId (Core.Just Core.AltJSON) payload
              analyticsService
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy AnalyticsManagementCustomMetricsInsertResource)
                      Core.mempty
