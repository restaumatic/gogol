               ...
               "webproperties" Core.:>
                 Core.QueryParam "alt" Core.AltJSON Core.:>
                   Core.ReqBody '[Core.JSON] Webproperty Core.:>
                     Core.Post '[Core.JSON] Webproperty
...
newAnalyticsManagementWebpropertiesInsert accountId payload
  = AnalyticsManagementWebpropertiesInsert{accountId = accountId,
                                           payload = payload}
instance Core.GoogleRequest AnalyticsManagementWebpropertiesInsert
         where
        type Rs AnalyticsManagementWebpropertiesInsert = Webproperty
        type Scopes AnalyticsManagementWebpropertiesInsert =
             '[Analytics'Edit]
        requestClient AnalyticsManagementWebpropertiesInsert{..}
          = go accountId (Core.Just Core.AltJSON) payload analyticsService
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy AnalyticsManagementWebpropertiesInsertResource)
                      Core.mempty
