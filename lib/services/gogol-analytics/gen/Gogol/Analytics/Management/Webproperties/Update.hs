...
newAnalyticsManagementWebpropertiesUpdate 
    ...
    -> AnalyticsManagementWebpropertiesUpdate
newAnalyticsManagementWebpropertiesUpdate accountId payload
  webPropertyId
  = AnalyticsManagementWebpropertiesUpdate{accountId = accountId,
                                           payload = payload, webPropertyId = webPropertyId}
instance Core.GoogleRequest AnalyticsManagementWebpropertiesUpdate
         where
        type Rs AnalyticsManagementWebpropertiesUpdate = Webproperty
        type Scopes AnalyticsManagementWebpropertiesUpdate =
             '[Analytics'Edit]
        requestClient AnalyticsManagementWebpropertiesUpdate{..}
          = go accountId webPropertyId (Core.Just Core.AltJSON) payload
              analyticsService
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy AnalyticsManagementWebpropertiesUpdateResource)
                      Core.mempty
