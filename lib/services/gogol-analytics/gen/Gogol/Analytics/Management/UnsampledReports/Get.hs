                   ...
                   "profiles" Core.:>
                     Core.Capture "profileId" Core.Text Core.:>
                       "unsampledReports" Core.:>
                         Core.Capture "unsampledReportId" Core.Text Core.:>
                           Core.QueryParam "alt" Core.AltJSON Core.:>
                             Core.Get '[Core.JSON] UnsampledReport
...
newAnalyticsManagementUnsampledReportsGet 
    ...
    -> AnalyticsManagementUnsampledReportsGet
newAnalyticsManagementUnsampledReportsGet accountId profileId
  unsampledReportId webPropertyId
  = AnalyticsManagementUnsampledReportsGet{accountId = accountId,
                                           profileId = profileId,
                                           unsampledReportId = unsampledReportId,
                                           webPropertyId = webPropertyId}
instance Core.GoogleRequest AnalyticsManagementUnsampledReportsGet
         where
        type Rs AnalyticsManagementUnsampledReportsGet = UnsampledReport
        type Scopes AnalyticsManagementUnsampledReportsGet =
             '[Analytics'FullControl, Analytics'Edit, Analytics'Readonly]
        requestClient AnalyticsManagementUnsampledReportsGet{..}
          = go accountId webPropertyId profileId unsampledReportId
              (Core.Just Core.AltJSON)
              analyticsService
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy AnalyticsManagementUnsampledReportsGetResource)
                      Core.mempty
