         ...
         "data" Core.:>
           "ga" Core.:>
             Core.QueryParam "ids" Core.Text Core.:>
               Core.QueryParam "start-date" Core.Text Core.:>
                 Core.QueryParam "end-date" Core.Text Core.:>
                   Core.QueryParam "metrics" Core.Text Core.:>
                     Core.QueryParam "dimensions" Core.Text Core.:>
                       Core.QueryParam "filters" Core.Text Core.:>
                         Core.QueryParam "include-empty-rows" Core.Bool Core.:>
                           Core.QueryParam "max-results" Core.Int32 Core.:>
                             Core.QueryParam "output" DataGaGetOutput Core.:>
                               Core.QueryParam "samplingLevel" DataGaGetSamplingLevel Core.:>
                                 Core.QueryParam "segment" Core.Text Core.:>
                                   Core.QueryParam "sort" Core.Text Core.:>
                                     Core.QueryParam "start-index" Core.Int32 Core.:>
                                       Core.QueryParam "alt" Core.AltJSON Core.:>
                                         Core.Get '[Core.JSON] GaData
...
newAnalyticsDataGaGet endDate ids metrics startDate
  = AnalyticsDataGaGet{dimensions = Core.Nothing, endDate = endDate,
                       filters = Core.Nothing, ids = ids, includeEmptyRows = Core.Nothing,
                       maxResults = Core.Nothing, metrics = metrics,
                       output = Core.Nothing, samplingLevel = Core.Nothing,
                       segment = Core.Nothing, sort = Core.Nothing, startDate = startDate,
                       startIndex = Core.Nothing}
instance Core.GoogleRequest AnalyticsDataGaGet where
        type Rs AnalyticsDataGaGet = GaData
        type Scopes AnalyticsDataGaGet =
             '[Analytics'FullControl, Analytics'Readonly]
        requestClient AnalyticsDataGaGet{..}
          = go (Core.Just ids) (Core.Just startDate) (Core.Just endDate)
              (Core.Just metrics)
              dimensions
              filters
              includeEmptyRows
