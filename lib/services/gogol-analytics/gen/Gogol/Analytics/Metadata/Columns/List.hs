...
instance Core.GoogleRequest AnalyticsMetadataColumnsList where
        ...
        requestClient AnalyticsMetadataColumnsList{..}
          = go reportType (Core.Just Core.AltJSON) analyticsService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy AnalyticsMetadataColumnsListResource)
                      Core.mempty
