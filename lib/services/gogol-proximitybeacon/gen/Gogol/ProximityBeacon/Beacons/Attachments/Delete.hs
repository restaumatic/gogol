...
type ProximityBeaconBeaconsAttachmentsDeleteResource =
     "v1beta1" Core.:>
       Core.Capture "attachmentName" Core.Text Core.:>
...
newProximityBeaconBeaconsAttachmentsDelete attachmentName
  = ProximityBeaconBeaconsAttachmentsDelete{xgafv = Core.Nothing,
                                            accessToken = Core.Nothing,
                                            attachmentName = attachmentName,
                                            callback = Core.Nothing, projectId = Core.Nothing,
                                            uploadType = Core.Nothing,
                                            uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ProximityBeaconBeaconsAttachmentsDelete
         where
        type Rs ProximityBeaconBeaconsAttachmentsDelete = Empty
        type Scopes ProximityBeaconBeaconsAttachmentsDelete =
             '[Userlocation'Beacon'Registry]
        requestClient ProximityBeaconBeaconsAttachmentsDelete{..}
          = go attachmentName xgafv accessToken callback projectId uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy ProximityBeaconBeaconsAttachmentsDeleteResource)
                      Core.mempty
