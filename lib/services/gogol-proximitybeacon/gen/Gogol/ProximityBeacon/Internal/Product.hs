...
newAdvertisedId 
    ::  AdvertisedId
newAdvertisedId
  = AdvertisedId{id = Core.Nothing, type' = Core.Nothing}
instance Core.FromJSON AdvertisedId where
        parseJSON
          = Core.withObject "AdvertisedId"
              (\ o ->
                 AdvertisedId Core.<$>
...
newAttachmentInfo 
    ::  AttachmentInfo
newAttachmentInfo
  = AttachmentInfo{data' = Core.Nothing,
                   maxDistanceMeters = Core.Nothing, namespacedType = Core.Nothing}
instance Core.FromJSON AttachmentInfo where
        parseJSON
          = Core.withObject "AttachmentInfo"
              (\ o ->
                 AttachmentInfo Core.<$>
                   (o Core..:? "data") Core.<*> (o Core..:? "maxDistanceMeters")
...
instance Core.ToJSON AttachmentInfo where
        toJSON AttachmentInfo{..}
          = Core.object
              (Core.catMaybes
                 [("data" Core..=) Core.<$> data',
                  ("maxDistanceMeters" Core..=) Core.<$> maxDistanceMeters,
...
newBeacon 
    ::  Beacon
newBeacon
  = Beacon{advertisedId = Core.Nothing, beaconName = Core.Nothing,
           description = Core.Nothing, ephemeralIdRegistration = Core.Nothing,
           expectedStability = Core.Nothing, indoorLevel = Core.Nothing,
           latLng = Core.Nothing, placeId = Core.Nothing,
           properties = Core.Nothing, provisioningKey = Core.Nothing,
           status = Core.Nothing}
instance Core.FromJSON Beacon where
        parseJSON
          = Core.withObject "Beacon"
              (\ o ->
                 Beacon Core.<$>
                   (o Core..:? "advertisedId") Core.<*> (o Core..:? "beaconName")
          ...
          = Core.object
              (Core.catMaybes
                 [("advertisedId" Core..=) Core.<$> advertisedId,
                  ...
                  ("ephemeralIdRegistration" Core..=) Core.<$>
                    ephemeralIdRegistration,
                  ("expectedStability" Core..=) Core.<$> expectedStability,
...
newBeacon_Properties 
    :: 
       ...
       -- ^  See 'additional'.
    -> Beacon_Properties
newBeacon_Properties additional
  = Beacon_Properties{additional = additional}
instance Core.FromJSON Beacon_Properties where
        parseJSON
          = Core.withObject "Beacon_Properties"
              (\ o -> Beacon_Properties Core.<$> (Core.parseJSONObject o))
...
newBeaconAttachment 
    ::  BeaconAttachment
newBeaconAttachment
  = BeaconAttachment{attachmentName = Core.Nothing,
                     creationTimeMs = Core.Nothing, data' = Core.Nothing,
                     maxDistanceMeters = Core.Nothing, namespacedType = Core.Nothing}
instance Core.FromJSON BeaconAttachment where
        parseJSON
          = Core.withObject "BeaconAttachment"
              (\ o ->
                 BeaconAttachment Core.<$>
                   (o Core..:? "attachmentName") Core.<*>
                     (o Core..:? "creationTimeMs")
                     Core.<*> (o Core..:? "data")
...
newBeaconInfo 
    ::  BeaconInfo
newBeaconInfo
  = BeaconInfo{advertisedId = Core.Nothing,
               attachments = Core.Nothing, beaconName = Core.Nothing}
instance Core.FromJSON BeaconInfo where
        parseJSON
          = Core.withObject "BeaconInfo"
              (\ o ->
                 BeaconInfo Core.<$>
                   (o Core..:? "advertisedId") Core.<*> (o Core..:? "attachments")
...
instance Core.FromJSON DeleteAttachmentsResponse where
        parseJSON
          = Core.withObject "DeleteAttachmentsResponse"
              (\ o ->
                 DeleteAttachmentsResponse Core.<$> (o Core..:? "numDeleted"))

instance Core.ToJSON DeleteAttachmentsResponse where
        toJSON DeleteAttachmentsResponse{..}
          = Core.object
              (Core.catMaybes [("numDeleted" Core..=) Core.<$> numDeleted])
...
newDiagnostics 
    ::  Diagnostics
newDiagnostics
  = Diagnostics{alerts = Core.Nothing, beaconName = Core.Nothing,
                estimatedLowBatteryDate = Core.Nothing}
instance Core.FromJSON Diagnostics where
        parseJSON
          = Core.withObject "Diagnostics"
              (\ o ->
                 Diagnostics Core.<$>
                   (o Core..:? "alerts") Core.<*> (o Core..:? "beaconName") Core.<*>
                     (o Core..:? "estimatedLowBatteryDate"))
          ...
          = Core.object
              (Core.catMaybes
                 [("alerts" Core..=) Core.<$> alerts,
                  ("beaconName" Core..=) Core.<$> beaconName,
                  ("estimatedLowBatteryDate" Core..=) Core.<$>
...
newEphemeralIdRegistration 
    ::  EphemeralIdRegistration
newEphemeralIdRegistration
  = EphemeralIdRegistration{beaconEcdhPublicKey = Core.Nothing,
                            beaconIdentityKey = Core.Nothing, initialClockValue = Core.Nothing,
                            initialEid = Core.Nothing, rotationPeriodExponent = Core.Nothing,
                            serviceEcdhPublicKey = Core.Nothing}
instance Core.FromJSON EphemeralIdRegistration where
        parseJSON
          = Core.withObject "EphemeralIdRegistration"
              (\ o ->
                 EphemeralIdRegistration Core.<$>
                   (o Core..:? "beaconEcdhPublicKey") Core.<*>
                     (o Core..:? "beaconIdentityKey")
                     Core.<*>
                     (o Core..:? "initialClockValue" Core.<&> Core.fmap Core.fromAsText)
...
instance Core.ToJSON EphemeralIdRegistration where
        toJSON EphemeralIdRegistration{..}
          = Core.object
              (Core.catMaybes
                 [("beaconEcdhPublicKey" Core..=) Core.<$> beaconEcdhPublicKey,
                  ("beaconIdentityKey" Core..=) Core.<$> beaconIdentityKey,
                  ("initialClockValue" Core..=) Core.. Core.AsText Core.<$>
                    initialClockValue,
                  ("initialEid" Core..=) Core.<$> initialEid,
                  ("rotationPeriodExponent" Core..=) Core.<$> rotationPeriodExponent,
                  ("serviceEcdhPublicKey" Core..=) Core.<$> serviceEcdhPublicKey])
...
newEphemeralIdRegistrationParams 
    ::  EphemeralIdRegistrationParams
newEphemeralIdRegistrationParams
  = EphemeralIdRegistrationParams{maxRotationPeriodExponent =
                                    Core.Nothing,
                                  minRotationPeriodExponent = Core.Nothing,
                                  serviceEcdhPublicKey = Core.Nothing}
instance Core.FromJSON EphemeralIdRegistrationParams where
        parseJSON
          = Core.withObject "EphemeralIdRegistrationParams"
              (\ o ->
                 EphemeralIdRegistrationParams Core.<$>
                   (o Core..:? "maxRotationPeriodExponent") Core.<*>
                     (o Core..:? "minRotationPeriodExponent")
                     Core.<*> (o Core..:? "serviceEcdhPublicKey"))
...
instance Core.ToJSON EphemeralIdRegistrationParams where
        toJSON EphemeralIdRegistrationParams{..}
          = Core.object
              (Core.catMaybes
                 [("maxRotationPeriodExponent" Core..=) Core.<$>
                    maxRotationPeriodExponent,
                  ("minRotationPeriodExponent" Core..=) Core.<$>
...
newGetInfoForObservedBeaconsRequest 
    :: 
                                    GetInfoForObservedBeaconsRequest
newGetInfoForObservedBeaconsRequest
  = GetInfoForObservedBeaconsRequest{namespacedTypes = Core.Nothing,
                                     observations = Core.Nothing}
instance Core.FromJSON GetInfoForObservedBeaconsRequest where
        parseJSON
          = Core.withObject "GetInfoForObservedBeaconsRequest"
              (\ o ->
                 GetInfoForObservedBeaconsRequest Core.<$>
                   (o Core..:? "namespacedTypes") Core.<*>
                     (o Core..:? "observations"))
...
instance Core.ToJSON GetInfoForObservedBeaconsRequest where
        toJSON GetInfoForObservedBeaconsRequest{..}
          = Core.object
              (Core.catMaybes
                 [("namespacedTypes" Core..=) Core.<$> namespacedTypes,
...
instance Core.FromJSON GetInfoForObservedBeaconsResponse where
        parseJSON
          = Core.withObject "GetInfoForObservedBeaconsResponse"
              (\ o ->
                 GetInfoForObservedBeaconsResponse Core.<$> (o Core..:? "beacons"))

instance Core.ToJSON GetInfoForObservedBeaconsResponse where
        toJSON GetInfoForObservedBeaconsResponse{..}
          = Core.object
              (Core.catMaybes [("beacons" Core..=) Core.<$> beacons])
...
instance Core.FromJSON ListBeaconAttachmentsResponse where
        parseJSON
          = Core.withObject "ListBeaconAttachmentsResponse"
              (\ o ->
                 ListBeaconAttachmentsResponse Core.<$> (o Core..:? "attachments"))
...
newListBeaconsResponse 
    ::  ListBeaconsResponse
newListBeaconsResponse
  = ListBeaconsResponse{beacons = Core.Nothing,
                        nextPageToken = Core.Nothing, totalCount = Core.Nothing}
instance Core.FromJSON ListBeaconsResponse where
        parseJSON
          = Core.withObject "ListBeaconsResponse"
              (\ o ->
                 ListBeaconsResponse Core.<$>
                   (o Core..:? "beacons") Core.<*> (o Core..:? "nextPageToken")
                     Core.<*>
...
newListDiagnosticsResponse 
    ::  ListDiagnosticsResponse
newListDiagnosticsResponse
  = ListDiagnosticsResponse{diagnostics = Core.Nothing,
                            nextPageToken = Core.Nothing}
instance Core.FromJSON ListDiagnosticsResponse where
        parseJSON
          = Core.withObject "ListDiagnosticsResponse"
              (\ o ->
                 ListDiagnosticsResponse Core.<$>
                   (o Core..:? "diagnostics") Core.<*> (o Core..:? "nextPageToken"))
...
instance Core.FromJSON ListNamespacesResponse where
        parseJSON
          = Core.withObject "ListNamespacesResponse"
              (\ o -> ListNamespacesResponse Core.<$> (o Core..:? "namespaces"))

instance Core.ToJSON ListNamespacesResponse where
        toJSON ListNamespacesResponse{..}
          = Core.object
              (Core.catMaybes [("namespaces" Core..=) Core.<$> namespaces])
...
newNamespace 
    ::  Namespace
newNamespace
  = Namespace{namespaceName = Core.Nothing,
              servingVisibility = Core.Nothing}
instance Core.FromJSON Namespace where
        parseJSON
          = Core.withObject "Namespace"
              (\ o ->
                 Namespace Core.<$>
                   (o Core..:? "namespaceName") Core.<*>
                     (o Core..:? "servingVisibility"))
          ...
          = Core.object
              (Core.catMaybes
                 [("namespaceName" Core..=) Core.<$> namespaceName,
                  ("servingVisibility" Core..=) Core.<$> servingVisibility])
...
newObservation 
    ::  Observation
newObservation
  = Observation{advertisedId = Core.Nothing,
                telemetry = Core.Nothing, timestampMs = Core.Nothing}
instance Core.FromJSON Observation where
        parseJSON
          = Core.withObject "Observation"
              (\ o ->
                 Observation Core.<$>
                   (o Core..:? "advertisedId") Core.<*> (o Core..:? "telemetry")
