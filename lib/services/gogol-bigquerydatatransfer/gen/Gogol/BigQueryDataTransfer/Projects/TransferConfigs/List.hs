...
newBigQueryDataTransferProjectsTransferConfigsList 
    :: 
                                                   Core.Text
       -- ^  Required. The BigQuery project id for which transfer configs should be returned. If you are using the regionless method, the location must be @US@ and @parent@ should be in the following form: * @projects\/{project_id} If you are using the regionalized method,@parent@should be in the following form: *@projects\/{project/id}\/locations\/{location/id}\` See 'parent'.
    -> BigQueryDataTransferProjectsTransferConfigsList
newBigQueryDataTransferProjectsTransferConfigsList parent
  = BigQueryDataTransferProjectsTransferConfigsList{xgafv =
                                                      Core.Nothing,
...
instance Core.GoogleRequest
           BigQueryDataTransferProjectsTransferConfigsList
         where
        type Rs BigQueryDataTransferProjectsTransferConfigsList =
             ListTransferConfigsResponse
        type Scopes BigQueryDataTransferProjectsTransferConfigsList =
             '[Bigquery'FullControl, CloudPlatform'FullControl,
               CloudPlatform'ReadOnly]
        requestClient BigQueryDataTransferProjectsTransferConfigsList{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy BigQueryDataTransferProjectsTransferConfigsListResource)
                      Core.mempty
