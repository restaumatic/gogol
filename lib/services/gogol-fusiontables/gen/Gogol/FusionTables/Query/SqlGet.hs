             ...
             Core.QueryParam "hdrs" Core.Bool Core.:>
               Core.QueryParam "typed" Core.Bool Core.:>
                 Core.QueryParam "alt" Core.AltJSON Core.:>
                   Core.Get '[Core.JSON] Sqlresponse
       Core.:<|>
       "fusiontables" Core.:>
         "v2" Core.:>
           "query" Core.:>
...
instance Core.GoogleRequest
        ...
        type Scopes (Core.MediaDownload FusionTablesQuerySqlGet) =
             Core.Scopes FusionTablesQuerySqlGet
        requestClient (Core.MediaDownload FusionTablesQuerySqlGet{..})
          = go (Core.Just sql) hdrs typed (Core.Just Core.AltMedia)
              fusionTablesService
          where _ Core.:<|> go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy FusionTablesQuerySqlGetResource)
