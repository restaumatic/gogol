       ...
       "v2" Core.:>
         "tables" Core.:>
           Core.Capture "tableId" Core.Text Core.:>
             Core.QueryParam "replaceViewDefinition" Core.Bool Core.:>
               Core.QueryParam "alt" Core.AltJSON Core.:>
                 Core.ReqBody '[Core.JSON] Table Core.:> Core.Put '[Core.JSON] Table
...
newFusionTablesTableUpdate payload tableId
  = FusionTablesTableUpdate{payload = payload,
                            replaceViewDefinition = Core.Nothing, tableId = tableId}
instance Core.GoogleRequest FusionTablesTableUpdate where
        ...
        requestClient FusionTablesTableUpdate{..}
          = go tableId replaceViewDefinition (Core.Just Core.AltJSON) payload
              fusionTablesService
          where go
                  = Core.buildClient
