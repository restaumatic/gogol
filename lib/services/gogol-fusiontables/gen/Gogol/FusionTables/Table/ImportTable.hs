                     ...
                     Core.QueryParam "encoding" Core.Text Core.:>
                       Core.QueryParam "alt" Core.AltJSON Core.:>
                         Core.QueryParam "uploadType" Core.AltMedia Core.:>
                           Core.AltMedia Core.:> Core.Post '[Core.JSON] Table
...
newFusionTablesTableImportTable name
  = FusionTablesTableImportTable{delimiter = Core.Nothing,
                                 encoding = Core.Nothing, name = name}
instance Core.GoogleRequest FusionTablesTableImportTable where
        type Rs FusionTablesTableImportTable = Table
        type Scopes FusionTablesTableImportTable =
             '[Fusiontables'FullControl]
        requestClient FusionTablesTableImportTable{..}
          = go (Core.Just name) delimiter encoding (Core.Just Core.AltJSON)
              fusionTablesService
          where go Core.:<|> _
                  = Core.buildClient
...
instance Core.GoogleRequest
        ...
        type Scopes (Core.MediaUpload FusionTablesTableImportTable) =
