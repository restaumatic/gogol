       ...
       "v2" Core.:>
         "reports" Core.:>
           Core.Capture "reportId" Core.Text Core.:>
             "files" Core.:>
               Core.Capture "reportFragment" Core.Int32 Core.:>
...
newDoubleClickSearchReportsGetFile reportFragment reportId
  = DoubleClickSearchReportsGetFile{xgafv = Core.Nothing,
                                    accessToken = Core.Nothing, callback = Core.Nothing,
                                    reportFragment = reportFragment, reportId = reportId,
                                    uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DoubleClickSearchReportsGetFile where
        type Rs DoubleClickSearchReportsGetFile = ()
        type Scopes DoubleClickSearchReportsGetFile =
             '[Doubleclicksearch'FullControl]
        requestClient DoubleClickSearchReportsGetFile{..}
          = go reportId reportFragment xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              doubleClickSearchService
          where go Core.:<|> _
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DoubleClickSearchReportsGetFileResource)
                      Core.mempty

instance Core.GoogleRequest
           (Core.MediaDownload DoubleClickSearchReportsGetFile)
         where
        type Rs (Core.MediaDownload DoubleClickSearchReportsGetFile) =
             Core.Stream
        type Scopes (Core.MediaDownload DoubleClickSearchReportsGetFile) =
