...
--
-- /See:/ <https://developers.google.com/search/apis/indexing-api/ Web Search Indexing API Reference> for @indexing.urlNotifications.getMetadata@.
module Gogol.Indexing.UrlNotifications.GetMetadata
    (
    -- * Resource
...
newIndexingUrlNotificationsGetMetadata 
    :: 
                                       IndexingUrlNotificationsGetMetadata
newIndexingUrlNotificationsGetMetadata
  = IndexingUrlNotificationsGetMetadata{xgafv = Core.Nothing,
                                        accessToken = Core.Nothing, callback = Core.Nothing,
                                        uploadType = Core.Nothing, uploadProtocol = Core.Nothing,
                                        url = Core.Nothing}
instance Core.GoogleRequest IndexingUrlNotificationsGetMetadata
         where
        ...
        requestClient IndexingUrlNotificationsGetMetadata{..}
          = go xgafv accessToken callback uploadType uploadProtocol url
              (Core.Just Core.AltJSON)
              indexingService
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy IndexingUrlNotificationsGetMetadataResource)
                      Core.mempty
