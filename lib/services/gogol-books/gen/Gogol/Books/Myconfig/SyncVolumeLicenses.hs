                   ...
                   Core.QueryParam "$.xgafv" Xgafv Core.:>
                     Core.QueryParam "access_token" Core.Text Core.:>
                       Core.QueryParam "callback" Core.Text Core.:>
                         Core.QueryParams "features" MyconfigSyncVolumeLicensesFeatures
                           Core.:>
                           Core.QueryParam "includeNonComicsSeries" Core.Bool Core.:>
                             Core.QueryParam "locale" Core.Text Core.:>
                               Core.QueryParam "showPreorders" Core.Bool Core.:>
                                 Core.QueryParam "uploadType" Core.Text Core.:>
                                   Core.QueryParam "upload_protocol" Core.Text Core.:>
                                     Core.QueryParams "volumeIds" Core.Text Core.:>
                                       Core.QueryParam "alt" Core.AltJSON Core.:>
                                         Core.Post '[Core.JSON] Volumes
...
newBooksMyconfigSyncVolumeLicenses cpksver nonce source
  = BooksMyconfigSyncVolumeLicenses{xgafv = Core.Nothing,
                                    accessToken = Core.Nothing, callback = Core.Nothing,
                                    cpksver = cpksver, features = Core.Nothing,
                                    includeNonComicsSeries = Core.Nothing, locale = Core.Nothing,
                                    nonce = nonce, showPreorders = Core.Nothing, source = source,
                                    uploadType = Core.Nothing, uploadProtocol = Core.Nothing,
                                    volumeIds = Core.Nothing}
instance Core.GoogleRequest BooksMyconfigSyncVolumeLicenses where
        type Rs BooksMyconfigSyncVolumeLicenses = Volumes
        type Scopes BooksMyconfigSyncVolumeLicenses = '[Books'FullControl]
        requestClient BooksMyconfigSyncVolumeLicenses{..}
          = go (Core.Just cpksver) (Core.Just nonce) (Core.Just source) xgafv
              accessToken
              callback
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy BooksMyconfigSyncVolumeLicensesResource)
                      Core.mempty
