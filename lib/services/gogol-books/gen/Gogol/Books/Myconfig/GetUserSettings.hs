                     ...
                     Core.QueryParam "uploadType" Core.Text Core.:>
                       Core.QueryParam "upload_protocol" Core.Text Core.:>
                         Core.QueryParam "alt" Core.AltJSON Core.:>
                           Core.Get '[Core.JSON] Usersettings
...
newBooksMyconfigGetUserSettings 
    ::  BooksMyconfigGetUserSettings
newBooksMyconfigGetUserSettings
  = BooksMyconfigGetUserSettings{xgafv = Core.Nothing,
                                 accessToken = Core.Nothing, callback = Core.Nothing,
                                 country = Core.Nothing, uploadType = Core.Nothing,
                                 uploadProtocol = Core.Nothing}
instance Core.GoogleRequest BooksMyconfigGetUserSettings where
        type Rs BooksMyconfigGetUserSettings = Usersettings
        type Scopes BooksMyconfigGetUserSettings = '[Books'FullControl]
        requestClient BooksMyconfigGetUserSettings{..}
          = go xgafv accessToken callback country uploadType uploadProtocol
              (Core.Just Core.AltJSON)
              booksService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy BooksMyconfigGetUserSettingsResource)
                      Core.mempty
