                     ...
                     Core.QueryParam "$.xgafv" Xgafv Core.:>
                       Core.QueryParam "access_token" Core.Text Core.:>
                         Core.QueryParam "callback" Core.Text Core.:>
                           Core.QueryParam "licenseTypes" MyconfigRequestAccessLicenseTypes
                             Core.:>
...
newBooksMyconfigRequestAccess cpksver nonce source volumeId
  = BooksMyconfigRequestAccess{xgafv = Core.Nothing,
                               accessToken = Core.Nothing, callback = Core.Nothing,
                               cpksver = cpksver, licenseTypes = Core.Nothing,
                               locale = Core.Nothing, nonce = nonce, source = source,
                               uploadType = Core.Nothing, uploadProtocol = Core.Nothing,
                               volumeId = volumeId}
instance Core.GoogleRequest BooksMyconfigRequestAccess where
        type Rs BooksMyconfigRequestAccess = RequestAccessData
        type Scopes BooksMyconfigRequestAccess = '[Books'FullControl]
        requestClient BooksMyconfigRequestAccess{..}
          = go (Core.Just cpksver) (Core.Just nonce) (Core.Just source)
              (Core.Just volumeId)
              xgafv
              accessToken
              callback
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy BooksMyconfigRequestAccessResource)
                      Core.mempty
