     ...
     "books" Core.:>
       "v1" Core.:>
         "volumes" Core.:>
           "recommended" Core.:>
             "rate" Core.:>
               Core.QueryParam "rating" VolumesRecommendedRateRating Core.:>
                             ...
                             Core.QueryParam "uploadType" Core.Text Core.:>
                               Core.QueryParam "upload_protocol" Core.Text Core.:>
                                 Core.QueryParam "alt" Core.AltJSON Core.:>
                                   Core.Post '[Core.JSON] BooksVolumesRecommendedRateResponse
...
newBooksVolumesRecommendedRate rating volumeId
  = BooksVolumesRecommendedRate{xgafv = Core.Nothing,
                                accessToken = Core.Nothing, callback = Core.Nothing,
                                locale = Core.Nothing, rating = rating, source = Core.Nothing,
                                uploadType = Core.Nothing, uploadProtocol = Core.Nothing,
                                volumeId = volumeId}
instance Core.GoogleRequest BooksVolumesRecommendedRate where
        type Rs BooksVolumesRecommendedRate =
             BooksVolumesRecommendedRateResponse
        type Scopes BooksVolumesRecommendedRate = '[Books'FullControl]
        requestClient BooksVolumesRecommendedRate{..}
          = go (Core.Just rating) (Core.Just volumeId) xgafv accessToken
              callback
              locale
              source
              uploadType
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy BooksVolumesRecommendedRateResource)
                      Core.mempty
