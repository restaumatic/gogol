...
newBooksPromoofferDismiss 
    ::  BooksPromoofferDismiss
newBooksPromoofferDismiss
  = BooksPromoofferDismiss{xgafv = Core.Nothing,
                           accessToken = Core.Nothing, androidId = Core.Nothing,
                           callback = Core.Nothing, device = Core.Nothing,
                           manufacturer = Core.Nothing, model = Core.Nothing,
                           offerId = Core.Nothing, product = Core.Nothing,
                           serial = Core.Nothing, uploadType = Core.Nothing,
                           uploadProtocol = Core.Nothing}
instance Core.GoogleRequest BooksPromoofferDismiss where
        type Rs BooksPromoofferDismiss = Empty
        type Scopes BooksPromoofferDismiss = '[Books'FullControl]
        requestClient BooksPromoofferDismiss{..}
          = go xgafv accessToken androidId callback device manufacturer model
              offerId
              product
              serial
              uploadType
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy BooksPromoofferDismissResource)
                      Core.mempty
