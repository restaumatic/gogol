...
newBooksMylibraryBookshelvesVolumesList shelf
  = BooksMylibraryBookshelvesVolumesList{xgafv = Core.Nothing,
                                         accessToken = Core.Nothing, callback = Core.Nothing,
                                         country = Core.Nothing, maxResults = Core.Nothing,
                                         projection = Core.Nothing, q = Core.Nothing, shelf = shelf,
                                         showPreorders = Core.Nothing, source = Core.Nothing,
                                         startIndex = Core.Nothing, uploadType = Core.Nothing,
                                         uploadProtocol = Core.Nothing}
instance Core.GoogleRequest BooksMylibraryBookshelvesVolumesList
         where
        type Rs BooksMylibraryBookshelvesVolumesList = Volumes
        type Scopes BooksMylibraryBookshelvesVolumesList =
             '[Books'FullControl]
        requestClient BooksMylibraryBookshelvesVolumesList{..}
          = go shelf xgafv accessToken callback country maxResults projection
              q
              showPreorders
              source
              startIndex
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy BooksMylibraryBookshelvesVolumesListResource)
                      Core.mempty
