             ...
             Core.Capture "shelf" Core.Text Core.:>
               "moveVolume" Core.:>
                 Core.QueryParam "volumeId" Core.Text Core.:>
                   Core.QueryParam "volumePosition" Core.Int32 Core.:>
...
newBooksMylibraryBookshelvesMoveVolume 
    ...
    -> BooksMylibraryBookshelvesMoveVolume
newBooksMylibraryBookshelvesMoveVolume shelf volumeId
  volumePosition
  = BooksMylibraryBookshelvesMoveVolume{xgafv = Core.Nothing,
                                        accessToken = Core.Nothing, callback = Core.Nothing,
                                        shelf = shelf, source = Core.Nothing,
                                        uploadType = Core.Nothing, uploadProtocol = Core.Nothing,
                                        volumeId = volumeId, volumePosition = volumePosition}
instance Core.GoogleRequest BooksMylibraryBookshelvesMoveVolume
         where
        type Rs BooksMylibraryBookshelvesMoveVolume = Empty
        type Scopes BooksMylibraryBookshelvesMoveVolume =
             '[Books'FullControl]
        requestClient BooksMylibraryBookshelvesMoveVolume{..}
          = go shelf (Core.Just volumeId) (Core.Just volumePosition) xgafv
              accessToken
              callback
              source
              uploadType
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy BooksMylibraryBookshelvesMoveVolumeResource)
                      Core.mempty
