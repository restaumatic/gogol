                   ...
                   Core.QueryParam "timestamp" Core.Text Core.:>
                     Core.QueryParam "$.xgafv" Xgafv Core.:>
                       Core.QueryParam "access_token" Core.Text Core.:>
                         Core.QueryParam "action" MylibraryReadingpositionsSetPositionAction
                           Core.:>
...
newBooksMylibraryReadingpositionsSetPosition 
    ...
    -> BooksMylibraryReadingpositionsSetPosition
newBooksMylibraryReadingpositionsSetPosition position timestamp
  volumeId
  = BooksMylibraryReadingpositionsSetPosition{xgafv = Core.Nothing,
                                              accessToken = Core.Nothing, action = Core.Nothing,
                                              callback = Core.Nothing,
                                              contentVersion = Core.Nothing,
                                              deviceCookie = Core.Nothing, position = position,
                                              source = Core.Nothing, timestamp = timestamp,
                                              uploadType = Core.Nothing,
                                              uploadProtocol = Core.Nothing, volumeId = volumeId}
instance Core.GoogleRequest
           BooksMylibraryReadingpositionsSetPosition
         where
        type Rs BooksMylibraryReadingpositionsSetPosition = Empty
        type Scopes BooksMylibraryReadingpositionsSetPosition =
             '[Books'FullControl]
        requestClient BooksMylibraryReadingpositionsSetPosition{..}
          = go volumeId (Core.Just position) (Core.Just timestamp) xgafv
              accessToken
              action
              callback
              contentVersion
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy BooksMylibraryReadingpositionsSetPositionResource)
                      Core.mempty
