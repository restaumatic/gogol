...
newBooksMylibraryAnnotationsSummary layerIds volumeId
  = BooksMylibraryAnnotationsSummary{xgafv = Core.Nothing,
                                     accessToken = Core.Nothing, callback = Core.Nothing,
                                     layerIds = layerIds, source = Core.Nothing,
                                     uploadType = Core.Nothing, uploadProtocol = Core.Nothing,
                                     volumeId = volumeId}
instance Core.GoogleRequest BooksMylibraryAnnotationsSummary where
        type Rs BooksMylibraryAnnotationsSummary = AnnotationsSummary
        type Scopes BooksMylibraryAnnotationsSummary = '[Books'FullControl]
        requestClient BooksMylibraryAnnotationsSummary{..}
          = go layerIds (Core.Just volumeId) xgafv accessToken callback
              source
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy BooksMylibraryAnnotationsSummaryResource)
                      Core.mempty
