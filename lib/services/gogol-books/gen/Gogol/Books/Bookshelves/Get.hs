...
newBooksBookshelvesGet shelf userId
  = BooksBookshelvesGet{xgafv = Core.Nothing,
                        accessToken = Core.Nothing, callback = Core.Nothing, shelf = shelf,
...
instance Core.GoogleRequest BooksBookshelvesGet where
        ...
        type Scopes BooksBookshelvesGet = '[Books'FullControl]
        requestClient BooksBookshelvesGet{..}
          = go userId shelf xgafv accessToken callback source uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              booksService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy BooksBookshelvesGetResource)
                      Core.mempty
