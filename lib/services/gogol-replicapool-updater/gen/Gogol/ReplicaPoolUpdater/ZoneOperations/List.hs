...
newReplicaPoolUpdaterZoneOperationsList project zone
  = ReplicaPoolUpdaterZoneOperationsList{filter = Core.Nothing,
                                         maxResults = 500, pageToken = Core.Nothing,
                                         project = project, zone = zone}
instance Core.GoogleRequest ReplicaPoolUpdaterZoneOperationsList
         where
        type Rs ReplicaPoolUpdaterZoneOperationsList = OperationList
        type Scopes ReplicaPoolUpdaterZoneOperationsList =
             '[CloudPlatform'FullControl, Replicapool'FullControl]
        requestClient ReplicaPoolUpdaterZoneOperationsList{..}
          = go project zone filter (Core.Just maxResults) pageToken
              (Core.Just Core.AltJSON)
              replicaPoolUpdaterService
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy ReplicaPoolUpdaterZoneOperationsListResource)
                      Core.mempty
