...
newReplicaPoolUpdaterRollingUpdatesResume 
    ...
    -> ReplicaPoolUpdaterRollingUpdatesResume
newReplicaPoolUpdaterRollingUpdatesResume project rollingUpdate
  zone
  = ReplicaPoolUpdaterRollingUpdatesResume{project = project,
                                           rollingUpdate = rollingUpdate, zone = zone}
instance Core.GoogleRequest ReplicaPoolUpdaterRollingUpdatesResume
         where
        type Rs ReplicaPoolUpdaterRollingUpdatesResume = Operation
        type Scopes ReplicaPoolUpdaterRollingUpdatesResume =
             '[CloudPlatform'FullControl, Replicapool'FullControl]
        requestClient ReplicaPoolUpdaterRollingUpdatesResume{..}
          = go project zone rollingUpdate (Core.Just Core.AltJSON)
              replicaPoolUpdaterService
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy ReplicaPoolUpdaterRollingUpdatesResumeResource)
                      Core.mempty
