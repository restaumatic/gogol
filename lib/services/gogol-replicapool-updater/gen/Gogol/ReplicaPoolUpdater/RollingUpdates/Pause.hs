...
-- |
-- Module      : Gogol.ReplicaPoolUpdater.RollingUpdates.Pause
...
newReplicaPoolUpdaterRollingUpdatesPause project rollingUpdate zone
  = ReplicaPoolUpdaterRollingUpdatesPause{project = project,
                                          rollingUpdate = rollingUpdate, zone = zone}
instance Core.GoogleRequest ReplicaPoolUpdaterRollingUpdatesPause
         where
        type Rs ReplicaPoolUpdaterRollingUpdatesPause = Operation
        type Scopes ReplicaPoolUpdaterRollingUpdatesPause =
             '[CloudPlatform'FullControl, Replicapool'FullControl]
        requestClient ReplicaPoolUpdaterRollingUpdatesPause{..}
          = go project zone rollingUpdate (Core.Just Core.AltJSON)
              replicaPoolUpdaterService
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy ReplicaPoolUpdaterRollingUpdatesPauseResource)
                      Core.mempty
