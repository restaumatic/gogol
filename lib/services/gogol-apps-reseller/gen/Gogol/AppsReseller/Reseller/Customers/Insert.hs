...
--
-- Orders a new customer\'s account. Before ordering a new customer account, establish whether the customer account already exists using the <https://developers.google.com/workspace/admin/reseller/v1/reference/customers/get customers.get> If the customer account exists as a direct Google account or as a resold customer account from another reseller, use the @customerAuthToken\\@ as described in <https://developers.google.com/workspace/admin/reseller/v1/how-tos/manage_customers#create_existing_customer order a resold account for an existing customer>. For more information about ordering a new customer account, see <https://developers.google.com/workspace/admin/reseller/v1/how-tos/manage_customers#create_customer order a new customer account>. After creating a new customer account, you must provision a user as an administrator. The customer\'s administrator is required to sign in to the Admin console and sign the G Suite via Reseller agreement to activate the account. Resellers are prohibited from signing the G
-- Suite via Reseller agreement on the customer\'s behalf. For more information, see <https://developers.google.com/workspace/admin/reseller/v1/how-tos/manage_customers#tos order a new customer account>.
--
...
newResellerCustomersInsert payload
  = ResellerCustomersInsert{xgafv = Core.Nothing,
                            accessToken = Core.Nothing, callback = Core.Nothing,
                            customerAuthToken = Core.Nothing, payload = payload,
...
instance Core.GoogleRequest ResellerCustomersInsert where
        ...
        requestClient ResellerCustomersInsert{..}
          = go xgafv accessToken callback customerAuthToken uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ResellerCustomersInsertResource)
                      Core.mempty
