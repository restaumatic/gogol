...
-- |
-- Module      : Gogol.AppsReseller.Reseller.Customers.Update
...
--
-- Updates a customer account\'s settings. You cannot update @customerType@ via the Reseller API, but a @\"team\"@ customer can verify their domain and become @customerType = \"domain\"@. For more information, see <https://developers.google.com/workspace/admin/reseller/v1/how-tos/manage_customers#update_customer update a customer\'s settings>.
--
...
newResellerCustomersUpdate customerId payload
  = ResellerCustomersUpdate{xgafv = Core.Nothing,
...
instance Core.GoogleRequest ResellerCustomersUpdate where
        ...
        requestClient ResellerCustomersUpdate{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ResellerCustomersUpdateResource)
                      Core.mempty
