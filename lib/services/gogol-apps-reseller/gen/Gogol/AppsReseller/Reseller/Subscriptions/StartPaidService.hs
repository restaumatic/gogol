...
--
-- Immediately move a 30-day free trial subscription to a paid service subscription. This method is only applicable if a payment plan has already been set up for the 30-day trial subscription. For more information, see <https://developers.google.com/workspace/admin/reseller/v1/how-tos/manage_subscriptions#paid_service manage subscriptions>.
--
...
newResellerSubscriptionsStartPaidService customerId subscriptionId
  = ResellerSubscriptionsStartPaidService{xgafv = Core.Nothing,
                                          accessToken = Core.Nothing, callback = Core.Nothing,
                                          customerId = customerId, subscriptionId = subscriptionId,
                                          uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ResellerSubscriptionsStartPaidService
         where
        type Rs ResellerSubscriptionsStartPaidService = Subscription
        type Scopes ResellerSubscriptionsStartPaidService = '[Apps'Order]
        requestClient ResellerSubscriptionsStartPaidService{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy ResellerSubscriptionsStartPaidServiceResource)
                      Core.mempty
