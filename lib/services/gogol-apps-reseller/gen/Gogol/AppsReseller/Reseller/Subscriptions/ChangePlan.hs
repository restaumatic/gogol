...
--
-- Updates a subscription plan. Use this method to update a plan for a 30-day trial or a flexible plan subscription to an annual commitment plan with monthly or yearly payments. How a plan is updated differs depending on the plan and the products. For more information, see the description in <https://developers.google.com/workspace/admin/reseller/v1/how-tos/manage_subscriptions#update_subscription_plan manage subscriptions>.
--
                           ...
                           Core.QueryParam "uploadType" Core.Text Core.:>
                             Core.QueryParam "upload_protocol" Core.Text Core.:>
                               Core.QueryParam "alt" Core.AltJSON Core.:>
                                 Core.ReqBody '[Core.JSON] ChangePlanRequest Core.:>
...
newResellerSubscriptionsChangePlan 
    ...
    -> ResellerSubscriptionsChangePlan
newResellerSubscriptionsChangePlan customerId payload
  subscriptionId
  = ResellerSubscriptionsChangePlan{xgafv = Core.Nothing,
                                    accessToken = Core.Nothing, callback = Core.Nothing,
                                    customerId = customerId, payload = payload,
                                    subscriptionId = subscriptionId, uploadType = Core.Nothing,
                                    uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ResellerSubscriptionsChangePlan where
        type Rs ResellerSubscriptionsChangePlan = Subscription
        type Scopes ResellerSubscriptionsChangePlan = '[Apps'Order]
        requestClient ResellerSubscriptionsChangePlan{..}
          = go customerId subscriptionId xgafv accessToken callback
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ResellerSubscriptionsChangePlanResource)
                      Core.mempty
