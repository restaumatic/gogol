...
newContentAccountstatusesGet accountId merchantId
  = ContentAccountstatusesGet{xgafv = Core.Nothing,
                              accessToken = Core.Nothing, accountId = accountId,
                              callback = Core.Nothing, destinations = Core.Nothing,
                              merchantId = merchantId, uploadType = Core.Nothing,
                              uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ContentAccountstatusesGet where
        type Rs ContentAccountstatusesGet = AccountStatus
        type Scopes ContentAccountstatusesGet = '[Content'FullControl]
        requestClient ContentAccountstatusesGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ContentAccountstatusesGetResource)
                      Core.mempty
