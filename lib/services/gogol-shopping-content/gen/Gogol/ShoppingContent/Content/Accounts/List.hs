...
newContentAccountsList merchantId
  = ContentAccountsList{xgafv = Core.Nothing,
                        accessToken = Core.Nothing, callback = Core.Nothing,
                        label = Core.Nothing, maxResults = Core.Nothing,
                        merchantId = merchantId, name = Core.Nothing,
...
instance Core.GoogleRequest ContentAccountsList where
        ...
        type Scopes ContentAccountsList = '[Content'FullControl]
        requestClient ContentAccountsList{..}
          = go merchantId xgafv accessToken callback label maxResults name
              pageToken
              uploadType
              uploadProtocol
              view
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ContentAccountsListResource)
                      Core.mempty
