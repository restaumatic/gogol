...
-- |
-- Module      : Gogol.ShoppingContent.Content.Accounts.Credentials.Create
                     ...
                     Core.QueryParam "uploadType" Core.Text Core.:>
                       Core.QueryParam "upload_protocol" Core.Text Core.:>
                         Core.QueryParam "alt" Core.AltJSON Core.:>
                           Core.ReqBody '[Core.JSON] AccountCredentials Core.:>
                             Core.Post '[Core.JSON] AccountCredentials
...
newContentAccountsCredentialsCreate accountId payload
  = ContentAccountsCredentialsCreate{xgafv = Core.Nothing,
...
instance Core.GoogleRequest ContentAccountsCredentialsCreate where
        type Rs ContentAccountsCredentialsCreate = AccountCredentials
        type Scopes ContentAccountsCredentialsCreate =
             '[Content'FullControl]
        requestClient ContentAccountsCredentialsCreate{..}
          = go accountId xgafv accessToken callback uploadType uploadProtocol
              (Core.Just Core.AltJSON)
              payload
              shoppingContentService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ContentAccountsCredentialsCreateResource)
                      Core.mempty
