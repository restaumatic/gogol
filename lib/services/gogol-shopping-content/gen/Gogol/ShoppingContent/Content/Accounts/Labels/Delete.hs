...
newContentAccountsLabelsDelete accountId labelId
  = ContentAccountsLabelsDelete{xgafv = Core.Nothing,
                                accessToken = Core.Nothing, accountId = accountId,
                                callback = Core.Nothing, labelId = labelId,
                                uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ContentAccountsLabelsDelete where
        type Rs ContentAccountsLabelsDelete = ()
        type Scopes ContentAccountsLabelsDelete = '[Content'FullControl]
        requestClient ContentAccountsLabelsDelete{..}
          = go accountId labelId xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              shoppingContentService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ContentAccountsLabelsDeleteResource)
                      Core.mempty
