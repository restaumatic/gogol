...
newContentPosGet merchantId storeCode targetMerchantId
  = ContentPosGet{xgafv = Core.Nothing, accessToken = Core.Nothing,
                  callback = Core.Nothing, merchantId = merchantId,
                  storeCode = storeCode, targetMerchantId = targetMerchantId,
...
instance Core.GoogleRequest ContentPosGet where
        ...
        requestClient ContentPosGet{..}
          = go merchantId targetMerchantId storeCode xgafv accessToken
              callback
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient (Core.Proxy :: Core.Proxy ContentPosGetResource)
                      Core.mempty
