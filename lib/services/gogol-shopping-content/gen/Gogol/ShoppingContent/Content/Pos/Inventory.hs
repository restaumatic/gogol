                       ...
                       Core.QueryParam "uploadType" Core.Text Core.:>
                         Core.QueryParam "upload_protocol" Core.Text Core.:>
                           Core.QueryParam "alt" Core.AltJSON Core.:>
                             Core.ReqBody '[Core.JSON] PosInventoryRequest Core.:>
...
newContentPosInventory merchantId payload targetMerchantId
  = ContentPosInventory{xgafv = Core.Nothing,
                        accessToken = Core.Nothing, callback = Core.Nothing,
                        merchantId = merchantId, payload = payload,
                        targetMerchantId = targetMerchantId, uploadType = Core.Nothing,
                        uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ContentPosInventory where
        type Rs ContentPosInventory = PosInventoryResponse
        type Scopes ContentPosInventory = '[Content'FullControl]
        requestClient ContentPosInventory{..}
          = go merchantId targetMerchantId xgafv accessToken callback
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ContentPosInventoryResource)
                      Core.mempty
