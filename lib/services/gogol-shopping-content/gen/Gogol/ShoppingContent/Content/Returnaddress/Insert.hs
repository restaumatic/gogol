...
newContentReturnaddressInsert merchantId payload
  = ContentReturnaddressInsert{xgafv = Core.Nothing,
                               accessToken = Core.Nothing, callback = Core.Nothing,
                               merchantId = merchantId, payload = payload,
                               uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ContentReturnaddressInsert where
        type Rs ContentReturnaddressInsert = ReturnAddress
        type Scopes ContentReturnaddressInsert = '[Content'FullControl]
        requestClient ContentReturnaddressInsert{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ContentReturnaddressInsertResource)
                      Core.mempty
