...
newContentReturnpolicyonlineDelete merchantId returnPolicyId
  = ContentReturnpolicyonlineDelete{xgafv = Core.Nothing,
                                    accessToken = Core.Nothing, callback = Core.Nothing,
                                    merchantId = merchantId, returnPolicyId = returnPolicyId,
                                    uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ContentReturnpolicyonlineDelete where
        type Rs ContentReturnpolicyonlineDelete = ()
        type Scopes ContentReturnpolicyonlineDelete =
             '[Content'FullControl]
        requestClient ContentReturnpolicyonlineDelete{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ContentReturnpolicyonlineDeleteResource)
                      Core.mempty
