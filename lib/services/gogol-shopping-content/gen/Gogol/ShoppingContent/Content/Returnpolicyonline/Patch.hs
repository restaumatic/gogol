...
newContentReturnpolicyonlinePatch merchantId payload returnPolicyId
  = ContentReturnpolicyonlinePatch{xgafv = Core.Nothing,
                                   accessToken = Core.Nothing, callback = Core.Nothing,
                                   merchantId = merchantId, payload = payload,
                                   returnPolicyId = returnPolicyId, uploadType = Core.Nothing,
                                   uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ContentReturnpolicyonlinePatch where
        type Rs ContentReturnpolicyonlinePatch = ReturnPolicyOnline
        type Scopes ContentReturnpolicyonlinePatch = '[Content'FullControl]
        requestClient ContentReturnpolicyonlinePatch{..}
          = go merchantId returnPolicyId xgafv accessToken callback
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ContentReturnpolicyonlinePatchResource)
                      Core.mempty
