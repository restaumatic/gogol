...
-- |
-- Module      : Gogol.ShoppingContent.Content.Accounttax.Get
...
newContentAccounttaxGet accountId merchantId
  = ContentAccounttaxGet{xgafv = Core.Nothing,
...
instance Core.GoogleRequest ContentAccounttaxGet where
        ...
        type Scopes ContentAccounttaxGet = '[Content'FullControl]
        requestClient ContentAccounttaxGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ContentAccounttaxGetResource)
                      Core.mempty
