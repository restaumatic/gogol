                       ...
                       Core.QueryParam "uploadType" Core.Text Core.:>
                         Core.QueryParam "upload_protocol" Core.Text Core.:>
                           Core.QueryParam "alt" Core.AltJSON Core.:>
                             Core.Get '[Core.JSON] DatafeedstatusesListResponse
...
newContentDatafeedstatusesList merchantId
  = ContentDatafeedstatusesList{xgafv = Core.Nothing,
                                accessToken = Core.Nothing, callback = Core.Nothing,
                                maxResults = Core.Nothing, merchantId = merchantId,
                                pageToken = Core.Nothing, uploadType = Core.Nothing,
                                uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ContentDatafeedstatusesList where
        type Rs ContentDatafeedstatusesList = DatafeedstatusesListResponse
        type Scopes ContentDatafeedstatusesList = '[Content'FullControl]
        requestClient ContentDatafeedstatusesList{..}
          = go merchantId xgafv accessToken callback maxResults pageToken
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ContentDatafeedstatusesListResource)
                      Core.mempty
