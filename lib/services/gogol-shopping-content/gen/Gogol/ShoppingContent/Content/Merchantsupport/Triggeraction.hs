                       ...
                       Core.QueryParam "uploadType" Core.Text Core.:>
                         Core.QueryParam "upload_protocol" Core.Text Core.:>
                           Core.QueryParam "alt" Core.AltJSON Core.:>
                             Core.ReqBody '[Core.JSON] TriggerActionPayload Core.:>
...
newContentMerchantsupportTriggeraction merchantId payload
  = ContentMerchantsupportTriggeraction{xgafv = Core.Nothing,
                                        accessToken = Core.Nothing, callback = Core.Nothing,
                                        languageCode = Core.Nothing, merchantId = merchantId,
                                        payload = payload, uploadType = Core.Nothing,
                                        uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ContentMerchantsupportTriggeraction
         where
        type Rs ContentMerchantsupportTriggeraction = TriggerActionResponse
        type Scopes ContentMerchantsupportTriggeraction =
             '[Content'FullControl]
        requestClient ContentMerchantsupportTriggeraction{..}
          = go merchantId xgafv accessToken callback languageCode uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy ContentMerchantsupportTriggeractionResource)
                      Core.mempty
