...
newContentRegionsDelete merchantId regionId
  = ContentRegionsDelete{xgafv = Core.Nothing,
...
instance Core.GoogleRequest ContentRegionsDelete where
        ...
        type Scopes ContentRegionsDelete = '[Content'FullControl]
        requestClient ContentRegionsDelete{..}
          = go merchantId regionId xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              shoppingContentService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ContentRegionsDeleteResource)
                      Core.mempty
