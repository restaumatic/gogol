                       ...
                       Core.QueryParam "uploadType" Core.Text Core.:>
                         Core.QueryParam "upload_protocol" Core.Text Core.:>
                           Core.QueryParam "alt" Core.AltJSON Core.:>
                             Core.Get '[Core.JSON] ListCollectionStatusesResponse
...
newContentCollectionstatusesList merchantId
  = ContentCollectionstatusesList{xgafv = Core.Nothing,
                                  accessToken = Core.Nothing, callback = Core.Nothing,
                                  merchantId = merchantId, pageSize = Core.Nothing,
                                  pageToken = Core.Nothing, uploadType = Core.Nothing,
                                  uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ContentCollectionstatusesList where
        type Rs ContentCollectionstatusesList =
             ListCollectionStatusesResponse
        type Scopes ContentCollectionstatusesList = '[Content'FullControl]
        requestClient ContentCollectionstatusesList{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ContentCollectionstatusesListResource)
                      Core.mempty
