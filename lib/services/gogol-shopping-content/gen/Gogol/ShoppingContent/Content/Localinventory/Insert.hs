...
newContentLocalinventoryInsert merchantId payload productId
  = ContentLocalinventoryInsert{xgafv = Core.Nothing,
                                accessToken = Core.Nothing, callback = Core.Nothing,
                                merchantId = merchantId, payload = payload, productId = productId,
                                uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ContentLocalinventoryInsert where
        type Rs ContentLocalinventoryInsert = LocalInventory
        type Scopes ContentLocalinventoryInsert = '[Content'FullControl]
        requestClient ContentLocalinventoryInsert{..}
          = go merchantId productId xgafv accessToken callback uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ContentLocalinventoryInsertResource)
                      Core.mempty
