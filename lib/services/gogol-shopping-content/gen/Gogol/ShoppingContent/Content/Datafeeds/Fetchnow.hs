     ...
     "content" Core.:>
       "v2.1" Core.:>
         Core.Capture "merchantId" Core.Word64 Core.:>
           "datafeeds" Core.:>
             Core.Capture "datafeedId" Core.Word64 Core.:>
               "fetchNow" Core.:>
                       ...
                       Core.QueryParam "uploadType" Core.Text Core.:>
                         Core.QueryParam "upload_protocol" Core.Text Core.:>
                           Core.QueryParam "alt" Core.AltJSON Core.:>
                             Core.Post '[Core.JSON] DatafeedsFetchNowResponse
...
newContentDatafeedsFetchnow datafeedId merchantId
  = ContentDatafeedsFetchnow{xgafv = Core.Nothing,
                             accessToken = Core.Nothing, callback = Core.Nothing,
                             datafeedId = datafeedId, merchantId = merchantId,
                             uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ContentDatafeedsFetchnow where
        type Rs ContentDatafeedsFetchnow = DatafeedsFetchNowResponse
        type Scopes ContentDatafeedsFetchnow = '[Content'FullControl]
        requestClient ContentDatafeedsFetchnow{..}
          = go merchantId datafeedId xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              shoppingContentService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ContentDatafeedsFetchnowResource)
                      Core.mempty
