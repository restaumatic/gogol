...
newContentDatafeedsInsert merchantId payload
  = ContentDatafeedsInsert{xgafv = Core.Nothing,
                           accessToken = Core.Nothing, callback = Core.Nothing,
                           merchantId = merchantId, payload = payload,
                           uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ContentDatafeedsInsert where
        type Rs ContentDatafeedsInsert = Datafeed
        type Scopes ContentDatafeedsInsert = '[Content'FullControl]
        requestClient ContentDatafeedsInsert{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ContentDatafeedsInsertResource)
                      Core.mempty
