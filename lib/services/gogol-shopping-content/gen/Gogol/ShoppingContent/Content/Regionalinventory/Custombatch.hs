                   ...
                   Core.QueryParam "uploadType" Core.Text Core.:>
                     Core.QueryParam "upload_protocol" Core.Text Core.:>
                       Core.QueryParam "alt" Core.AltJSON Core.:>
                         Core.ReqBody '[Core.JSON] RegionalinventoryCustomBatchRequest
                           Core.:> Core.Post '[Core.JSON] RegionalinventoryCustomBatchResponse
...
newContentRegionalinventoryCustombatch payload
  = ContentRegionalinventoryCustombatch{xgafv = Core.Nothing,
                                        accessToken = Core.Nothing, callback = Core.Nothing,
                                        payload = payload, uploadType = Core.Nothing,
                                        uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ContentRegionalinventoryCustombatch
         where
        type Rs ContentRegionalinventoryCustombatch =
             RegionalinventoryCustomBatchResponse
        type Scopes ContentRegionalinventoryCustombatch =
             '[Content'FullControl]
        requestClient ContentRegionalinventoryCustombatch{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy ContentRegionalinventoryCustombatchResource)
                      Core.mempty
