...
newContentLiasettingsUpdate accountId merchantId payload
  = ContentLiasettingsUpdate{xgafv = Core.Nothing,
...
instance Core.GoogleRequest ContentLiasettingsUpdate where
        type Rs ContentLiasettingsUpdate = LiaSettings
        type Scopes ContentLiasettingsUpdate = '[Content'FullControl]
        requestClient ContentLiasettingsUpdate{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ContentLiasettingsUpdateResource)
                      Core.mempty
