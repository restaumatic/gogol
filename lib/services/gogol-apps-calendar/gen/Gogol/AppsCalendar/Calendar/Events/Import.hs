...
--
-- /See:/ <https://developers.google.com/workspace/calendar/firstapp Calendar API Reference> for @calendar.events.import@.
module Gogol.AppsCalendar.Calendar.Events.Import
    (
    -- * Resource
...
newCalendarEventsImport calendarId payload
  = CalendarEventsImport{calendarId = calendarId,
                         conferenceDataVersion = Core.Nothing, payload = payload,
...
instance Core.GoogleRequest CalendarEventsImport where
        ...
        requestClient CalendarEventsImport{..}
          = go calendarId conferenceDataVersion supportsAttachments
