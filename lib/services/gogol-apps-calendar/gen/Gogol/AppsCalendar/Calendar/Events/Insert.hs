...
-- Creates an event.
--
-- /See:/ <https://developers.google.com/workspace/calendar/firstapp Calendar API Reference> for @calendar.events.insert@.
module Gogol.AppsCalendar.Calendar.Events.Insert
    (
    -- * Resource
               ...
               Core.QueryParam "conferenceDataVersion" Core.Int32 Core.:>
                 Core.QueryParam "maxAttendees" Core.Int32 Core.:>
                   Core.QueryParam "sendNotifications" Core.Bool Core.:>
                     Core.QueryParam "sendUpdates" EventsInsertSendUpdates Core.:>
...
newCalendarEventsInsert calendarId payload
  = CalendarEventsInsert{calendarId = calendarId,
                         conferenceDataVersion = Core.Nothing, maxAttendees = Core.Nothing,
                         payload = payload, sendNotifications = Core.Nothing,
                         sendUpdates = Core.Nothing, supportsAttachments = Core.Nothing}
instance Core.GoogleRequest CalendarEventsInsert where
        type Rs CalendarEventsInsert = Event
        type Scopes CalendarEventsInsert =
             '[Calendar'FullControl, Calendar'App'Created, Calendar'Events,
               Calendar'Events'Owned]
        requestClient CalendarEventsInsert{..}
          = go calendarId conferenceDataVersion maxAttendees
              sendNotifications
              sendUpdates
              supportsAttachments
