...
--
-- /See:/ <https://developers.google.com/workspace/calendar/firstapp Calendar API Reference> for @calendar.calendarList.update@.
module Gogol.AppsCalendar.Calendar.CalendarList.Update
    (
    -- * Resource
...
newCalendarCalendarListUpdate calendarId payload
  = CalendarCalendarListUpdate{calendarId = calendarId,
                               colorRgbFormat = Core.Nothing, payload = payload}
instance Core.GoogleRequest CalendarCalendarListUpdate where
        ...
        requestClient CalendarCalendarListUpdate{..}
          = go calendarId colorRgbFormat (Core.Just Core.AltJSON) payload
              appsCalendarService
          where go
                  = Core.buildClient
