...
--
-- /See:/ <https://developers.google.com/workspace/calendar/firstapp Calendar API Reference> for @calendar.calendarList.watch@.
module Gogol.AppsCalendar.Calendar.CalendarList.Watch
    (
    -- * Resource
             ...
             "calendarList" Core.:>
               "watch" Core.:>
                 Core.QueryParam "maxResults" Core.Int32 Core.:>
                   Core.QueryParam "minAccessRole" CalendarListWatchMinAccessRole
                     Core.:>
...
newCalendarCalendarListWatch payload
  = CalendarCalendarListWatch{maxResults = Core.Nothing,
                              minAccessRole = Core.Nothing, pageToken = Core.Nothing,
                              payload = payload, showDeleted = Core.Nothing,
                              showHidden = Core.Nothing, syncToken = Core.Nothing}
instance Core.GoogleRequest CalendarCalendarListWatch where
        type Rs CalendarCalendarListWatch = Channel
        type Scopes CalendarCalendarListWatch =
             '[Calendar'FullControl, Calendar'Calendarlist,
               Calendar'Calendarlist'Readonly, Calendar'Readonly]
        requestClient CalendarCalendarListWatch{..}
          = go maxResults minAccessRole pageToken showDeleted showHidden
              syncToken
