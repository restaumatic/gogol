...
--
-- /See:/ <https://developers.google.com/workspace/calendar/firstapp Calendar API Reference> for @calendar.settings.list@.
module Gogol.AppsCalendar.Calendar.Settings.List
    (
    -- * Resource
...
newCalendarSettingsList 
    ::  CalendarSettingsList
newCalendarSettingsList
  = CalendarSettingsList{maxResults = Core.Nothing,
...
instance Core.GoogleRequest CalendarSettingsList where
        ...
        type Scopes CalendarSettingsList =
             '[Calendar'FullControl, Calendar'Readonly,
               Calendar'Settings'Readonly]
        requestClient CalendarSettingsList{..}
          = go maxResults pageToken syncToken (Core.Just Core.AltJSON)
              appsCalendarService
          where go
                  = Core.buildClient
