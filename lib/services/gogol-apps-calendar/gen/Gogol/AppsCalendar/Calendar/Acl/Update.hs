...
--
-- /See:/ <https://developers.google.com/workspace/calendar/firstapp Calendar API Reference> for @calendar.acl.update@.
module Gogol.AppsCalendar.Calendar.Acl.Update
    (
    -- * Resource
...
newCalendarAclUpdate calendarId payload ruleId
  = CalendarAclUpdate{calendarId = calendarId, payload = payload,
                      ruleId = ruleId, sendNotifications = Core.Nothing}
instance Core.GoogleRequest CalendarAclUpdate where
        ...
        requestClient CalendarAclUpdate{..}
          = go calendarId ruleId sendNotifications (Core.Just Core.AltJSON)
              payload
              appsCalendarService
          where go
                  = Core.buildClient
