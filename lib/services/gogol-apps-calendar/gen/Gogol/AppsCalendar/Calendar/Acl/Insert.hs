...
--
-- /See:/ <https://developers.google.com/workspace/calendar/firstapp Calendar API Reference> for @calendar.acl.insert@.
module Gogol.AppsCalendar.Calendar.Acl.Insert
    (
    -- * Resource
...
newCalendarAclInsert calendarId payload
  = CalendarAclInsert{calendarId = calendarId, payload = payload,
...
instance Core.GoogleRequest CalendarAclInsert where
        ...
        requestClient CalendarAclInsert{..}
          = go calendarId sendNotifications (Core.Just Core.AltJSON) payload
              appsCalendarService
          where go
                  = Core.buildClient
