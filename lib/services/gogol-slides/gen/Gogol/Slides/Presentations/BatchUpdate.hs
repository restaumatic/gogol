...
--
-- /See:/ <https://developers.google.com/workspace/slides/ Google Slides API Reference> for @slides.presentations.batchUpdate@.
module Gogol.Slides.Presentations.BatchUpdate
    (
    -- * Resource
...
type SlidesPresentationsBatchUpdateResource =
     "v1" Core.:>
       "presentations" Core.:>
         Core.CaptureMode "presentationId" "batchUpdate" Core.Text Core.:>
                 ...
                 Core.QueryParam "uploadType" Core.Text Core.:>
                   Core.QueryParam "upload_protocol" Core.Text Core.:>
                     Core.QueryParam "alt" Core.AltJSON Core.:>
                       Core.ReqBody '[Core.JSON] BatchUpdatePresentationRequest Core.:>
                         Core.Post '[Core.JSON] BatchUpdatePresentationResponse
...
newSlidesPresentationsBatchUpdate payload presentationId
  = SlidesPresentationsBatchUpdate{xgafv = Core.Nothing,
                                   accessToken = Core.Nothing, callback = Core.Nothing,
                                   payload = payload, presentationId = presentationId,
                                   uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest SlidesPresentationsBatchUpdate where
        type Rs SlidesPresentationsBatchUpdate =
             BatchUpdatePresentationResponse
        type Scopes SlidesPresentationsBatchUpdate =
             '[Drive'FullControl, Drive'File, Drive'Readonly,
               Presentations'FullControl, Spreadsheets'FullControl,
               Spreadsheets'Readonly]
        requestClient SlidesPresentationsBatchUpdate{..}
          = go presentationId xgafv accessToken callback uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy SlidesPresentationsBatchUpdateResource)
                      Core.mempty
