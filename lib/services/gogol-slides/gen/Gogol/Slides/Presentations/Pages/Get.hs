...
--
-- /See:/ <https://developers.google.com/workspace/slides/ Google Slides API Reference> for @slides.presentations.pages.get@.
module Gogol.Slides.Presentations.Pages.Get
    (
    -- * Resource
...
newSlidesPresentationsPagesGet pageObjectId presentationId
  = SlidesPresentationsPagesGet{xgafv = Core.Nothing,
                                accessToken = Core.Nothing, callback = Core.Nothing,
                                pageObjectId = pageObjectId, presentationId = presentationId,
                                uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest SlidesPresentationsPagesGet where
        type Rs SlidesPresentationsPagesGet = Page
        type Scopes SlidesPresentationsPagesGet =
        ...
        requestClient SlidesPresentationsPagesGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy SlidesPresentationsPagesGetResource)
                      Core.mempty
