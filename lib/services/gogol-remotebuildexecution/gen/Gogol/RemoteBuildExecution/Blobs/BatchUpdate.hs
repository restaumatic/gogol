...
newRemoteBuildExecutionBlobsBatchUpdate instanceName payload
  = RemoteBuildExecutionBlobsBatchUpdate{xgafv = Core.Nothing,
                                         accessToken = Core.Nothing, callback = Core.Nothing,
                                         instanceName = instanceName, payload = payload,
                                         uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest RemoteBuildExecutionBlobsBatchUpdate
         where
        type Rs RemoteBuildExecutionBlobsBatchUpdate =
             BuildBazelRemoteExecutionV2BatchUpdateBlobsResponse
        type Scopes RemoteBuildExecutionBlobsBatchUpdate =
             '[CloudPlatform'FullControl]
        requestClient RemoteBuildExecutionBlobsBatchUpdate{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy RemoteBuildExecutionBlobsBatchUpdateResource)
                      Core.mempty
