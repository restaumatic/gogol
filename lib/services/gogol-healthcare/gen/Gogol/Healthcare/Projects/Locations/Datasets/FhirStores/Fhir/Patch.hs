...
--
-- Updates part of an existing resource by applying the operations specified in a <http://jsonpatch.com/ JSON Patch> document. Implements the FHIR standard patch interaction (<https://hl7.org/fhir/STU3/http.html#patch STU3>, <https://hl7.org/fhir/R4/http.html#patch R4>, <https://hl7.org/fhir/R5/http.html#patch R5>). DSTU2 doesn\'t define a patch method, but the server supports it in the same way it supports STU3. The request body must contain a JSON Patch document, and the request headers must contain @Content-Type: application\/json-patch+json@. On success, the response body contains a JSON-encoded representation of the updated resource, including the server-assigned version ID. Errors generated by the FHIR store contain a JSON-encoded @OperationOutcome@ resource describing the reason for the error. If the request cannot be mapped to a valid API method on a FHIR store, a generic GCP error might be returned instead. For samples that show how to call @patch@, see
...
newHealthcareProjectsLocationsDatasetsFhirStoresFhirPatch 
    ...
    -> HealthcareProjectsLocationsDatasetsFhirStoresFhirPatch
newHealthcareProjectsLocationsDatasetsFhirStoresFhirPatch name
  payload
  = HealthcareProjectsLocationsDatasetsFhirStoresFhirPatch{xgafv =
                                                             Core.Nothing,
...
instance Core.GoogleRequest
           HealthcareProjectsLocationsDatasetsFhirStoresFhirPatch
         where
        type Rs HealthcareProjectsLocationsDatasetsFhirStoresFhirPatch =
             HttpBody
        type Scopes HealthcareProjectsLocationsDatasetsFhirStoresFhirPatch
