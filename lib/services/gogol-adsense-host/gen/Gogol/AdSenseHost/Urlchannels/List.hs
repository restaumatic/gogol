...
newAdSenseHostUrlchannelsList adClientId
  = AdSenseHostUrlchannelsList{adClientId = adClientId,
                               maxResults = Core.Nothing, pageToken = Core.Nothing}
instance Core.GoogleRequest AdSenseHostUrlchannelsList where
        type Rs AdSenseHostUrlchannelsList = UrlChannels
        type Scopes AdSenseHostUrlchannelsList = '[Adsensehost'FullControl]
        requestClient AdSenseHostUrlchannelsList{..}
          = go adClientId maxResults pageToken (Core.Just Core.AltJSON)
              adSenseHostService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy AdSenseHostUrlchannelsListResource)
                      Core.mempty
