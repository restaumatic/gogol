...
newAdSenseHostAccountsAdunitsList accountId adClientId
  = AdSenseHostAccountsAdunitsList{accountId = accountId,
                                   adClientId = adClientId, includeInactive = Core.Nothing,
                                   maxResults = Core.Nothing, pageToken = Core.Nothing}
instance Core.GoogleRequest AdSenseHostAccountsAdunitsList where
        type Rs AdSenseHostAccountsAdunitsList = AdUnits
        type Scopes AdSenseHostAccountsAdunitsList =
             '[Adsensehost'FullControl]
        requestClient AdSenseHostAccountsAdunitsList{..}
          = go accountId adClientId includeInactive maxResults pageToken
              (Core.Just Core.AltJSON)
              adSenseHostService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy AdSenseHostAccountsAdunitsListResource)
                      Core.mempty
