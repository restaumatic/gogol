...
instance Core.GoogleRequest AdSenseHostAccountsList where
        ...
        type Scopes AdSenseHostAccountsList = '[Adsensehost'FullControl]
        requestClient AdSenseHostAccountsList{..}
          = go filterAdClientId (Core.Just Core.AltJSON) adSenseHostService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy AdSenseHostAccountsListResource)
                      Core.mempty
