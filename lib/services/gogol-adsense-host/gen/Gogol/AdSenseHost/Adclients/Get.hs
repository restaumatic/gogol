...
instance Core.GoogleRequest AdSenseHostAdclientsGet where
        ...
        type Scopes AdSenseHostAdclientsGet = '[Adsensehost'FullControl]
        requestClient AdSenseHostAdclientsGet{..}
          = go adClientId (Core.Just Core.AltJSON) adSenseHostService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy AdSenseHostAdclientsGetResource)
                      Core.mempty
