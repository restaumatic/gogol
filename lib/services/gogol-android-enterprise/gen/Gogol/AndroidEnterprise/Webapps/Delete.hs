       ...
       "v1" Core.:>
         "enterprises" Core.:>
           Core.Capture "enterpriseId" Core.Text Core.:>
             "webApps" Core.:>
               Core.Capture "webAppId" Core.Text Core.:>
...
newAndroidEnterpriseWebappsDelete enterpriseId webAppId
  = AndroidEnterpriseWebappsDelete{xgafv = Core.Nothing,
                                   accessToken = Core.Nothing, callback = Core.Nothing,
                                   enterpriseId = enterpriseId, uploadType = Core.Nothing,
                                   uploadProtocol = Core.Nothing, webAppId = webAppId}
instance Core.GoogleRequest AndroidEnterpriseWebappsDelete where
        type Rs AndroidEnterpriseWebappsDelete = ()
        type Scopes AndroidEnterpriseWebappsDelete =
             '[Androidenterprise'FullControl]
        requestClient AndroidEnterpriseWebappsDelete{..}
          = go enterpriseId webAppId xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              androidEnterpriseService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy AndroidEnterpriseWebappsDeleteResource)
                      Core.mempty
