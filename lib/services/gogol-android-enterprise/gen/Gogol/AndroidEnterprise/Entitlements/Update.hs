...
newAndroidEnterpriseEntitlementsUpdate 
    ...
    -> AndroidEnterpriseEntitlementsUpdate
newAndroidEnterpriseEntitlementsUpdate enterpriseId entitlementId
  payload userId
  = AndroidEnterpriseEntitlementsUpdate{xgafv = Core.Nothing,
                                        accessToken = Core.Nothing, callback = Core.Nothing,
                                        enterpriseId = enterpriseId, entitlementId = entitlementId,
                                        install = Core.Nothing, payload = payload,
                                        uploadType = Core.Nothing, uploadProtocol = Core.Nothing,
                                        userId = userId}
instance Core.GoogleRequest AndroidEnterpriseEntitlementsUpdate
         where
        type Rs AndroidEnterpriseEntitlementsUpdate = Entitlement
        type Scopes AndroidEnterpriseEntitlementsUpdate =
             '[Androidenterprise'FullControl]
        requestClient AndroidEnterpriseEntitlementsUpdate{..}
          = go enterpriseId userId entitlementId xgafv accessToken callback
              install
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy AndroidEnterpriseEntitlementsUpdateResource)
                      Core.mempty
