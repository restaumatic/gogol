...
-- |
-- Module      : Gogol.AndroidEnterprise.Permissions.Get
...
newAndroidEnterprisePermissionsGet permissionId
  = AndroidEnterprisePermissionsGet{xgafv = Core.Nothing,
                                    accessToken = Core.Nothing, callback = Core.Nothing,
                                    language = Core.Nothing, permissionId = permissionId,
                                    uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest AndroidEnterprisePermissionsGet where
        type Rs AndroidEnterprisePermissionsGet = Permission
        type Scopes AndroidEnterprisePermissionsGet =
             '[Androidenterprise'FullControl]
        requestClient AndroidEnterprisePermissionsGet{..}
          = go permissionId xgafv accessToken callback language uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              androidEnterpriseService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy AndroidEnterprisePermissionsGetResource)
                      Core.mempty
