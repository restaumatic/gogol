...
newIAPTestIamPermissions payload resource
  = IAPTestIamPermissions{xgafv = Core.Nothing,
...
instance Core.GoogleRequest IAPTestIamPermissions where
        type Rs IAPTestIamPermissions = TestIamPermissionsResponse
        type Scopes IAPTestIamPermissions = '[CloudPlatform'FullControl]
        requestClient IAPTestIamPermissions{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy IAPTestIamPermissionsResource)
                      Core.mempty
