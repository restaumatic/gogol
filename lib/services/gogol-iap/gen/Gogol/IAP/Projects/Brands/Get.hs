...
-- |
-- Module      : Gogol.IAP.Projects.Brands.Get
...
newIAPProjectsBrandsGet name
  = IAPProjectsBrandsGet{xgafv = Core.Nothing,
...
instance Core.GoogleRequest IAPProjectsBrandsGet where
        ...
        type Scopes IAPProjectsBrandsGet = '[CloudPlatform'FullControl]
        requestClient IAPProjectsBrandsGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy IAPProjectsBrandsGetResource)
                      Core.mempty
