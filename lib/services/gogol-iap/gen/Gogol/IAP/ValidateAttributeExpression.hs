...
type IAPValidateAttributeExpressionResource =
     "v1" Core.:>
       Core.CaptureMode "name" "validateAttributeExpression" Core.Text
         Core.:>
         Core.QueryParam "$.xgafv" Xgafv Core.:>
           Core.QueryParam "access_token" Core.Text Core.:>
             Core.QueryParam "callback" Core.Text Core.:>
               Core.QueryParam "expression" Core.Text Core.:>
                 Core.QueryParam "uploadType" Core.Text Core.:>
                   Core.QueryParam "upload_protocol" Core.Text Core.:>
                     Core.QueryParam "alt" Core.AltJSON Core.:>
                       Core.Post '[Core.JSON] ValidateIapAttributeExpressionResponse
...
newIAPValidateAttributeExpression name
  = IAPValidateAttributeExpression{xgafv = Core.Nothing,
                                   accessToken = Core.Nothing, callback = Core.Nothing,
                                   expression = Core.Nothing, name = name,
                                   uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest IAPValidateAttributeExpression where
        type Rs IAPValidateAttributeExpression =
             ValidateIapAttributeExpressionResponse
        type Scopes IAPValidateAttributeExpression =
             '[CloudPlatform'FullControl]
        requestClient IAPValidateAttributeExpression{..}
          = go name xgafv accessToken callback expression uploadType
              uploadProtocol
