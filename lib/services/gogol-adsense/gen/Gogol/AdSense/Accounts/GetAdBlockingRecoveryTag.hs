                 ...
                 Core.QueryParam "uploadType" Core.Text Core.:>
                   Core.QueryParam "upload_protocol" Core.Text Core.:>
                     Core.QueryParam "alt" Core.AltJSON Core.:>
                       Core.Get '[Core.JSON] AdBlockingRecoveryTag
...
newAdSenseAccountsGetAdBlockingRecoveryTag name
  = AdSenseAccountsGetAdBlockingRecoveryTag{xgafv = Core.Nothing,
                                            accessToken = Core.Nothing, callback = Core.Nothing,
                                            name = name, uploadType = Core.Nothing,
                                            uploadProtocol = Core.Nothing}
instance Core.GoogleRequest AdSenseAccountsGetAdBlockingRecoveryTag
         where
        type Rs AdSenseAccountsGetAdBlockingRecoveryTag =
             AdBlockingRecoveryTag
        type Scopes AdSenseAccountsGetAdBlockingRecoveryTag =
             '[Adsense'FullControl, Adsense'Readonly]
        requestClient AdSenseAccountsGetAdBlockingRecoveryTag{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy AdSenseAccountsGetAdBlockingRecoveryTagResource)
                      Core.mempty
