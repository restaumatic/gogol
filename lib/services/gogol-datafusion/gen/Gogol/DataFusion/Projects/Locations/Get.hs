...
type DataFusionProjectsLocationsGetResource =
     "v1" Core.:>
...
-- | Gets information about a location.
--
-- /See:/ 'newDataFusionProjectsLocationsGet' smart constructor.
data DataFusionProjectsLocationsGet = DataFusionProjectsLocationsGet
    {
      -- | V1 error format.
...
-- | Creates a value of 'DataFusionProjectsLocationsGet' with the minimum fields required to make a request.
newDataFusionProjectsLocationsGet 
    :: 
                                  Core.Text
       -- ^  Resource name for the location. See 'name'.
    -> DataFusionProjectsLocationsGet
newDataFusionProjectsLocationsGet name
  = DataFusionProjectsLocationsGet{xgafv = Core.Nothing,
                                   accessToken = Core.Nothing, callback = Core.Nothing, name = name,
                                   uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DataFusionProjectsLocationsGet where
        type Rs DataFusionProjectsLocationsGet = Location
        type Scopes DataFusionProjectsLocationsGet =
             '[CloudPlatform'FullControl]
        requestClient DataFusionProjectsLocationsGet{..}
          = go name xgafv accessToken callback uploadType uploadProtocol
              (Core.Just Core.AltJSON)
              dataFusionService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DataFusionProjectsLocationsGetResource)
                      Core.mempty
