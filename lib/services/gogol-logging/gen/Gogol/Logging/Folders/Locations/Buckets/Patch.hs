...
-- 'LoggingFoldersLocationsBucketsPatch' request conforms to.
type LoggingFoldersLocationsBucketsPatchResource =
     "v2" Core.:>
...
-- | Creates a value of 'LoggingFoldersLocationsBucketsPatch' with the minimum fields required to make a request.
newLoggingFoldersLocationsBucketsPatch 
    :: 
                                       Core.Text
    ...
    -> LoggingFoldersLocationsBucketsPatch
newLoggingFoldersLocationsBucketsPatch name payload
  = LoggingFoldersLocationsBucketsPatch{xgafv = Core.Nothing,
                                        accessToken = Core.Nothing, callback = Core.Nothing,
                                        name = name, payload = payload, updateMask = Core.Nothing,
                                        uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest LoggingFoldersLocationsBucketsPatch
         where
        type Rs LoggingFoldersLocationsBucketsPatch = LogBucket
        type Scopes LoggingFoldersLocationsBucketsPatch =
             '[CloudPlatform'FullControl, Logging'Admin]
        requestClient LoggingFoldersLocationsBucketsPatch{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy LoggingFoldersLocationsBucketsPatchResource)
                      Core.mempty
