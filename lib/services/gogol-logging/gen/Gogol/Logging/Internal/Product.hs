...
data LogScope = LogScope
    ...
    , name :: (Core.Maybe Core.Text)
      -- | Required. Names of one or more parent resources (organizations and folders are not supported.): projects\/[PROJECT/ID]May alternatively be one or more views: projects\/[PROJECT/ID]\/locations\/[LOCATION/ID]\/buckets\/[BUCKET/ID]\/views\/[VIEW_ID]A log scope can include a maximum of 5 projects and a maximum of 100 resources in total.
    , resourceNames :: (Core.Maybe [Core.Text])
