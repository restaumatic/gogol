               ...
               Core.QueryParam "uploadType" Core.Text Core.:>
                 Core.QueryParam "upload_protocol" Core.Text Core.:>
                   Core.QueryParam "alt" Core.AltJSON Core.:>
                     Core.ReqBody '[Core.JSON] LogMetric Core.:>
                       Core.Put '[Core.JSON] LogMetric
...
newLoggingProjectsMetricsUpdate metricName payload
  = LoggingProjectsMetricsUpdate{xgafv = Core.Nothing,
                                 accessToken = Core.Nothing, callback = Core.Nothing,
                                 metricName = metricName, payload = payload,
                                 uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest LoggingProjectsMetricsUpdate where
        type Rs LoggingProjectsMetricsUpdate = LogMetric
        type Scopes LoggingProjectsMetricsUpdate =
        ...
        requestClient LoggingProjectsMetricsUpdate{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy LoggingProjectsMetricsUpdateResource)
                      Core.mempty
