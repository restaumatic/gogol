...
-- |
-- Module      : Gogol.Logging.Projects.Locations.Buckets.List
...
-- 'LoggingProjectsLocationsBucketsList' request conforms to.
type LoggingProjectsLocationsBucketsListResource =
     "v2" Core.:>
...
-- | Lists log buckets.
--
-- /See:/ 'newLoggingProjectsLocationsBucketsList' smart constructor.
data LoggingProjectsLocationsBucketsList = LoggingProjectsLocationsBucketsList
    {
      -- | V1 error format.
...
-- | Creates a value of 'LoggingProjectsLocationsBucketsList' with the minimum fields required to make a request.
newLoggingProjectsLocationsBucketsList 
    :: 
                                       Core.Text
    ...
    -> LoggingProjectsLocationsBucketsList
newLoggingProjectsLocationsBucketsList parent
  = LoggingProjectsLocationsBucketsList{xgafv = Core.Nothing,
                                        accessToken = Core.Nothing, callback = Core.Nothing,
                                        pageSize = Core.Nothing, pageToken = Core.Nothing,
                                        parent = parent, uploadType = Core.Nothing,
                                        uploadProtocol = Core.Nothing}
instance Core.GoogleRequest LoggingProjectsLocationsBucketsList
         where
        type Rs LoggingProjectsLocationsBucketsList = ListBucketsResponse
        type Scopes LoggingProjectsLocationsBucketsList =
             '[CloudPlatform'FullControl, CloudPlatform'ReadOnly, Logging'Admin,
               Logging'Read]
        requestClient LoggingProjectsLocationsBucketsList{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy LoggingProjectsLocationsBucketsListResource)
                      Core.mempty
