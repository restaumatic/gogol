...
newLoggingProjectsLocationsBucketsPatch name payload
  = LoggingProjectsLocationsBucketsPatch{xgafv = Core.Nothing,
                                         accessToken = Core.Nothing, callback = Core.Nothing,
                                         name = name, payload = payload, updateMask = Core.Nothing,
                                         uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest LoggingProjectsLocationsBucketsPatch
         where
        type Rs LoggingProjectsLocationsBucketsPatch = LogBucket
        type Scopes LoggingProjectsLocationsBucketsPatch =
             '[CloudPlatform'FullControl, Logging'Admin]
        requestClient LoggingProjectsLocationsBucketsPatch{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy LoggingProjectsLocationsBucketsPatchResource)
                      Core.mempty
