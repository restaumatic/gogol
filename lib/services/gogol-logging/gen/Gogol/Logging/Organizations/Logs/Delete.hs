...
newLoggingOrganizationsLogsDelete logName
  = LoggingOrganizationsLogsDelete{xgafv = Core.Nothing,
                                   accessToken = Core.Nothing, callback = Core.Nothing,
                                   logName = logName, uploadType = Core.Nothing,
                                   uploadProtocol = Core.Nothing}
instance Core.GoogleRequest LoggingOrganizationsLogsDelete where
        type Rs LoggingOrganizationsLogsDelete = Empty
        type Scopes LoggingOrganizationsLogsDelete =
             '[CloudPlatform'FullControl, Logging'Admin]
        requestClient LoggingOrganizationsLogsDelete{..}
          = go logName xgafv accessToken callback uploadType uploadProtocol
              (Core.Just Core.AltJSON)
              loggingService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy LoggingOrganizationsLogsDeleteResource)
                      Core.mempty
