...
--
-- Creates a file. For more information, see </workspace/drive/api/guides/create-file Create and manage files>. This method supports an /\/upload/ URI and accepts uploaded media with the following characteristics: - /Maximum file size:/ 5,120 GB - /Accepted Media MIME types:/ @*\/*@ (Specify a valid MIME type, rather than the literal @*\/*@ value. The literal @*\/*@ is only used to indicate that any valid MIME type can be uploaded. For more information, see </workspace/drive/api/guides/mime-types Google Workspace and Google Drive supported MIME types>.) For more information on uploading files, see </workspace/drive/api/guides/manage-uploads Upload file data>. Apps creating shortcuts with the @create@ method must specify the MIME type @application\/vnd.google-apps.shortcut@. Apps should specify a file extension in the @name@ property when inserting files with the API. For example, an operation to insert a JPEG file should specify something like @\"name\": \"cat.jpg\"@ in the metadata. Subsequent @GET@ requests
