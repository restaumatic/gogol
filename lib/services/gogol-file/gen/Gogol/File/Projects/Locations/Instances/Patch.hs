...
data FileProjectsLocationsInstancesPatch = FileProjectsLocationsInstancesPatch
    ...
    , payload :: Instance
      -- | Mask of fields to update. At least one path must be supplied in this field. The elements of the repeated paths field may only include these fields: * \"description\" * \"file/shares\" * \"labels\" * \"performance/config\" * \"deletion/protection/enabled\" * \"deletion/protection/reason\"
...
newFileProjectsLocationsInstancesPatch name payload
  = FileProjectsLocationsInstancesPatch{xgafv = Core.Nothing,
                                        accessToken = Core.Nothing, callback = Core.Nothing,
                                        name = name, payload = payload, updateMask = Core.Nothing,
                                        uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest FileProjectsLocationsInstancesPatch
         where
        type Rs FileProjectsLocationsInstancesPatch = Operation
        ...
        requestClient FileProjectsLocationsInstancesPatch{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy FileProjectsLocationsInstancesPatchResource)
                      Core.mempty
