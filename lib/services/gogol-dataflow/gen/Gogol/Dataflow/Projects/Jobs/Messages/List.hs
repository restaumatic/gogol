                                   ...
                                   Core.QueryParam "uploadType" Core.Text Core.:>
                                     Core.QueryParam "upload_protocol" Core.Text Core.:>
                                       Core.QueryParam "alt" Core.AltJSON Core.:>
                                         Core.Get '[Core.JSON] ListJobMessagesResponse
...
newDataflowProjectsJobsMessagesList jobId projectId
  = DataflowProjectsJobsMessagesList{xgafv = Core.Nothing,
                                     accessToken = Core.Nothing, callback = Core.Nothing,
                                     endTime = Core.Nothing, jobId = jobId, location = Core.Nothing,
                                     minimumImportance = Core.Nothing, pageSize = Core.Nothing,
                                     pageToken = Core.Nothing, projectId = projectId,
                                     startTime = Core.Nothing, uploadType = Core.Nothing,
                                     uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DataflowProjectsJobsMessagesList where
        type Rs DataflowProjectsJobsMessagesList = ListJobMessagesResponse
        type Scopes DataflowProjectsJobsMessagesList =
             '[CloudPlatform'FullControl, Compute'FullControl]
        requestClient DataflowProjectsJobsMessagesList{..}
          = go projectId jobId xgafv accessToken callback endTime location
              minimumImportance
              pageSize
              pageToken
              startTime
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DataflowProjectsJobsMessagesListResource)
                      Core.mempty
