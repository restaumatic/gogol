...
newDataflowProjectsJobsUpdate jobId payload projectId
  = DataflowProjectsJobsUpdate{xgafv = Core.Nothing,
...
instance Core.GoogleRequest DataflowProjectsJobsUpdate where
        ...
        requestClient DataflowProjectsJobsUpdate{..}
          = go projectId jobId xgafv accessToken callback location updateMask
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DataflowProjectsJobsUpdateResource)
                      Core.mempty
