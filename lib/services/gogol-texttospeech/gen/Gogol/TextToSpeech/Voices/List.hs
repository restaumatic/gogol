                 ...
                 Core.QueryParam "uploadType" Core.Text Core.:>
                   Core.QueryParam "upload_protocol" Core.Text Core.:>
                     Core.QueryParam "alt" Core.AltJSON Core.:>
                       Core.Get '[Core.JSON] ListVoicesResponse
...
newTextToSpeechVoicesList 
    ::  TextToSpeechVoicesList
newTextToSpeechVoicesList
  = TextToSpeechVoicesList{xgafv = Core.Nothing,
                           accessToken = Core.Nothing, callback = Core.Nothing,
                           languageCode = Core.Nothing, uploadType = Core.Nothing,
                           uploadProtocol = Core.Nothing}
instance Core.GoogleRequest TextToSpeechVoicesList where
        type Rs TextToSpeechVoicesList = ListVoicesResponse
        type Scopes TextToSpeechVoicesList = '[CloudPlatform'FullControl]
        requestClient TextToSpeechVoicesList{..}
          = go xgafv accessToken callback languageCode uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              textToSpeechService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy TextToSpeechVoicesListResource)
                      Core.mempty
