...
newConsumerSurveysMobileapppanelsList 
    :: 
                                      ConsumerSurveysMobileapppanelsList
newConsumerSurveysMobileapppanelsList
  = ConsumerSurveysMobileapppanelsList{maxResults = Core.Nothing,
                                       startIndex = Core.Nothing, token = Core.Nothing}
instance Core.GoogleRequest ConsumerSurveysMobileapppanelsList
         where
        type Rs ConsumerSurveysMobileapppanelsList =
             MobileAppPanelsListResponse
        type Scopes ConsumerSurveysMobileapppanelsList =
             '[Consumersurveys'FullControl, Consumersurveys'Readonly,
               Userinfo'Email]
        requestClient ConsumerSurveysMobileapppanelsList{..}
          = go maxResults startIndex token (Core.Just Core.AltJSON)
