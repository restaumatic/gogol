...
type CloudAssetExportAssetsResource =
     "v1p7beta1" Core.:>
       Core.CaptureMode "parent" "exportAssets" Core.Text Core.:>
...
newCloudAssetExportAssets parent payload
  = CloudAssetExportAssets{xgafv = Core.Nothing,
...
instance Core.GoogleRequest CloudAssetExportAssets where
        ...
        type Scopes CloudAssetExportAssets = '[CloudPlatform'FullControl]
        requestClient CloudAssetExportAssets{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy CloudAssetExportAssetsResource)
                      Core.mempty
