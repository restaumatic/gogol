...
type TranslateProjectsLocationsDatasetsImportDataResource =
     "v3" Core.:>
       Core.CaptureMode "dataset" "importData" Core.Text Core.:>
...
newTranslateProjectsLocationsDatasetsImportData dataset payload
  = TranslateProjectsLocationsDatasetsImportData{xgafv =
                                                   Core.Nothing,
                                                 accessToken = Core.Nothing,
                                                 callback = Core.Nothing, dataset = dataset,
                                                 payload = payload, uploadType = Core.Nothing,
                                                 uploadProtocol = Core.Nothing}
instance Core.GoogleRequest
           TranslateProjectsLocationsDatasetsImportData
         where
        type Rs TranslateProjectsLocationsDatasetsImportData = Operation
        type Scopes TranslateProjectsLocationsDatasetsImportData =
             '[CloudPlatform'FullControl, CloudTranslation'FullControl]
        requestClient TranslateProjectsLocationsDatasetsImportData{..}
          = go dataset xgafv accessToken callback uploadType uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy TranslateProjectsLocationsDatasetsImportDataResource)
                      Core.mempty
