...
newYouTubeLiveChatModeratorsInsert part payload
  = YouTubeLiveChatModeratorsInsert{xgafv = Core.Nothing,
                                    accessToken = Core.Nothing, callback = Core.Nothing,
                                    part = part, payload = payload, uploadType = Core.Nothing,
                                    uploadProtocol = Core.Nothing}
instance Core.GoogleRequest YouTubeLiveChatModeratorsInsert where
        type Rs YouTubeLiveChatModeratorsInsert = LiveChatModerator
        ...
        requestClient YouTubeLiveChatModeratorsInsert{..}
          = go part xgafv accessToken callback uploadType uploadProtocol
              (Core.Just Core.AltJSON)
              payload
              youTubeService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy YouTubeLiveChatModeratorsInsertResource)
                      Core.mempty
