                           ...
                           Core.QueryParam "uploadType" Core.Text Core.:>
                             Core.QueryParam "upload_protocol" Core.Text Core.:>
                               Core.QueryParam "alt" Core.AltJSON Core.:>
                                 Core.Get '[Core.JSON] LiveChatModeratorListResponse
...
newYouTubeLiveChatModeratorsList liveChatId part
  = YouTubeLiveChatModeratorsList{xgafv = Core.Nothing,
                                  accessToken = Core.Nothing, callback = Core.Nothing,
                                  liveChatId = liveChatId, maxResults = 5, pageToken = Core.Nothing,
                                  part = part, uploadType = Core.Nothing,
                                  uploadProtocol = Core.Nothing}
instance Core.GoogleRequest YouTubeLiveChatModeratorsList where
        type Rs YouTubeLiveChatModeratorsList =
             LiveChatModeratorListResponse
        type Scopes YouTubeLiveChatModeratorsList =
             '[Youtube'FullControl, Youtube'ForceSsl, Youtube'Readonly]
        requestClient YouTubeLiveChatModeratorsList{..}
          = go (Core.Just liveChatId) part xgafv accessToken callback
