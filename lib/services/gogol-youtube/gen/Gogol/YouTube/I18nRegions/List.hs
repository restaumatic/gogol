                     ...
                     Core.QueryParam "uploadType" Core.Text Core.:>
                       Core.QueryParam "upload_protocol" Core.Text Core.:>
                         Core.QueryParam "alt" Core.AltJSON Core.:>
                           Core.Get '[Core.JSON] I18nRegionListResponse
...
newYouTubeI18nRegionsList part
  = YouTubeI18nRegionsList{xgafv = Core.Nothing,
                           accessToken = Core.Nothing, callback = Core.Nothing, hl = "en_US",
                           part = part, uploadType = Core.Nothing,
                           uploadProtocol = Core.Nothing}
instance Core.GoogleRequest YouTubeI18nRegionsList where
        type Rs YouTubeI18nRegionsList = I18nRegionListResponse
        type Scopes YouTubeI18nRegionsList =
             '[Youtube'FullControl, Youtube'ForceSsl, Youtube'Readonly,
               Youtubepartner'FullControl]
        requestClient YouTubeI18nRegionsList{..}
          = go part xgafv accessToken callback (Core.Just hl) uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              youTubeService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy YouTubeI18nRegionsListResource)
                      Core.mempty
