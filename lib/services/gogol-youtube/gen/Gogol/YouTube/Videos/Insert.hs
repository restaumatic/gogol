                                   ...
                                   Core.QueryParam "upload_protocol" Core.Text Core.:>
                                     Core.QueryParam "alt" Core.AltJSON Core.:>
                                       Core.QueryParam "uploadType" Core.Multipart Core.:>
                                         Core.MultipartRelated '[Core.JSON] Video Core.:>
                                           Core.Post '[Core.JSON] Video
...
newYouTubeVideosInsert part payload
  = YouTubeVideosInsert{xgafv = Core.Nothing,
                        accessToken = Core.Nothing, autoLevels = Core.Nothing,
                        callback = Core.Nothing, notifySubscribers = Core.True,
                        onBehalfOfContentOwner = Core.Nothing,
                        onBehalfOfContentOwnerChannel = Core.Nothing, part = part,
                        payload = payload, stabilize = Core.Nothing,
...
instance Core.GoogleRequest YouTubeVideosInsert where
        ...
        requestClient YouTubeVideosInsert{..}
          ...
          where go Core.:<|> _
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy YouTubeVideosInsertResource)
                      Core.mempty

instance Core.GoogleRequest (Core.MediaUpload YouTubeVideosInsert)
         where
        type Rs (Core.MediaUpload YouTubeVideosInsert) = Video
        type Scopes (Core.MediaUpload YouTubeVideosInsert) =
             Core.Scopes YouTubeVideosInsert
        requestClient (Core.MediaUpload YouTubeVideosInsert{..} body)
          = go part xgafv accessToken autoLevels callback
              (Core.Just notifySubscribers)
              onBehalfOfContentOwner
              onBehalfOfContentOwnerChannel
