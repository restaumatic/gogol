...
newYouTubeSubscriptionsDelete id
  = YouTubeSubscriptionsDelete{xgafv = Core.Nothing,
                               accessToken = Core.Nothing, callback = Core.Nothing, id = id,
                               uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest YouTubeSubscriptionsDelete where
        type Rs YouTubeSubscriptionsDelete = ()
        type Scopes YouTubeSubscriptionsDelete =
             '[Youtube'FullControl, Youtube'ForceSsl,
               Youtubepartner'FullControl]
        requestClient YouTubeSubscriptionsDelete{..}
          = go (Core.Just id) xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              youTubeService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy YouTubeSubscriptionsDeleteResource)
                      Core.mempty
