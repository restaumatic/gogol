...
newYouTubeCommentsUpdate part payload
  = YouTubeCommentsUpdate{xgafv = Core.Nothing,
                          accessToken = Core.Nothing, callback = Core.Nothing, part = part,
                          payload = payload, uploadType = Core.Nothing,
                          uploadProtocol = Core.Nothing}
instance Core.GoogleRequest YouTubeCommentsUpdate where
        type Rs YouTubeCommentsUpdate = Comment
        type Scopes YouTubeCommentsUpdate = '[Youtube'ForceSsl]
        requestClient YouTubeCommentsUpdate{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy YouTubeCommentsUpdateResource)
                      Core.mempty
