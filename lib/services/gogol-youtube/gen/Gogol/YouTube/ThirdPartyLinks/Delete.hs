       ...
       "v3" Core.:>
         "thirdPartyLinks" Core.:>
           Core.QueryParam "linkingToken" Core.Text Core.:>
             Core.QueryParam "type" ThirdPartyLinksDeleteType Core.:>
...
newYouTubeThirdPartyLinksDelete linkingToken type'
  = YouTubeThirdPartyLinksDelete{xgafv = Core.Nothing,
                                 accessToken = Core.Nothing, callback = Core.Nothing,
                                 externalChannelId = Core.Nothing, linkingToken = linkingToken,
                                 part = Core.Nothing, type' = type', uploadType = Core.Nothing,
                                 uploadProtocol = Core.Nothing}
instance Core.GoogleRequest YouTubeThirdPartyLinksDelete where
        type Rs YouTubeThirdPartyLinksDelete = ()
        type Scopes YouTubeThirdPartyLinksDelete = '[]
        requestClient YouTubeThirdPartyLinksDelete{..}
          = go (Core.Just linkingToken) (Core.Just type') xgafv accessToken
              callback
              externalChannelId
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy YouTubeThirdPartyLinksDeleteResource)
                      Core.mempty
