                     ...
                     Core.QueryParam "maxResults" Core.Word32 Core.:>
                       Core.QueryParam "onBehalfOfContentOwner" Core.Text Core.:>
                         Core.QueryParam "pageToken" Core.Text Core.:>
                           Core.QueryParam "playlistId" Core.Text Core.:>
                             Core.QueryParam "uploadType" Core.Text Core.:>
                               Core.QueryParam "upload_protocol" Core.Text Core.:>
                                 Core.QueryParam "videoId" Core.Text Core.:>
                                   Core.QueryParam "alt" Core.AltJSON Core.:>
                                     Core.Get '[Core.JSON] PlaylistItemListResponse
...
newYouTubePlaylistItemsList part
  = YouTubePlaylistItemsList{xgafv = Core.Nothing,
                             accessToken = Core.Nothing, callback = Core.Nothing,
                             id = Core.Nothing, maxResults = 5,
                             onBehalfOfContentOwner = Core.Nothing, pageToken = Core.Nothing,
                             part = part, playlistId = Core.Nothing, uploadType = Core.Nothing,
                             uploadProtocol = Core.Nothing, videoId = Core.Nothing}
instance Core.GoogleRequest YouTubePlaylistItemsList where
        type Rs YouTubePlaylistItemsList = PlaylistItemListResponse
        type Scopes YouTubePlaylistItemsList =
             '[Youtube'FullControl, Youtube'ForceSsl, Youtube'Readonly,
               Youtubepartner'FullControl]
        requestClient YouTubePlaylistItemsList{..}
          = go part xgafv accessToken callback (id Core.^. Core._Default)
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy YouTubePlaylistItemsListResource)
                      Core.mempty
