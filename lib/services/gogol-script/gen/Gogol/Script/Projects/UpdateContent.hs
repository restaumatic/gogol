                   ...
                   Core.QueryParam "uploadType" Core.Text Core.:>
                     Core.QueryParam "upload_protocol" Core.Text Core.:>
                       Core.QueryParam "alt" Core.AltJSON Core.:>
                         Core.ReqBody '[Core.JSON] Content Core.:>
                           Core.Put '[Core.JSON] Content
...
newScriptProjectsUpdateContent payload scriptId
  = ScriptProjectsUpdateContent{xgafv = Core.Nothing,
                                accessToken = Core.Nothing, callback = Core.Nothing,
                                payload = payload, scriptId = scriptId, uploadType = Core.Nothing,
                                uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ScriptProjectsUpdateContent where
        type Rs ScriptProjectsUpdateContent = Content
        type Scopes ScriptProjectsUpdateContent = '[Script'Projects]
        requestClient ScriptProjectsUpdateContent{..}
          = go scriptId xgafv accessToken callback uploadType uploadProtocol
              (Core.Just Core.AltJSON)
              payload
              scriptService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ScriptProjectsUpdateContentResource)
                      Core.mempty
