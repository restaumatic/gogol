...
newRunProjectsLocationsJobsGet name
  = RunProjectsLocationsJobsGet{xgafv = Core.Nothing,
                                accessToken = Core.Nothing, callback = Core.Nothing, name = name,
                                uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest RunProjectsLocationsJobsGet where
        type Rs RunProjectsLocationsJobsGet = GoogleCloudRunV2Job
        ...
        requestClient RunProjectsLocationsJobsGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy RunProjectsLocationsJobsGetResource)
                      Core.mempty
