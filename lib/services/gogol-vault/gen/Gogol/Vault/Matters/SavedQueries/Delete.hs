...
--
-- /See:/ <https://developers.google.com/workspace/vault Google Vault API Reference> for @vault.matters.savedQueries.delete@.
module Gogol.Vault.Matters.SavedQueries.Delete
    (
    -- * Resource
     ...
     "v1" Core.:>
       "matters" Core.:>
         Core.Capture "matterId" Core.Text Core.:>
           "savedQueries" Core.:>
             Core.Capture "savedQueryId" Core.Text Core.:>
...
newVaultMattersSavedQueriesDelete matterId savedQueryId
  = VaultMattersSavedQueriesDelete{xgafv = Core.Nothing,
                                   accessToken = Core.Nothing, callback = Core.Nothing,
                                   matterId = matterId, savedQueryId = savedQueryId,
                                   uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest VaultMattersSavedQueriesDelete where
        type Rs VaultMattersSavedQueriesDelete = Empty
        type Scopes VaultMattersSavedQueriesDelete =
             '[Ediscovery'FullControl]
        requestClient VaultMattersSavedQueriesDelete{..}
          = go matterId savedQueryId xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              vaultService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy VaultMattersSavedQueriesDeleteResource)
                      Core.mempty
