...
--
-- /See:/ <https://developers.google.com/workspace/vault Google Vault API Reference> for @vault.matters.savedQueries.list@.
module Gogol.Vault.Matters.SavedQueries.List
    (
    -- * Resource
...
newVaultMattersSavedQueriesList matterId
  = VaultMattersSavedQueriesList{xgafv = Core.Nothing,
                                 accessToken = Core.Nothing, callback = Core.Nothing,
                                 matterId = matterId, pageSize = Core.Nothing,
                                 pageToken = Core.Nothing, uploadType = Core.Nothing,
                                 uploadProtocol = Core.Nothing}
instance Core.GoogleRequest VaultMattersSavedQueriesList where
        type Rs VaultMattersSavedQueriesList = ListSavedQueriesResponse
        type Scopes VaultMattersSavedQueriesList =
             '[Ediscovery'FullControl, Ediscovery'Readonly]
        requestClient VaultMattersSavedQueriesList{..}
          = go matterId xgafv accessToken callback pageSize pageToken
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy VaultMattersSavedQueriesListResource)
                      Core.mempty
