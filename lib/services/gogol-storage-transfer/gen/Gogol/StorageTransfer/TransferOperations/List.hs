...
newStorageTransferTransferOperationsList filter name
  = StorageTransferTransferOperationsList{xgafv = Core.Nothing,
                                          accessToken = Core.Nothing, callback = Core.Nothing,
                                          filter = filter, name = name, pageSize = Core.Nothing,
                                          pageToken = Core.Nothing, uploadType = Core.Nothing,
                                          uploadProtocol = Core.Nothing}
instance Core.GoogleRequest StorageTransferTransferOperationsList
         where
        type Rs StorageTransferTransferOperationsList =
             ListOperationsResponse
        type Scopes StorageTransferTransferOperationsList =
             '[CloudPlatform'FullControl]
        requestClient StorageTransferTransferOperationsList{..}
          = go name (Core.Just filter) xgafv accessToken callback pageSize
              pageToken
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy StorageTransferTransferOperationsListResource)
                      Core.mempty
