...
type StorageTransferTransferJobsRunResource =
     "v1" Core.:>
       Core.CaptureMode "jobName" "run" Core.Text Core.:>
               ...
               Core.QueryParam "uploadType" Core.Text Core.:>
                 Core.QueryParam "upload_protocol" Core.Text Core.:>
                   Core.QueryParam "alt" Core.AltJSON Core.:>
                     Core.ReqBody '[Core.JSON] RunTransferJobRequest Core.:>
...
newStorageTransferTransferJobsRun jobName payload
  = StorageTransferTransferJobsRun{xgafv = Core.Nothing,
                                   accessToken = Core.Nothing, callback = Core.Nothing,
                                   jobName = jobName, payload = payload, uploadType = Core.Nothing,
                                   uploadProtocol = Core.Nothing}
instance Core.GoogleRequest StorageTransferTransferJobsRun where
        type Rs StorageTransferTransferJobsRun = Operation
        type Scopes StorageTransferTransferJobsRun =
             '[CloudPlatform'FullControl]
        requestClient StorageTransferTransferJobsRun{..}
          = go jobName xgafv accessToken callback uploadType uploadProtocol
              (Core.Just Core.AltJSON)
              payload
              storageTransferService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy StorageTransferTransferJobsRunResource)
                      Core.mempty
