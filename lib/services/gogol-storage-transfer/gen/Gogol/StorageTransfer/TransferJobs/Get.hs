...
-- |
-- Module      : Gogol.StorageTransfer.TransferJobs.Get
                 ...
                 Core.QueryParam "uploadType" Core.Text Core.:>
                   Core.QueryParam "upload_protocol" Core.Text Core.:>
                     Core.QueryParam "alt" Core.AltJSON Core.:>
                       Core.Get '[Core.JSON] TransferJob
...
newStorageTransferTransferJobsGet jobName projectId
  = StorageTransferTransferJobsGet{xgafv = Core.Nothing,
                                   accessToken = Core.Nothing, callback = Core.Nothing,
                                   jobName = jobName, projectId = projectId,
                                   uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest StorageTransferTransferJobsGet where
        type Rs StorageTransferTransferJobsGet = TransferJob
        type Scopes StorageTransferTransferJobsGet =
             '[CloudPlatform'FullControl]
        requestClient StorageTransferTransferJobsGet{..}
          = go jobName (Core.Just projectId) xgafv accessToken callback
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy StorageTransferTransferJobsGetResource)
                      Core.mempty
