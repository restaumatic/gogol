                   ...
                   Core.QueryParam "upload_protocol" Core.Text Core.:>
                     Core.QueryParam "alt" Core.AltJSON Core.:>
                       Core.ReqBody '[Core.JSON] AgentPool Core.:>
                         Core.Patch '[Core.JSON] AgentPool
...
newStorageTransferProjectsAgentPoolsPatch name payload
  = StorageTransferProjectsAgentPoolsPatch{xgafv = Core.Nothing,
                                           accessToken = Core.Nothing, callback = Core.Nothing,
                                           name = name, payload = payload,
                                           updateMask = Core.Nothing, uploadType = Core.Nothing,
                                           uploadProtocol = Core.Nothing}
instance Core.GoogleRequest StorageTransferProjectsAgentPoolsPatch
         where
        type Rs StorageTransferProjectsAgentPoolsPatch = AgentPool
        type Scopes StorageTransferProjectsAgentPoolsPatch =
             '[CloudPlatform'FullControl]
        requestClient StorageTransferProjectsAgentPoolsPatch{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy StorageTransferProjectsAgentPoolsPatchResource)
                      Core.mempty
