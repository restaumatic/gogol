...
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @directory.tokens.list@.
module Gogol.Admin.Directory.Tokens.List
    (
    -- * Resource
...
newDirectoryTokensList userKey
  = DirectoryTokensList{xgafv = Core.Nothing,
...
instance Core.GoogleRequest DirectoryTokensList where
        ...
        type Scopes DirectoryTokensList = '[Admin'Directory'User'Security]
        requestClient DirectoryTokensList{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DirectoryTokensListResource)
                      Core.mempty
