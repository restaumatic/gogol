...
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @directory.tokens.delete@.
module Gogol.Admin.Directory.Tokens.Delete
    (
    -- * Resource
...
newDirectoryTokensDelete clientId userKey
  = DirectoryTokensDelete{xgafv = Core.Nothing,
                          accessToken = Core.Nothing, callback = Core.Nothing,
                          clientId = clientId, uploadType = Core.Nothing,
                          uploadProtocol = Core.Nothing, userKey = userKey}
instance Core.GoogleRequest DirectoryTokensDelete where
        type Rs DirectoryTokensDelete = ()
        type Scopes DirectoryTokensDelete =
             '[Admin'Directory'User'Security]
        requestClient DirectoryTokensDelete{..}
          = go userKey clientId xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              adminDirectoryService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DirectoryTokensDeleteResource)
                      Core.mempty
