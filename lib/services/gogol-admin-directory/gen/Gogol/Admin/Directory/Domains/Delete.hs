...
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @directory.domains.delete@.
module Gogol.Admin.Directory.Domains.Delete
    (
    -- * Resource
...
newDirectoryDomainsDelete customer domainName
  = DirectoryDomainsDelete{xgafv = Core.Nothing,
                           accessToken = Core.Nothing, callback = Core.Nothing,
                           customer = customer, domainName = domainName,
                           uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DirectoryDomainsDelete where
        type Rs DirectoryDomainsDelete = ()
        type Scopes DirectoryDomainsDelete = '[Admin'Directory'Domain]
        requestClient DirectoryDomainsDelete{..}
          = go customer domainName xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              adminDirectoryService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DirectoryDomainsDeleteResource)
                      Core.mempty
