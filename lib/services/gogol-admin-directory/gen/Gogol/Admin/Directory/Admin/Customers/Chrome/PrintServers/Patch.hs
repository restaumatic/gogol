...
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @admin.customers.chrome.printServers.patch@.
module Gogol.Admin.Directory.Admin.Customers.Chrome.PrintServers.Patch
    (
    -- * Resource
                       ...
                       Core.QueryParam "upload_protocol" Core.Text Core.:>
                         Core.QueryParam "alt" Core.AltJSON Core.:>
                           Core.ReqBody '[Core.JSON] PrintServer Core.:>
                             Core.Patch '[Core.JSON] PrintServer
...
newAdminCustomersChromePrintServersPatch 
    :: 
                                         Core.Text
       -- ^  Identifier. Resource name of the print server. Leave empty when creating. Format: @customers\/{customer.id}\/printServers\/{print_server.id}@ See 'name'.
    -> PrintServer
       -- ^  Multipart request metadata. See 'payload'.
    -> AdminCustomersChromePrintServersPatch
newAdminCustomersChromePrintServersPatch name payload
  = AdminCustomersChromePrintServersPatch{xgafv = Core.Nothing,
                                          accessToken = Core.Nothing, callback = Core.Nothing,
                                          name = name, payload = payload, updateMask = Core.Nothing,
                                          uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest AdminCustomersChromePrintServersPatch
         where
        type Rs AdminCustomersChromePrintServersPatch = PrintServer
        type Scopes AdminCustomersChromePrintServersPatch =
