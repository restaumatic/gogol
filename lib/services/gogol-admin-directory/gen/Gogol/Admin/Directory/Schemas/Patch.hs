...
-- Patches a schema.
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @directory.schemas.patch@.
module Gogol.Admin.Directory.Schemas.Patch
    (
    -- * Resource
...
newDirectorySchemasPatch customerId payload schemaKey
  = DirectorySchemasPatch{xgafv = Core.Nothing,
                          accessToken = Core.Nothing, callback = Core.Nothing,
                          customerId = customerId, payload = payload, schemaKey = schemaKey,
...
instance Core.GoogleRequest DirectorySchemasPatch where
        ...
        type Scopes DirectorySchemasPatch = '[Admin'Directory'Userschema]
        requestClient DirectorySchemasPatch{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DirectorySchemasPatchResource)
                      Core.mempty
