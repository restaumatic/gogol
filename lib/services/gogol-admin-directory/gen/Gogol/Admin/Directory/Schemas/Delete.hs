...
-- Deletes a schema.
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @directory.schemas.delete@.
module Gogol.Admin.Directory.Schemas.Delete
    (
    -- * Resource
...
newDirectorySchemasDelete customerId schemaKey
  = DirectorySchemasDelete{xgafv = Core.Nothing,
                           accessToken = Core.Nothing, callback = Core.Nothing,
                           customerId = customerId, schemaKey = schemaKey,
                           uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DirectorySchemasDelete where
        type Rs DirectorySchemasDelete = ()
        type Scopes DirectorySchemasDelete = '[Admin'Directory'Userschema]
        requestClient DirectorySchemasDelete{..}
          = go customerId schemaKey xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              adminDirectoryService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DirectorySchemasDeleteResource)
                      Core.mempty
