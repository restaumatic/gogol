...
-- Patches a customer.
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @directory.customers.patch@.
module Gogol.Admin.Directory.Customers.Patch
    (
    -- * Resource
...
newDirectoryCustomersPatch customerKey payload
  = DirectoryCustomersPatch{xgafv = Core.Nothing,
                            accessToken = Core.Nothing, callback = Core.Nothing,
                            customerKey = customerKey, payload = payload,
...
instance Core.GoogleRequest DirectoryCustomersPatch where
        ...
        type Scopes DirectoryCustomersPatch = '[Admin'Directory'Customer]
        requestClient DirectoryCustomersPatch{..}
          = go customerKey xgafv accessToken callback uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DirectoryCustomersPatchResource)
                      Core.mempty
