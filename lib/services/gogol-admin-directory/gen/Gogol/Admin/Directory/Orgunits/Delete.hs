...
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @directory.orgunits.delete@.
module Gogol.Admin.Directory.Orgunits.Delete
    (
    -- * Resource
...
newDirectoryOrgunitsDelete customerId orgUnitPath
  = DirectoryOrgunitsDelete{xgafv = Core.Nothing,
...
instance Core.GoogleRequest DirectoryOrgunitsDelete where
        ...
        type Scopes DirectoryOrgunitsDelete = '[Admin'Directory'Orgunit]
        requestClient DirectoryOrgunitsDelete{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DirectoryOrgunitsDeleteResource)
                      Core.mempty
