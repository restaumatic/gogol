...
--
-- Updates a user. This method supports patch semantics, meaning that you only need to include the fields you wish to update. Fields that are not present in the request will be preserved, and fields set to @null@ will be cleared. For repeating fields that contain arrays, individual items in the array can\'t be patched piecemeal; they must be supplied in the request body with the desired values for all items. See the <https://developers.google.com/workspace/admin/directory/v1/guides/manage-users#update_user user accounts guide> for more information.
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @directory.users.update@.
module Gogol.Admin.Directory.Users.Update
    (
    -- * Resource
                     ...
                     Core.QueryParam "uploadType" Core.Text Core.:>
                       Core.QueryParam "upload_protocol" Core.Text Core.:>
                         Core.QueryParam "alt" Core.AltJSON Core.:>
                           Core.ReqBody '[Core.JSON] User Core.:> Core.Put '[Core.JSON] User
...
newDirectoryUsersUpdate payload userKey
  = DirectoryUsersUpdate{xgafv = Core.Nothing,
...
instance Core.GoogleRequest DirectoryUsersUpdate where
        ...
        type Scopes DirectoryUsersUpdate = '[Admin'Directory'User]
        requestClient DirectoryUsersUpdate{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DirectoryUsersUpdateResource)
                      Core.mempty
