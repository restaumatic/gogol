...
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @directory.users.photos.delete@.
module Gogol.Admin.Directory.Users.Photos.Delete
    (
    -- * Resource
...
newDirectoryUsersPhotosDelete userKey
  = DirectoryUsersPhotosDelete{xgafv = Core.Nothing,
                               accessToken = Core.Nothing, callback = Core.Nothing,
                               uploadType = Core.Nothing, uploadProtocol = Core.Nothing,
                               userKey = userKey}
instance Core.GoogleRequest DirectoryUsersPhotosDelete where
        type Rs DirectoryUsersPhotosDelete = ()
        type Scopes DirectoryUsersPhotosDelete = '[Admin'Directory'User]
        requestClient DirectoryUsersPhotosDelete{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DirectoryUsersPhotosDeleteResource)
                      Core.mempty
