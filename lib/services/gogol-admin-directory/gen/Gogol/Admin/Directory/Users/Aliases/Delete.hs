...
-- Removes an alias.
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @directory.users.aliases.delete@.
module Gogol.Admin.Directory.Users.Aliases.Delete
    (
    -- * Resource
...
newDirectoryUsersAliasesDelete alias userKey
  = DirectoryUsersAliasesDelete{xgafv = Core.Nothing,
                                accessToken = Core.Nothing, alias = alias, callback = Core.Nothing,
                                uploadType = Core.Nothing, uploadProtocol = Core.Nothing,
                                userKey = userKey}
instance Core.GoogleRequest DirectoryUsersAliasesDelete where
        type Rs DirectoryUsersAliasesDelete = ()
        type Scopes DirectoryUsersAliasesDelete =
             '[Admin'Directory'User, Admin'Directory'User'Alias]
        requestClient DirectoryUsersAliasesDelete{..}
          = go userKey alias xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              adminDirectoryService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DirectoryUsersAliasesDeleteResource)
                      Core.mempty
