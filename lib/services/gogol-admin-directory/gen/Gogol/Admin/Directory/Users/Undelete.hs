...
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @directory.users.undelete@.
module Gogol.Admin.Directory.Users.Undelete
    (
    -- * Resource
...
newDirectoryUsersUndelete payload userKey
  = DirectoryUsersUndelete{xgafv = Core.Nothing,
...
instance Core.GoogleRequest DirectoryUsersUndelete where
        ...
        type Scopes DirectoryUsersUndelete = '[Admin'Directory'User]
        requestClient DirectoryUsersUndelete{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DirectoryUsersUndeleteResource)
                      Core.mempty
