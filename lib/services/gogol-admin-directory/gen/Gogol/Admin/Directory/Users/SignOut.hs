...
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @directory.users.signOut@.
module Gogol.Admin.Directory.Users.SignOut
    (
    -- * Resource
...
newDirectoryUsersSignOut userKey
  = DirectoryUsersSignOut{xgafv = Core.Nothing,
...
instance Core.GoogleRequest DirectoryUsersSignOut where
        ...
        type Scopes DirectoryUsersSignOut =
        ...
        requestClient DirectoryUsersSignOut{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DirectoryUsersSignOutResource)
                      Core.mempty
