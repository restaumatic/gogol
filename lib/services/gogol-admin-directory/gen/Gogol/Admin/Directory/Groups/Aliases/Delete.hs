...
-- Removes an alias.
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @directory.groups.aliases.delete@.
module Gogol.Admin.Directory.Groups.Aliases.Delete
    (
    -- * Resource
...
newDirectoryGroupsAliasesDelete alias groupKey
  = DirectoryGroupsAliasesDelete{xgafv = Core.Nothing,
                                 accessToken = Core.Nothing, alias = alias, callback = Core.Nothing,
                                 groupKey = groupKey, uploadType = Core.Nothing,
                                 uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DirectoryGroupsAliasesDelete where
        type Rs DirectoryGroupsAliasesDelete = ()
        type Scopes DirectoryGroupsAliasesDelete = '[Admin'Directory'Group]
        requestClient DirectoryGroupsAliasesDelete{..}
          = go groupKey alias xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              adminDirectoryService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DirectoryGroupsAliasesDeleteResource)
                      Core.mempty
