...
-- Creates a group.
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @directory.groups.insert@.
module Gogol.Admin.Directory.Groups.Insert
    (
    -- * Resource
...
newDirectoryGroupsInsert payload
  = DirectoryGroupsInsert{xgafv = Core.Nothing,
...
instance Core.GoogleRequest DirectoryGroupsInsert where
        ...
        type Scopes DirectoryGroupsInsert = '[Admin'Directory'Group]
        requestClient DirectoryGroupsInsert{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DirectoryGroupsInsertResource)
                      Core.mempty
