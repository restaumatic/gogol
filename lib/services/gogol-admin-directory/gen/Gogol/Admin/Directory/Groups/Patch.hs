...
--
-- Updates a group\'s properties. This method supports <https://developers.google.com/workspace/admin/directory/v1/guides/performance#patch patch semantics>.
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @directory.groups.patch@.
module Gogol.Admin.Directory.Groups.Patch
    (
    -- * Resource
...
newDirectoryGroupsPatch groupKey payload
  = DirectoryGroupsPatch{xgafv = Core.Nothing,
                         accessToken = Core.Nothing, callback = Core.Nothing,
                         groupKey = groupKey, payload = payload, uploadType = Core.Nothing,
                         uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DirectoryGroupsPatch where
        type Rs DirectoryGroupsPatch = Group
        type Scopes DirectoryGroupsPatch = '[Admin'Directory'Group]
        requestClient DirectoryGroupsPatch{..}
          = go groupKey xgafv accessToken callback uploadType uploadProtocol
              (Core.Just Core.AltJSON)
              payload
              adminDirectoryService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DirectoryGroupsPatchResource)
                      Core.mempty
