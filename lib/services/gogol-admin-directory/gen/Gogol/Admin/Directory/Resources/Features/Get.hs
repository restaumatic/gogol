...
-- |
-- Module      : Gogol.Admin.Directory.Resources.Features.Get
...
-- Retrieves a feature.
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @directory.resources.features.get@.
module Gogol.Admin.Directory.Resources.Features.Get
    (
    -- * Resource
...
newDirectoryResourcesFeaturesGet customer featureKey
  = DirectoryResourcesFeaturesGet{xgafv = Core.Nothing,
                                  accessToken = Core.Nothing, callback = Core.Nothing,
                                  customer = customer, featureKey = featureKey,
                                  uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DirectoryResourcesFeaturesGet where
        type Rs DirectoryResourcesFeaturesGet = Feature
        type Scopes DirectoryResourcesFeaturesGet =
             '[Admin'Directory'Resource'Calendar,
               Admin'Directory'Resource'Calendar'Readonly]
        requestClient DirectoryResourcesFeaturesGet{..}
          = go customer featureKey xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              adminDirectoryService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DirectoryResourcesFeaturesGetResource)
                      Core.mempty
