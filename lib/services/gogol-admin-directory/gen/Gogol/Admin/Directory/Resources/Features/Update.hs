...
-- Updates a feature.
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @directory.resources.features.update@.
module Gogol.Admin.Directory.Resources.Features.Update
    (
    -- * Resource
...
newDirectoryResourcesFeaturesUpdate customer featureKey payload
  = DirectoryResourcesFeaturesUpdate{xgafv = Core.Nothing,
                                     accessToken = Core.Nothing, callback = Core.Nothing,
                                     customer = customer, featureKey = featureKey,
                                     payload = payload, uploadType = Core.Nothing,
                                     uploadProtocol = Core.Nothing}
instance Core.GoogleRequest DirectoryResourcesFeaturesUpdate where
        type Rs DirectoryResourcesFeaturesUpdate = Feature
        type Scopes DirectoryResourcesFeaturesUpdate =
        ...
        requestClient DirectoryResourcesFeaturesUpdate{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy DirectoryResourcesFeaturesUpdateResource)
                      Core.mempty
