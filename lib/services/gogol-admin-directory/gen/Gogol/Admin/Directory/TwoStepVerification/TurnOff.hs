...
--
-- /See:/ <https://developers.google.com/workspace/admin/ Admin SDK API Reference> for @directory.twoStepVerification.turnOff@.
module Gogol.Admin.Directory.TwoStepVerification.TurnOff
    (
    -- * Resource
...
newDirectoryTwoStepVerificationTurnOff userKey
  = DirectoryTwoStepVerificationTurnOff{xgafv = Core.Nothing,
                                        accessToken = Core.Nothing, callback = Core.Nothing,
                                        uploadType = Core.Nothing, uploadProtocol = Core.Nothing,
                                        userKey = userKey}
instance Core.GoogleRequest DirectoryTwoStepVerificationTurnOff
         where
        type Rs DirectoryTwoStepVerificationTurnOff = ()
        type Scopes DirectoryTwoStepVerificationTurnOff =
        ...
        requestClient DirectoryTwoStepVerificationTurnOff{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy DirectoryTwoStepVerificationTurnOffResource)
                      Core.mempty
