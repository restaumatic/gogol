...
--
-- /See:/ <https://developers.google.com/workspace/tasks/ Google Tasks API Reference> for @tasks.tasklists.update@.
module Gogol.AppsTasks.Tasks.Tasklists.Update
    (
    -- * Resource
...
newTasksTasklistsUpdate payload tasklist
  = TasksTasklistsUpdate{xgafv = Core.Nothing,
                         accessToken = Core.Nothing, callback = Core.Nothing,
                         payload = payload, tasklist = tasklist, uploadType = Core.Nothing,
                         uploadProtocol = Core.Nothing}
instance Core.GoogleRequest TasksTasklistsUpdate where
        type Rs TasksTasklistsUpdate = TaskList
        type Scopes TasksTasklistsUpdate = '[Tasks'FullControl]
        requestClient TasksTasklistsUpdate{..}
          = go tasklist xgafv accessToken callback uploadType uploadProtocol
              (Core.Just Core.AltJSON)
              payload
              appsTasksService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy TasksTasklistsUpdateResource)
                      Core.mempty
