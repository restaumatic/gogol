...
--
-- /See:/ <https://developers.google.com/workspace/tasks/ Google Tasks API Reference> for @tasks.tasks.get@.
module Gogol.AppsTasks.Tasks.Get
    (
    -- * Resource
...
newTasksTasksGet task tasklist
  = TasksTasksGet{xgafv = Core.Nothing, accessToken = Core.Nothing,
                  callback = Core.Nothing, task = task, tasklist = tasklist,
...
instance Core.GoogleRequest TasksTasksGet where
        ...
        requestClient TasksTasksGet{..}
          = go tasklist task xgafv accessToken callback uploadType
              ...
              appsTasksService
          where go
                  = Core.buildClient (Core.Proxy :: Core.Proxy TasksTasksGetResource)
                      Core.mempty
