             ...
             "reports" Core.:>
               Core.QueryParam "startDate" Core.Text Core.:>
                 Core.QueryParam "endDate" Core.Text Core.:>
                   Core.QueryParams "dimension" Core.Text Core.:>
                     Core.QueryParams "filter" Core.Text Core.:>
                       Core.QueryParam "locale" Core.Text Core.:>
                         Core.QueryParam "maxResults" Core.Word32 Core.:>
                           Core.QueryParams "metric" Core.Text Core.:>
                             Core.QueryParams "sort" Core.Text Core.:>
...
newAdExchangeSellerAccountsReportsGenerate 
    ...
    -> AdExchangeSellerAccountsReportsGenerate
newAdExchangeSellerAccountsReportsGenerate accountId endDate
  startDate
  = AdExchangeSellerAccountsReportsGenerate{accountId = accountId,
                                            dimension = Core.Nothing, endDate = endDate,
                                            filter = Core.Nothing, locale = Core.Nothing,
                                            maxResults = Core.Nothing, metric = Core.Nothing,
                                            sort = Core.Nothing, startDate = startDate,
                                            startIndex = Core.Nothing}
instance Core.GoogleRequest AdExchangeSellerAccountsReportsGenerate
         where
        type Rs AdExchangeSellerAccountsReportsGenerate = Report
        type Scopes AdExchangeSellerAccountsReportsGenerate =
             '[Adexchange'Seller, Adexchange'Seller'Readonly]
        requestClient AdExchangeSellerAccountsReportsGenerate{..}
          = go accountId (Core.Just startDate) (Core.Just endDate)
