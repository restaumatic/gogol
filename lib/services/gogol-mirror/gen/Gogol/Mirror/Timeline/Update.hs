           ...
           Core.Capture "id" Core.Text Core.:>
             Core.QueryParam "alt" Core.AltJSON Core.:>
               Core.ReqBody '[Core.JSON] TimelineItem Core.:>
                 Core.Put '[Core.JSON] TimelineItem
       Core.:<|>
       "upload" Core.:>
         "mirror" Core.:>
           "v1" Core.:>
               ...
               Core.Capture "id" Core.Text Core.:>
                 Core.QueryParam "alt" Core.AltJSON Core.:>
                   Core.QueryParam "uploadType" Core.Multipart Core.:>
                     Core.MultipartRelated '[Core.JSON] TimelineItem Core.:>
                       Core.Put '[Core.JSON] TimelineItem
...
instance Core.GoogleRequest MirrorTimelineUpdate where
        ...
        type Scopes MirrorTimelineUpdate =
...
instance Core.GoogleRequest (Core.MediaUpload MirrorTimelineUpdate)
         where
        type Rs (Core.MediaUpload MirrorTimelineUpdate) = TimelineItem
        type Scopes (Core.MediaUpload MirrorTimelineUpdate) =
             Core.Scopes MirrorTimelineUpdate
        requestClient (Core.MediaUpload MirrorTimelineUpdate{..} body)
          = go id (Core.Just Core.AltJSON) (Core.Just Core.Multipart) payload
              body
              mirrorService
          where _ Core.:<|> go
                  = Core.buildClient
