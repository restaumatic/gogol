                 ...
                 "attachments" Core.:>
                   Core.QueryParam "alt" Core.AltJSON Core.:>
                     Core.QueryParam "uploadType" Core.AltMedia Core.:>
                       Core.AltMedia Core.:> Core.Post '[Core.JSON] Attachment
...
instance Core.GoogleRequest
         ...
         where
        type Rs (Core.MediaUpload MirrorTimelineAttachmentsInsert) =
             Attachment
        type Scopes (Core.MediaUpload MirrorTimelineAttachmentsInsert) =
             Core.Scopes MirrorTimelineAttachmentsInsert
        requestClient
          (Core.MediaUpload MirrorTimelineAttachmentsInsert{..} body)
          = go itemId (Core.Just Core.AltJSON) (Core.Just Core.AltMedia) body
              mirrorService
          where _ Core.:<|> go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy MirrorTimelineAttachmentsInsertResource)
