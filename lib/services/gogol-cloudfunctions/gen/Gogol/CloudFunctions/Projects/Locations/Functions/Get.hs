...
newCloudFunctionsProjectsLocationsFunctionsGet name
  = CloudFunctionsProjectsLocationsFunctionsGet{xgafv = Core.Nothing,
                                                accessToken = Core.Nothing, callback = Core.Nothing,
                                                name = name, revision = Core.Nothing,
                                                uploadType = Core.Nothing,
                                                uploadProtocol = Core.Nothing}
instance Core.GoogleRequest
           CloudFunctionsProjectsLocationsFunctionsGet
         where
        type Rs CloudFunctionsProjectsLocationsFunctionsGet = Function
        type Scopes CloudFunctionsProjectsLocationsFunctionsGet =
             '[CloudPlatform'FullControl]
        requestClient CloudFunctionsProjectsLocationsFunctionsGet{..}
          = go name xgafv accessToken callback revision uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy CloudFunctionsProjectsLocationsFunctionsGetResource)
                      Core.mempty
