...
type PubSubProjectsSubscriptionsGetResource =
     "v1" Core.:>
       Core.Capture "subscription" Core.Text Core.:>
...
newPubSubProjectsSubscriptionsGet subscription
  = PubSubProjectsSubscriptionsGet{xgafv = Core.Nothing,
                                   accessToken = Core.Nothing, callback = Core.Nothing,
                                   subscription = subscription, uploadType = Core.Nothing,
                                   uploadProtocol = Core.Nothing}
instance Core.GoogleRequest PubSubProjectsSubscriptionsGet where
        type Rs PubSubProjectsSubscriptionsGet = Subscription
        type Scopes PubSubProjectsSubscriptionsGet =
        ...
        requestClient PubSubProjectsSubscriptionsGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy PubSubProjectsSubscriptionsGetResource)
                      Core.mempty
