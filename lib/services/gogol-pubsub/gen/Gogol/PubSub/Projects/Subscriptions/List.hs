...
newPubSubProjectsSubscriptionsList project
  = PubSubProjectsSubscriptionsList{xgafv = Core.Nothing,
                                    accessToken = Core.Nothing, callback = Core.Nothing,
                                    pageSize = Core.Nothing, pageToken = Core.Nothing,
                                    project = project, uploadType = Core.Nothing,
                                    uploadProtocol = Core.Nothing}
instance Core.GoogleRequest PubSubProjectsSubscriptionsList where
        type Rs PubSubProjectsSubscriptionsList = ListSubscriptionsResponse
        type Scopes PubSubProjectsSubscriptionsList =
             '[CloudPlatform'FullControl, Pubsub'FullControl]
        requestClient PubSubProjectsSubscriptionsList{..}
          = go project xgafv accessToken callback pageSize pageToken
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy PubSubProjectsSubscriptionsListResource)
                      Core.mempty
