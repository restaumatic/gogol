...
-- |
-- Module      : Gogol.PubSub.Projects.Topics.TestIamPermissions
...
newPubSubProjectsTopicsTestIamPermissions payload resource
  = PubSubProjectsTopicsTestIamPermissions{xgafv = Core.Nothing,
                                           accessToken = Core.Nothing, callback = Core.Nothing,
                                           payload = payload, resource = resource,
                                           uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest PubSubProjectsTopicsTestIamPermissions
         where
        type Rs PubSubProjectsTopicsTestIamPermissions =
             TestIamPermissionsResponse
        type Scopes PubSubProjectsTopicsTestIamPermissions =
             '[CloudPlatform'FullControl, Pubsub'FullControl]
        requestClient PubSubProjectsTopicsTestIamPermissions{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy PubSubProjectsTopicsTestIamPermissionsResource)
                      Core.mempty
