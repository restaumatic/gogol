...
type PubSubProjectsTopicsPublishResource =
     "v1" Core.:>
       Core.CaptureMode "topic" "publish" Core.Text Core.:>
               ...
               Core.QueryParam "uploadType" Core.Text Core.:>
                 Core.QueryParam "upload_protocol" Core.Text Core.:>
                   Core.QueryParam "alt" Core.AltJSON Core.:>
                     Core.ReqBody '[Core.JSON] PublishRequest Core.:>
                       Core.Post '[Core.JSON] PublishResponse
...
newPubSubProjectsTopicsPublish payload topic
  = PubSubProjectsTopicsPublish{xgafv = Core.Nothing,
                                accessToken = Core.Nothing, callback = Core.Nothing,
                                payload = payload, topic = topic, uploadType = Core.Nothing,
                                uploadProtocol = Core.Nothing}
instance Core.GoogleRequest PubSubProjectsTopicsPublish where
        type Rs PubSubProjectsTopicsPublish = PublishResponse
        type Scopes PubSubProjectsTopicsPublish =
             '[CloudPlatform'FullControl, Pubsub'FullControl]
        requestClient PubSubProjectsTopicsPublish{..}
          = go topic xgafv accessToken callback uploadType uploadProtocol
              (Core.Just Core.AltJSON)
              payload
              pubSubService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy PubSubProjectsTopicsPublishResource)
                      Core.mempty
