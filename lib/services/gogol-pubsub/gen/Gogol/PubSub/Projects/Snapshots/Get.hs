...
--
-- Gets the configuration details of a snapshot. Snapshots are used in <https://cloud.google.com/pubsub/docs/replay-overview Seek> operations, which allow you to manage message acknowledgments in bulk. That is, you can set the acknowledgment state of messages in an existing subscription to the state captured by a snapshot.
--
...
newPubSubProjectsSnapshotsGet snapshot
  = PubSubProjectsSnapshotsGet{xgafv = Core.Nothing,
                               accessToken = Core.Nothing, callback = Core.Nothing,
                               snapshot = snapshot, uploadType = Core.Nothing,
                               uploadProtocol = Core.Nothing}
instance Core.GoogleRequest PubSubProjectsSnapshotsGet where
        type Rs PubSubProjectsSnapshotsGet = Snapshot
        type Scopes PubSubProjectsSnapshotsGet =
             '[CloudPlatform'FullControl, Pubsub'FullControl]
        requestClient PubSubProjectsSnapshotsGet{..}
          = go snapshot xgafv accessToken callback uploadType uploadProtocol
              (Core.Just Core.AltJSON)
              pubSubService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy PubSubProjectsSnapshotsGetResource)
                      Core.mempty
