...
newBigtableAdminProjectsInstancesAppProfilesPatch 
    :: 
                                                  Core.Text
       -- ^  The unique name of the app profile, up to 50 characters long. Values are of the form @projects\/{project}\/instances\/{instance}\/appProfiles\/_a-zA-Z0-9*@. See 'name'.
    -> AppProfile
       -- ^  Multipart request metadata. See 'payload'.
    -> BigtableAdminProjectsInstancesAppProfilesPatch
newBigtableAdminProjectsInstancesAppProfilesPatch name payload
  = BigtableAdminProjectsInstancesAppProfilesPatch{xgafv =
                                                     Core.Nothing,
                                                   accessToken = Core.Nothing,
                                                   callback = Core.Nothing,
                                                   ignoreWarnings = Core.Nothing, name = name,
...
instance Core.GoogleRequest
           BigtableAdminProjectsInstancesAppProfilesPatch
         where
        type Rs BigtableAdminProjectsInstancesAppProfilesPatch = Operation
        type Scopes BigtableAdminProjectsInstancesAppProfilesPatch =
             '[Bigtable'Admin, Bigtable'Admin'Cluster, Bigtable'Admin'Instance,
               CloudBigtable'Admin, CloudBigtable'Admin'Cluster,
               CloudPlatform'FullControl]
        requestClient BigtableAdminProjectsInstancesAppProfilesPatch{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy BigtableAdminProjectsInstancesAppProfilesPatchResource)
                      Core.mempty
