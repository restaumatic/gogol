...
newTagManagerAccountsContainersGet path
  = TagManagerAccountsContainersGet{xgafv = Core.Nothing,
                                    accessToken = Core.Nothing, callback = Core.Nothing,
                                    path = path, uploadType = Core.Nothing,
                                    uploadProtocol = Core.Nothing}
instance Core.GoogleRequest TagManagerAccountsContainersGet where
        type Rs TagManagerAccountsContainersGet = Container
        type Scopes TagManagerAccountsContainersGet =
        ...
        requestClient TagManagerAccountsContainersGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy TagManagerAccountsContainersGetResource)
                      Core.mempty
