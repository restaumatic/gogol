...
type TagManagerAccountsContainersSnippetResource =
     "tagmanager" Core.:>
       "v2" Core.:>
         Core.CaptureMode "path" "snippet" Core.Text Core.:>
                 ...
                 Core.QueryParam "uploadType" Core.Text Core.:>
                   Core.QueryParam "upload_protocol" Core.Text Core.:>
                     Core.QueryParam "alt" Core.AltJSON Core.:>
                       Core.Get '[Core.JSON] GetContainerSnippetResponse
...
newTagManagerAccountsContainersSnippet 
    :: 
                                       Core.Text
       -- ^  Container snippet\'s API relative path. See 'path'.
    -> TagManagerAccountsContainersSnippet
newTagManagerAccountsContainersSnippet path
  = TagManagerAccountsContainersSnippet{xgafv = Core.Nothing,
                                        accessToken = Core.Nothing, callback = Core.Nothing,
                                        path = path, uploadType = Core.Nothing,
                                        uploadProtocol = Core.Nothing}
instance Core.GoogleRequest TagManagerAccountsContainersSnippet
         where
        type Rs TagManagerAccountsContainersSnippet =
             GetContainerSnippetResponse
        type Scopes TagManagerAccountsContainersSnippet =
        ...
        requestClient TagManagerAccountsContainersSnippet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy TagManagerAccountsContainersSnippetResource)
                      Core.mempty
