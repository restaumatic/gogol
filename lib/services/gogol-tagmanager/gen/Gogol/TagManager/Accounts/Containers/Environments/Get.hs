...
newTagManagerAccountsContainersEnvironmentsGet 
    :: 
                                               Core.Text
       -- ^  GTM Environment\'s API relative path. See 'path'.
    -> TagManagerAccountsContainersEnvironmentsGet
newTagManagerAccountsContainersEnvironmentsGet path
  = TagManagerAccountsContainersEnvironmentsGet{xgafv = Core.Nothing,
...
instance Core.GoogleRequest
           TagManagerAccountsContainersEnvironmentsGet
         where
        type Rs TagManagerAccountsContainersEnvironmentsGet = Environment
        type Scopes TagManagerAccountsContainersEnvironmentsGet =
             '[Tagmanager'Edit'Containers, Tagmanager'Readonly]
        requestClient TagManagerAccountsContainersEnvironmentsGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy TagManagerAccountsContainersEnvironmentsGetResource)
                      Core.mempty
