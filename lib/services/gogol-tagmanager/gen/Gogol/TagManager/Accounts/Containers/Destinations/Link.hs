                       ...
                       Core.QueryParam "uploadType" Core.Text Core.:>
                         Core.QueryParam "upload_protocol" Core.Text Core.:>
                           Core.QueryParam "alt" Core.AltJSON Core.:>
                             Core.Post '[Core.JSON] Destination
...
newTagManagerAccountsContainersDestinationsLink 
    :: 
                                                Core.Text
       -- ^  GTM parent Container\'s API relative path. See 'parent'.
    -> TagManagerAccountsContainersDestinationsLink
newTagManagerAccountsContainersDestinationsLink parent
  = TagManagerAccountsContainersDestinationsLink{xgafv =
                                                   Core.Nothing,
                                                 accessToken = Core.Nothing,
                                                 allowUserPermissionFeatureUpdate = Core.Nothing,
                                                 callback = Core.Nothing,
                                                 destinationId = Core.Nothing, parent = parent,
                                                 uploadType = Core.Nothing,
                                                 uploadProtocol = Core.Nothing}
instance Core.GoogleRequest
           TagManagerAccountsContainersDestinationsLink
         where
        type Rs TagManagerAccountsContainersDestinationsLink = Destination
        type Scopes TagManagerAccountsContainersDestinationsLink =
             '[Tagmanager'Edit'Containers]
        requestClient TagManagerAccountsContainersDestinationsLink{..}
          = go parent xgafv accessToken allowUserPermissionFeatureUpdate
              callback
              destinationId
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy TagManagerAccountsContainersDestinationsLinkResource)
                      Core.mempty
