...
newTagManagerAccountsGet 
    ::  Core.Text
       -- ^  GTM Account\'s API relative path. See 'path'.
    -> TagManagerAccountsGet
newTagManagerAccountsGet path
  = TagManagerAccountsGet{xgafv = Core.Nothing,
                          accessToken = Core.Nothing, callback = Core.Nothing, path = path,
                          uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest TagManagerAccountsGet where
        type Rs TagManagerAccountsGet = Account
        type Scopes TagManagerAccountsGet =
             '[Tagmanager'Edit'Containers, Tagmanager'Manage'Accounts,
               Tagmanager'Readonly]
        requestClient TagManagerAccountsGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy TagManagerAccountsGetResource)
                      Core.mempty
