...
type AppsActivityActivitiesListResource =
     "appsactivity" Core.:>
       "v1" Core.:>
         "activities" Core.:>
           Core.QueryParam "drive.ancestorId" Core.Text Core.:>
             Core.QueryParam "drive.fileId" Core.Text Core.:>
               Core.QueryParam "groupingStrategy" ActivitiesListGroupingStrategy
                 Core.:>
                     ...
                     Core.QueryParam "source" Core.Text Core.:>
                       Core.QueryParam "userId" Core.Text Core.:>
                         Core.QueryParam "alt" Core.AltJSON Core.:>
                           Core.Get '[Core.JSON] ListActivitiesResponse
...
newAppsActivityActivitiesList 
    ::  AppsActivityActivitiesList
newAppsActivityActivitiesList
  = AppsActivityActivitiesList{driveAncestorId = Core.Nothing,
                               driveFileId = Core.Nothing,
                               groupingStrategy = ActivitiesListGroupingStrategy_DriveUi,
                               pageSize = 50, pageToken = Core.Nothing, source = Core.Nothing,
                               userId = "me"}
instance Core.GoogleRequest AppsActivityActivitiesList where
        type Rs AppsActivityActivitiesList = ListActivitiesResponse
        type Scopes AppsActivityActivitiesList = '[Activity'FullControl]
        requestClient AppsActivityActivitiesList{..}
          = go driveAncestorId driveFileId (Core.Just groupingStrategy)
              (Core.Just pageSize)
              pageToken
              source
              (Core.Just userId)
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy AppsActivityActivitiesListResource)
                      Core.mempty
