               ...
               Core.QueryParam "uploadType" Core.Text Core.:>
                 Core.QueryParam "upload_protocol" Core.Text Core.:>
                   Core.QueryParam "alt" Core.AltJSON Core.:>
                     Core.ReqBody '[Core.JSON] StopNodeRequest Core.:>
...
newTPUProjectsLocationsNodesStop name payload
  = TPUProjectsLocationsNodesStop{xgafv = Core.Nothing,
                                  accessToken = Core.Nothing, callback = Core.Nothing, name = name,
                                  payload = payload, uploadType = Core.Nothing,
                                  uploadProtocol = Core.Nothing}
instance Core.GoogleRequest TPUProjectsLocationsNodesStop where
        type Rs TPUProjectsLocationsNodesStop = Operation
        type Scopes TPUProjectsLocationsNodesStop =
        ...
        requestClient TPUProjectsLocationsNodesStop{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy TPUProjectsLocationsNodesStopResource)
                      Core.mempty
