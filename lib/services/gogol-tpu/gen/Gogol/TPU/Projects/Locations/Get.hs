...
newTPUProjectsLocationsGet name
  = TPUProjectsLocationsGet{xgafv = Core.Nothing,
...
instance Core.GoogleRequest TPUProjectsLocationsGet where
        ...
        type Scopes TPUProjectsLocationsGet = '[CloudPlatform'FullControl]
        requestClient TPUProjectsLocationsGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy TPUProjectsLocationsGetResource)
                      Core.mempty
