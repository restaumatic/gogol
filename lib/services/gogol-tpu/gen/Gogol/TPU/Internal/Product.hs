...
--
-- /See:/ 'newBootDiskConfig' smart constructor.
newtype BootDiskConfig = BootDiskConfig
    {
      -- | Optional. Customer encryption key for boot disk.
      customerEncryptionKey :: (Core.Maybe CustomerEncryptionKey)
    }
    deriving (Core.Eq, Core.Show, Core.Generic)

-- | Creates a value of 'BootDiskConfig' with the minimum fields required to make a request.
newBootDiskConfig 
    ::  BootDiskConfig
newBootDiskConfig
  = BootDiskConfig{customerEncryptionKey = Core.Nothing}
instance Core.FromJSON BootDiskConfig where
        parseJSON
          = Core.withObject "BootDiskConfig"
              (\ o ->
                 BootDiskConfig Core.<$> (o Core..:? "customerEncryptionKey"))

instance Core.ToJSON BootDiskConfig where
        toJSON BootDiskConfig{..}
          = Core.object
              (Core.catMaybes
                 [("customerEncryptionKey" Core..=) Core.<$> customerEncryptionKey])
...
newtype CustomerEncryptionKey = CustomerEncryptionKey
    {
      -- | The name of the encryption key that is stored in Google Cloud KMS. For example: \"kmsKeyName\": \"projects\/KMS/PROJECT/ID\/locations\/REGION\/keyRings\/KEY/REGION\/cryptoKeys\/KEY The fully-qualifed key name may be returned for resource GET requests. For example: \"kmsKeyName\": \"projects\/KMS/PROJECT/ID\/locations\/REGION\/keyRings\/KEY/REGION\/cryptoKeys\/KEY\/cryptoKeyVersions\/1
...
newCustomerEncryptionKey 
    ::  CustomerEncryptionKey
newCustomerEncryptionKey
  = CustomerEncryptionKey{kmsKeyName = Core.Nothing}
instance Core.FromJSON CustomerEncryptionKey where
        parseJSON
          = Core.withObject "CustomerEncryptionKey"
              (\ o -> CustomerEncryptionKey Core.<$> (o Core..:? "kmsKeyName"))
...
data Node = Node
    ...
    , bootDiskConfig :: (Core.Maybe BootDiskConfig)
...
newNode
  = Node{acceleratorConfig = Core.Nothing,
         ...
         bootDiskConfig = Core.Nothing, cidrBlock = Core.Nothing,
         createTime = Core.Nothing, dataDisks = Core.Nothing,
         description = Core.Nothing, health = Core.Nothing,
         healthDescription = Core.Nothing, id = Core.Nothing,
         labels = Core.Nothing, metadata = Core.Nothing,
          ...
          = Core.object
              (Core.catMaybes
                 [("acceleratorConfig" Core..=) Core.<$> acceleratorConfig,
                  ...
                  ("bootDiskConfig" Core..=) Core.<$> bootDiskConfig,
