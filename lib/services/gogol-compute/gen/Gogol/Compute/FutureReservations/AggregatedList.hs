...
-- |
-- Module      : Gogol.Compute.FutureReservations.AggregatedList
...
-- 
-- To prevent failure, recommendation is that you set the @returnPartialSuccess@ parameter to @true@.
--
-- /See:/ <https://cloud.google.com/compute/ Compute Engine API Reference> for @compute.futureReservations.aggregatedList@.
module Gogol.Compute.FutureReservations.AggregatedList
    (
    -- * Resource
...
-- | A resource alias for @compute.futureReservations.aggregatedList@ method which the
-- 'ComputeFutureReservationsAggregatedList' request conforms to.
type ComputeFutureReservationsAggregatedListResource =
     "compute" Core.:>
       "v1" Core.:>
         "projects" Core.:>
...
-- To prevent failure, recommendation is that you set the @returnPartialSuccess@ parameter to @true@.
--
-- /See:/ 'newComputeFutureReservationsAggregatedList' smart constructor.
data ComputeFutureReservationsAggregatedList = ComputeFutureReservationsAggregatedList
    {
      -- | V1 error format.
...
-- | Creates a value of 'ComputeFutureReservationsAggregatedList' with the minimum fields required to make a request.
newComputeFutureReservationsAggregatedList 
    :: 
                                           Core.Text
       -- ^  Project ID for this request. See 'project'.
    -> ComputeFutureReservationsAggregatedList
newComputeFutureReservationsAggregatedList project
  = ComputeFutureReservationsAggregatedList{xgafv = Core.Nothing,
...
instance Core.GoogleRequest ComputeFutureReservationsAggregatedList
         where
        type Rs ComputeFutureReservationsAggregatedList =
             FutureReservationsAggregatedListResponse
        type Scopes ComputeFutureReservationsAggregatedList =
             '[CloudPlatform'FullControl, Compute'FullControl, Compute'Readonly]
        requestClient ComputeFutureReservationsAggregatedList{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy ComputeFutureReservationsAggregatedListResource)
                      Core.mempty
