       ...
       "v1" Core.:>
         "states" Core.:>
           Core.Capture "stateKey" Core.Int32 Core.:>
             Core.QueryParam "currentStateVersion" Core.Text Core.:>
...
newAppStateStatesUpdate payload stateKey
  = AppStateStatesUpdate{currentStateVersion = Core.Nothing,
                         payload = payload, stateKey = stateKey}
instance Core.GoogleRequest AppStateStatesUpdate where
        type Rs AppStateStatesUpdate = WriteResult
        type Scopes AppStateStatesUpdate = '[Appstate'FullControl]
        requestClient AppStateStatesUpdate{..}
          = go stateKey currentStateVersion (Core.Just Core.AltJSON) payload
              appStateService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy AppStateStatesUpdateResource)
                      Core.mempty
