       ...
       "v1" Core.:>
         "states" Core.:>
           Core.Capture "stateKey" Core.Int32 Core.:>
             "clear" Core.:>
               Core.QueryParam "currentDataVersion" Core.Text Core.:>
...
newAppStateStatesClear stateKey
  = AppStateStatesClear{currentDataVersion = Core.Nothing,
                        stateKey = stateKey}
instance Core.GoogleRequest AppStateStatesClear where
        type Rs AppStateStatesClear = WriteResult
        type Scopes AppStateStatesClear = '[Appstate'FullControl]
        requestClient AppStateStatesClear{..}
          = go stateKey currentDataVersion (Core.Just Core.AltJSON)
              appStateService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy AppStateStatesClearResource)
                      Core.mempty
