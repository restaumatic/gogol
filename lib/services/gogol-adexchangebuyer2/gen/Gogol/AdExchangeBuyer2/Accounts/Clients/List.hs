                 ...
                 Core.QueryParam "callback" Core.Text Core.:>
                   Core.QueryParam "pageSize" Core.Int32 Core.:>
                     Core.QueryParam "pageToken" Core.Text Core.:>
                       Core.QueryParam "partnerClientId" Core.Text Core.:>
                         Core.QueryParam "uploadType" Core.Text Core.:>
                           Core.QueryParam "upload_protocol" Core.Text Core.:>
                             Core.QueryParam "alt" Core.AltJSON Core.:>
                               Core.Get '[Core.JSON] ListClientsResponse
...
newAdExchangeBuyer2AccountsClientsList accountId
  = AdExchangeBuyer2AccountsClientsList{xgafv = Core.Nothing,
                                        accessToken = Core.Nothing, accountId = accountId,
                                        callback = Core.Nothing, pageSize = Core.Nothing,
                                        pageToken = Core.Nothing, partnerClientId = Core.Nothing,
                                        uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest AdExchangeBuyer2AccountsClientsList
         where
        type Rs AdExchangeBuyer2AccountsClientsList = ListClientsResponse
        type Scopes AdExchangeBuyer2AccountsClientsList =
             '[Adexchange'Buyer]
        requestClient AdExchangeBuyer2AccountsClientsList{..}
          = go accountId xgafv accessToken callback pageSize pageToken
              partnerClientId
              uploadType
              uploadProtocol
