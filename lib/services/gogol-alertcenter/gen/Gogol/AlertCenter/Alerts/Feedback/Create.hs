...
--
-- /See:/ <https://developers.google.com/workspace/admin/alertcenter/ Google Workspace Alert Center API Reference> for @alertcenter.alerts.feedback.create@.
module Gogol.AlertCenter.Alerts.Feedback.Create
    (
    -- * Resource
...
type AlertCenterAlertsFeedbackCreateResource =
     "v1beta1" Core.:>
       "alerts" Core.:>
         Core.Capture "alertId" Core.Text Core.:>
           "feedback" Core.:>
                     ...
                     Core.QueryParam "uploadType" Core.Text Core.:>
                       Core.QueryParam "upload_protocol" Core.Text Core.:>
                         Core.QueryParam "alt" Core.AltJSON Core.:>
                           Core.ReqBody '[Core.JSON] AlertFeedback Core.:>
                             Core.Post '[Core.JSON] AlertFeedback
...
newAlertCenterAlertsFeedbackCreate alertId payload
  = AlertCenterAlertsFeedbackCreate{xgafv = Core.Nothing,
                                    accessToken = Core.Nothing, alertId = alertId,
                                    callback = Core.Nothing, customerId = Core.Nothing,
                                    payload = payload, uploadType = Core.Nothing,
                                    uploadProtocol = Core.Nothing}
instance Core.GoogleRequest AlertCenterAlertsFeedbackCreate where
        type Rs AlertCenterAlertsFeedbackCreate = AlertFeedback
        type Scopes AlertCenterAlertsFeedbackCreate = '[Apps'Alerts]
        requestClient AlertCenterAlertsFeedbackCreate{..}
          = go alertId xgafv accessToken callback customerId uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy AlertCenterAlertsFeedbackCreateResource)
                      Core.mempty
