...
newStorageAnywhereCachesPause anywhereCacheId bucket
  = StorageAnywhereCachesPause{anywhereCacheId = anywhereCacheId,
                               bucket = bucket, uploadType = Core.Nothing}
instance Core.GoogleRequest StorageAnywhereCachesPause where
        type Rs StorageAnywhereCachesPause = AnywhereCache
        type Scopes StorageAnywhereCachesPause =
             '[CloudPlatform'FullControl, Devstorage'FullControl,
               Devstorage'ReadWrite]
        requestClient StorageAnywhereCachesPause{..}
          = go bucket anywhereCacheId uploadType (Core.Just Core.AltJSON)
              storageService
          where go
                  = Core.buildClient
