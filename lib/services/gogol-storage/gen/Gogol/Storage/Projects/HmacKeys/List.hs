             ...
             "hmacKeys" Core.:>
               Core.QueryParam "maxResults" Core.Word32 Core.:>
                 Core.QueryParam "pageToken" Core.Text Core.:>
                   Core.QueryParam "serviceAccountEmail" Core.Text Core.:>
...
newStorageProjectsHmacKeysList projectId
  = StorageProjectsHmacKeysList{maxResults = 250,
                                pageToken = Core.Nothing, projectId = projectId,
                                serviceAccountEmail = Core.Nothing, showDeletedKeys = Core.Nothing,
                                uploadType = Core.Nothing, userProject = Core.Nothing}
instance Core.GoogleRequest StorageProjectsHmacKeysList where
        type Rs StorageProjectsHmacKeysList = HmacKeysMetadata
        ...
        requestClient StorageProjectsHmacKeysList{..}
          = go projectId (Core.Just maxResults) pageToken serviceAccountEmail
              showDeletedKeys
              uploadType
              userProject
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy StorageProjectsHmacKeysListResource)
                      Core.mempty
