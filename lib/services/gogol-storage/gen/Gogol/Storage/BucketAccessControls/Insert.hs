                 ...
                 Core.QueryParam "userProject" Core.Text Core.:>
                   Core.QueryParam "alt" Core.AltJSON Core.:>
                     Core.ReqBody '[Core.JSON] BucketAccessControl Core.:>
                       Core.Post '[Core.JSON] BucketAccessControl
...
newStorageBucketAccessControlsInsert bucket payload
  = StorageBucketAccessControlsInsert{bucket = bucket,
                                      payload = payload, uploadType = Core.Nothing,
                                      userProject = Core.Nothing}
instance Core.GoogleRequest StorageBucketAccessControlsInsert where
        type Rs StorageBucketAccessControlsInsert = BucketAccessControl
        type Scopes StorageBucketAccessControlsInsert =
             '[CloudPlatform'FullControl, Devstorage'FullControl]
        requestClient StorageBucketAccessControlsInsert{..}
          = go bucket uploadType userProject (Core.Just Core.AltJSON) payload
              storageService
          where go
                  = Core.buildClient
                      (Core.Proxy ::
