...
newStorageBucketAccessControlsPatch bucket entity payload
  = StorageBucketAccessControlsPatch{bucket = bucket,
                                     entity = entity, payload = payload, uploadType = Core.Nothing,
                                     userProject = Core.Nothing}
instance Core.GoogleRequest StorageBucketAccessControlsPatch where
        type Rs StorageBucketAccessControlsPatch = BucketAccessControl
        type Scopes StorageBucketAccessControlsPatch =
             '[CloudPlatform'FullControl, Devstorage'FullControl]
        requestClient StorageBucketAccessControlsPatch{..}
          = go bucket entity uploadType userProject (Core.Just Core.AltJSON)
              payload
              storageService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy StorageBucketAccessControlsPatchResource)
                      Core.mempty
