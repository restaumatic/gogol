...
newStorageObjectsTestIamPermissions bucket object permissions
  = StorageObjectsTestIamPermissions{bucket = bucket,
...
instance Core.GoogleRequest StorageObjectsTestIamPermissions where
        ...
        requestClient StorageObjectsTestIamPermissions{..}
          = go bucket object permissions generation uploadType userProject
              (Core.Just Core.AltJSON)
              storageService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy StorageObjectsTestIamPermissionsResource)
                      Core.mempty
