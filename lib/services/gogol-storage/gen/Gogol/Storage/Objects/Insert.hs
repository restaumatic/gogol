           ...
           Core.Capture "bucket" Core.Text Core.:>
             "o" Core.:>
               Core.QueryParam "contentEncoding" Core.Text Core.:>
                 Core.QueryParam "ifGenerationMatch" Core.Int64 Core.:>
                   Core.QueryParam "ifGenerationNotMatch" Core.Int64 Core.:>
                     Core.QueryParam "ifMetagenerationMatch" Core.Int64 Core.:>
                       Core.QueryParam "ifMetagenerationNotMatch" Core.Int64 Core.:>
                         Core.QueryParam "kmsKeyName" Core.Text Core.:>
                           Core.QueryParam "name" Core.Text Core.:>
                             Core.QueryParam "predefinedAcl" ObjectsInsertPredefinedAcl Core.:>
                               Core.QueryParam "projection" ObjectsInsertProjection Core.:>
                                       ...
                                       Core.QueryParam "userProject" Core.Text Core.:>
                                         Core.QueryParam "alt" Core.AltJSON Core.:>
                                           Core.QueryParam "uploadType" Core.Multipart Core.:>
                                             Core.MultipartRelated '[Core.JSON] Object Core.:>
                                               Core.Post '[Core.JSON] Object
...
data StorageObjectsInsert = StorageObjectsInsert
    ...
    , kmsKeyName :: (Core.Maybe Core.Text)
      -- | Name of the object. Required when the object metadata is not otherwise provided. Overrides the object metadata\'s name value, if any. For information about how to URL encode object names to be path safe, see <https://cloud.google.com/storage/docs/request-endpoints#encoding Encoding URI Path Parts>.
...
newStorageObjectsInsert bucket payload
  = StorageObjectsInsert{bucket = bucket,
                         contentEncoding = Core.Nothing, ifGenerationMatch = Core.Nothing,
                         ifGenerationNotMatch = Core.Nothing,
                         ifMetagenerationMatch = Core.Nothing,
                         ifMetagenerationNotMatch = Core.Nothing, kmsKeyName = Core.Nothing,
                         name = Core.Nothing, payload = payload,
                         predefinedAcl = Core.Nothing, projection = Core.Nothing,
...
instance Core.GoogleRequest StorageObjectsInsert where
        ...
        requestClient StorageObjectsInsert{..}
          = go bucket contentEncoding ifGenerationMatch ifGenerationNotMatch
          ...
          where go Core.:<|> _
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy StorageObjectsInsertResource)
                      Core.mempty

instance Core.GoogleRequest (Core.MediaUpload StorageObjectsInsert)
         where
        type Rs (Core.MediaUpload StorageObjectsInsert) = Object
        type Scopes (Core.MediaUpload StorageObjectsInsert) =
             Core.Scopes StorageObjectsInsert
        requestClient (Core.MediaUpload StorageObjectsInsert{..} body)
          = go bucket contentEncoding ifGenerationMatch ifGenerationNotMatch
              ifMetagenerationMatch
              ifMetagenerationNotMatch
              kmsKeyName
