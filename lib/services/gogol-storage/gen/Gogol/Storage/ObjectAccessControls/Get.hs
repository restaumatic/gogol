...
newStorageObjectAccessControlsGet bucket entity object
  = StorageObjectAccessControlsGet{bucket = bucket, entity = entity,
                                   generation = Core.Nothing, object = object,
                                   uploadType = Core.Nothing, userProject = Core.Nothing}
instance Core.GoogleRequest StorageObjectAccessControlsGet where
        type Rs StorageObjectAccessControlsGet = ObjectAccessControl
        type Scopes StorageObjectAccessControlsGet =
             '[CloudPlatform'FullControl, Devstorage'FullControl]
        requestClient StorageObjectAccessControlsGet{..}
          = go bucket object entity generation uploadType userProject
              (Core.Just Core.AltJSON)
              storageService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy StorageObjectAccessControlsGetResource)
                      Core.mempty
