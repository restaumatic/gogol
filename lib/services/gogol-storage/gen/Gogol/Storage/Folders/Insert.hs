...
newStorageFoldersInsert bucket payload
  = StorageFoldersInsert{bucket = bucket, payload = payload,
                         recursive = Core.Nothing, uploadType = Core.Nothing}
instance Core.GoogleRequest StorageFoldersInsert where
        type Rs StorageFoldersInsert = Folder
        type Scopes StorageFoldersInsert =
             '[CloudPlatform'FullControl, Devstorage'FullControl,
               Devstorage'ReadWrite]
        requestClient StorageFoldersInsert{..}
          = go bucket recursive uploadType (Core.Just Core.AltJSON) payload
              storageService
          where go
                  = Core.buildClient
