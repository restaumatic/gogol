...
instance Core.GoogleRequest
        ...
        type Scopes IdentityToolkitRelyingpartyDownloadAccount =
             '[CloudPlatform'FullControl, Firebase'FullControl]
        requestClient IdentityToolkitRelyingpartyDownloadAccount{..}
          = go (Core.Just Core.AltJSON) payload identityToolkitService
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy IdentityToolkitRelyingpartyDownloadAccountResource)
                      Core.mempty
