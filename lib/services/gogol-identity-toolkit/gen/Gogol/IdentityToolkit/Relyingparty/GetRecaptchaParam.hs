...
instance Core.GoogleRequest
        ...
        type Scopes IdentityToolkitRelyingpartyGetRecaptchaParam =
             '[CloudPlatform'FullControl]
        requestClient IdentityToolkitRelyingpartyGetRecaptchaParam{}
          = go (Core.Just Core.AltJSON) identityToolkitService
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy IdentityToolkitRelyingpartyGetRecaptchaParamResource)
                      Core.mempty
