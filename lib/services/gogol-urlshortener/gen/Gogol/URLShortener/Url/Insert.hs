       ...
       "v1" Core.:>
         "url" Core.:>
           Core.QueryParam "alt" Core.AltJSON Core.:>
             Core.ReqBody '[Core.JSON] Url Core.:> Core.Post '[Core.JSON] Url
...
newURLShortenerUrlInsert 
    ...
    -> URLShortenerUrlInsert
newURLShortenerUrlInsert payload
  = URLShortenerUrlInsert{payload = payload}
instance Core.GoogleRequest URLShortenerUrlInsert where
        ...
        requestClient URLShortenerUrlInsert{..}
          = go (Core.Just Core.AltJSON) payload uRLShortenerService
          where go
                  = Core.buildClient
