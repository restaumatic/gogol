                                       ...
                                       Core.QueryParam "upload_protocol" Core.Text Core.:>
                                         Core.QueryParams "videoIds" Core.Text Core.:>
                                           Core.QueryParam "alt" Core.AltJSON Core.:>
                                             Core.Get '[Core.JSON] ListOrdersResponse
...
newPlayMoviesPartnerAccountsOrdersList accountId
  = PlayMoviesPartnerAccountsOrdersList{xgafv = Core.Nothing,
                                        accessToken = Core.Nothing, accountId = accountId,
                                        bearerToken = Core.Nothing, callback = Core.Nothing,
                                        customId = Core.Nothing, name = Core.Nothing,
                                        pageSize = Core.Nothing, pageToken = Core.Nothing,
                                        pp = Core.True, pphNames = Core.Nothing,
                                        status = Core.Nothing, studioNames = Core.Nothing,
                                        uploadType = Core.Nothing, uploadProtocol = Core.Nothing,
                                        videoIds = Core.Nothing}
instance Core.GoogleRequest PlayMoviesPartnerAccountsOrdersList
         where
        type Rs PlayMoviesPartnerAccountsOrdersList = ListOrdersResponse
        ...
        requestClient PlayMoviesPartnerAccountsOrdersList{..}
          = go accountId xgafv accessToken bearerToken callback customId name
              pageSize
              pageToken
              (Core.Just pp)
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy PlayMoviesPartnerAccountsOrdersListResource)
                      Core.mempty
