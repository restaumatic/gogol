     ...
     "v1" Core.:>
       "accounts" Core.:>
         Core.Capture "accountId" Core.Text Core.:>
           "avails" Core.:>
             Core.Capture "availId" Core.Text Core.:>
...
newPlayMoviesPartnerAccountsAvailsGet accountId availId
  = PlayMoviesPartnerAccountsAvailsGet{xgafv = Core.Nothing,
                                       accessToken = Core.Nothing, accountId = accountId,
                                       availId = availId, bearerToken = Core.Nothing,
                                       callback = Core.Nothing, pp = Core.True,
                                       uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest PlayMoviesPartnerAccountsAvailsGet
         where
        type Rs PlayMoviesPartnerAccountsAvailsGet = Avail
        type Scopes PlayMoviesPartnerAccountsAvailsGet =
             '[PlaymoviesPartner'Readonly]
        requestClient PlayMoviesPartnerAccountsAvailsGet{..}
          = go accountId availId xgafv accessToken bearerToken callback
              (Core.Just pp)
              uploadType
              uploadProtocol
