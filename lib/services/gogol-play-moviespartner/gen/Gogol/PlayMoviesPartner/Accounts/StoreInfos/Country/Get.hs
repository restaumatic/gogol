...
newPlayMoviesPartnerAccountsStoreInfosCountryGet 
    ...
    -> PlayMoviesPartnerAccountsStoreInfosCountryGet
newPlayMoviesPartnerAccountsStoreInfosCountryGet accountId country
  videoId
  = PlayMoviesPartnerAccountsStoreInfosCountryGet{xgafv =
                                                    Core.Nothing,
                                                  accessToken = Core.Nothing, accountId = accountId,
                                                  bearerToken = Core.Nothing,
                                                  callback = Core.Nothing, country = country,
                                                  pp = Core.True, uploadType = Core.Nothing,
                                                  uploadProtocol = Core.Nothing, videoId = videoId}
instance Core.GoogleRequest
           PlayMoviesPartnerAccountsStoreInfosCountryGet
         where
        type Rs PlayMoviesPartnerAccountsStoreInfosCountryGet = StoreInfo
        type Scopes PlayMoviesPartnerAccountsStoreInfosCountryGet =
             '[PlaymoviesPartner'Readonly]
        requestClient PlayMoviesPartnerAccountsStoreInfosCountryGet{..}
          = go accountId videoId country xgafv accessToken bearerToken
              callback
              (Core.Just pp)
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy PlayMoviesPartnerAccountsStoreInfosCountryGetResource)
                      Core.mempty
