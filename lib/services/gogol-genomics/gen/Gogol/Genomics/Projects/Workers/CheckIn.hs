...
type GenomicsProjectsWorkersCheckInResource =
     "v2alpha1" Core.:>
       Core.CaptureMode "id" "checkIn" Core.Text Core.:>
               ...
               Core.QueryParam "uploadType" Core.Text Core.:>
                 Core.QueryParam "upload_protocol" Core.Text Core.:>
                   Core.QueryParam "alt" Core.AltJSON Core.:>
                     Core.ReqBody '[Core.JSON] CheckInRequest Core.:>
                       Core.Post '[Core.JSON] CheckInResponse
...
newGenomicsProjectsWorkersCheckIn id payload
  = GenomicsProjectsWorkersCheckIn{xgafv = Core.Nothing,
                                   accessToken = Core.Nothing, callback = Core.Nothing, id = id,
                                   payload = payload, uploadType = Core.Nothing,
                                   uploadProtocol = Core.Nothing}
instance Core.GoogleRequest GenomicsProjectsWorkersCheckIn where
        type Rs GenomicsProjectsWorkersCheckIn = CheckInResponse
        type Scopes GenomicsProjectsWorkersCheckIn =
        ...
        requestClient GenomicsProjectsWorkersCheckIn{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy GenomicsProjectsWorkersCheckInResource)
                      Core.mempty
