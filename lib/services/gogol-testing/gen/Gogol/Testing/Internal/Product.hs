...
--
-- /See:/ 'newApkSplits' smart constructor.
newtype ApkSplits = ApkSplits
    {
      ...
      bundleSplits :: (Core.Maybe [FileReference])
    }
    deriving (Core.Eq, Core.Show, Core.Generic)

-- | Creates a value of 'ApkSplits' with the minimum fields required to make a request.
newApkSplits 
    ::  ApkSplits
newApkSplits = ApkSplits{bundleSplits = Core.Nothing}
instance Core.FromJSON ApkSplits where
        parseJSON
          = Core.withObject "ApkSplits"
              (\ o -> ApkSplits Core.<$> (o Core..:? "bundleSplits"))

instance Core.ToJSON ApkSplits where
        toJSON ApkSplits{..}
          = Core.object
              (Core.catMaybes [("bundleSplits" Core..=) Core.<$> bundleSplits])


-- | An Android App Bundle file format, containing a BundleConfig.pb file, a base module directory, zero or more dynamic feature module directories. See https:\/\/developer.android.com\/guide\/app-bundle\/build for guidance on building App Bundles.
--
-- /See:/ 'newAppBundle' smart constructor.
data AppBundle = AppBundle
    {
      -- | .apk files generated by bundletool to install as a single android app.
      apks :: (Core.Maybe ApkSplits)
...
newAppBundle 
    ::  AppBundle
newAppBundle
  = AppBundle{apks = Core.Nothing, bundleLocation = Core.Nothing}
instance Core.FromJSON AppBundle where
        parseJSON
          = Core.withObject "AppBundle"
              (\ o ->
                 AppBundle Core.<$>
                   (o Core..:? "apks") Core.<*> (o Core..:? "bundleLocation"))
...
data RegularFile = RegularFile
    {
      ...
      content :: (Core.Maybe FileReference)
      -- | Required. Where to put the content on the device. Must be an absolute, allowlisted path. If the file exists, it will be replaced. The following device-side directories and any of their subdirectories are allowlisted: ${EXTERNAL/STORAGE}, \/sdcard ${ANDROID/DATA}\/local\/tmp, or \/data\/local\/tmp Specifying a path outside of these directory trees is invalid. The paths \/sdcard and \/data will be made available and treated as implicit path substitutions. E.g. if \/sdcard on a particular device does not map to external storage, the system will replace it with the external storage path prefix for that device and copy the file there. It is strongly advised to use the Environment API in app and test code to access files on the device in a portable way.
    , devicePath :: (Core.Maybe Core.Text)
    }
    deriving (Core.Eq, Core.Show, Core.Generic)
