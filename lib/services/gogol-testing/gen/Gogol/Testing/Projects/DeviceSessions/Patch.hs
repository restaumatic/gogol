...
newTestingProjectsDeviceSessionsPatch name payload
  = TestingProjectsDeviceSessionsPatch{xgafv = Core.Nothing,
                                       accessToken = Core.Nothing, callback = Core.Nothing,
                                       name = name, payload = payload, updateMask = Core.Nothing,
                                       uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest TestingProjectsDeviceSessionsPatch
         where
        type Rs TestingProjectsDeviceSessionsPatch = DeviceSession
        ...
        requestClient TestingProjectsDeviceSessionsPatch{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy TestingProjectsDeviceSessionsPatchResource)
                      Core.mempty
