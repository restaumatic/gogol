...
type RedisProjectsLocationsClustersBackupResource =
     "v1" Core.:>
       Core.CaptureMode "name" "backup" Core.Text Core.:>
               ...
               Core.QueryParam "uploadType" Core.Text Core.:>
                 Core.QueryParam "upload_protocol" Core.Text Core.:>
                   Core.QueryParam "alt" Core.AltJSON Core.:>
                     Core.ReqBody '[Core.JSON] BackupClusterRequest Core.:>
...
newRedisProjectsLocationsClustersBackup 
    :: 
                                        Core.Text
       -- ^  Required. Redis cluster resource name using the form: @projects\/{project_id}\/locations\/{location_id}\/clusters\/{cluster_id}@ where @location_id@ refers to a Google Cloud region. See 'name'.
    -> BackupClusterRequest
       -- ^  Multipart request metadata. See 'payload'.
    -> RedisProjectsLocationsClustersBackup
newRedisProjectsLocationsClustersBackup name payload
  = RedisProjectsLocationsClustersBackup{xgafv = Core.Nothing,
                                         accessToken = Core.Nothing, callback = Core.Nothing,
                                         name = name, payload = payload, uploadType = Core.Nothing,
                                         uploadProtocol = Core.Nothing}
instance Core.GoogleRequest RedisProjectsLocationsClustersBackup
         where
        type Rs RedisProjectsLocationsClustersBackup = Operation
        type Scopes RedisProjectsLocationsClustersBackup =
