...
newRedisProjectsLocationsBackupCollectionsGet 
    :: 
                                              Core.Text
       -- ^  Required. Redis backupCollection resource name using the form: @projects\/{project_id}\/locations\/{location_id}\/backupCollections\/{backup_collection_id}@ where @location_id@ refers to a Google Cloud region. See 'name'.
    -> RedisProjectsLocationsBackupCollectionsGet
newRedisProjectsLocationsBackupCollectionsGet name
  = RedisProjectsLocationsBackupCollectionsGet{xgafv = Core.Nothing,
...
instance Core.GoogleRequest
        ...
        type Rs RedisProjectsLocationsBackupCollectionsGet =
             BackupCollection
        type Scopes RedisProjectsLocationsBackupCollectionsGet =
             '[CloudPlatform'FullControl]
        requestClient RedisProjectsLocationsBackupCollectionsGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy RedisProjectsLocationsBackupCollectionsGetResource)
                      Core.mempty
