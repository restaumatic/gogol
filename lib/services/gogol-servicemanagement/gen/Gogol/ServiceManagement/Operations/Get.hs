...
newServiceManagementOperationsGet name
  = ServiceManagementOperationsGet{xgafv = Core.Nothing,
                                   accessToken = Core.Nothing, callback = Core.Nothing, name = name,
                                   uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ServiceManagementOperationsGet where
        type Rs ServiceManagementOperationsGet = Operation
        type Scopes ServiceManagementOperationsGet =
        ...
        requestClient ServiceManagementOperationsGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ServiceManagementOperationsGetResource)
                      Core.mempty
