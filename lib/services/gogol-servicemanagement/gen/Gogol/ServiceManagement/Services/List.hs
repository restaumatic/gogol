         ...
         Core.QueryParam "$.xgafv" Xgafv Core.:>
           Core.QueryParam "access_token" Core.Text Core.:>
             Core.QueryParam "callback" Core.Text Core.:>
               Core.QueryParam "consumerId" Core.Text Core.:>
                 Core.QueryParam "pageSize" Core.Int32 Core.:>
                   Core.QueryParam "pageToken" Core.Text Core.:>
                     Core.QueryParam "producerProjectId" Core.Text Core.:>
...
newServiceManagementServicesList 
    ::  ServiceManagementServicesList
newServiceManagementServicesList
  = ServiceManagementServicesList{xgafv = Core.Nothing,
                                  accessToken = Core.Nothing, callback = Core.Nothing,
                                  consumerId = Core.Nothing, pageSize = Core.Nothing,
                                  pageToken = Core.Nothing, producerProjectId = Core.Nothing,
                                  uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ServiceManagementServicesList where
        type Rs ServiceManagementServicesList = ListServicesResponse
        type Scopes ServiceManagementServicesList =
             '[CloudPlatform'FullControl, CloudPlatform'ReadOnly,
               Service'Management, Service'Management'Readonly]
        requestClient ServiceManagementServicesList{..}
          = go xgafv accessToken callback consumerId pageSize pageToken
              producerProjectId
              uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ServiceManagementServicesListResource)
                      Core.mempty
