...
-- |
-- Module      : Gogol.ServiceManagement.Services.Rollouts.Get
     ...
     "v1" Core.:>
       "services" Core.:>
         Core.Capture "serviceName" Core.Text Core.:>
           "rollouts" Core.:>
             Core.Capture "rolloutId" Core.Text Core.:>
...
newServiceManagementServicesRolloutsGet rolloutId serviceName
  = ServiceManagementServicesRolloutsGet{xgafv = Core.Nothing,
                                         accessToken = Core.Nothing, callback = Core.Nothing,
                                         rolloutId = rolloutId, serviceName = serviceName,
                                         uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ServiceManagementServicesRolloutsGet
         where
        type Rs ServiceManagementServicesRolloutsGet = Rollout
        type Scopes ServiceManagementServicesRolloutsGet =
             '[CloudPlatform'FullControl, CloudPlatform'ReadOnly,
               Service'Management, Service'Management'Readonly]
        requestClient ServiceManagementServicesRolloutsGet{..}
          = go serviceName rolloutId xgafv accessToken callback uploadType
              uploadProtocol
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy ServiceManagementServicesRolloutsGetResource)
                      Core.mempty
