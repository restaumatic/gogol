...
newServiceManagementServicesDelete serviceName
  = ServiceManagementServicesDelete{xgafv = Core.Nothing,
                                    accessToken = Core.Nothing, callback = Core.Nothing,
                                    serviceName = serviceName, uploadType = Core.Nothing,
                                    uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ServiceManagementServicesDelete where
        type Rs ServiceManagementServicesDelete = Operation
        type Scopes ServiceManagementServicesDelete =
        ...
        requestClient ServiceManagementServicesDelete{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ServiceManagementServicesDeleteResource)
                      Core.mempty
