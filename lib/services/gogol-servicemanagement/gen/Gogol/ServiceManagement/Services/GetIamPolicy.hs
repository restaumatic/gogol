...
newServiceManagementServicesGetIamPolicy payload resource
  = ServiceManagementServicesGetIamPolicy{xgafv = Core.Nothing,
                                          accessToken = Core.Nothing, callback = Core.Nothing,
                                          payload = payload, resource = resource,
                                          uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest ServiceManagementServicesGetIamPolicy
         where
        type Rs ServiceManagementServicesGetIamPolicy = Policy
        type Scopes ServiceManagementServicesGetIamPolicy =
             '[CloudPlatform'FullControl, CloudPlatform'ReadOnly,
               Service'Management, Service'Management'Readonly]
        requestClient ServiceManagementServicesGetIamPolicy{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy ServiceManagementServicesGetIamPolicyResource)
                      Core.mempty
