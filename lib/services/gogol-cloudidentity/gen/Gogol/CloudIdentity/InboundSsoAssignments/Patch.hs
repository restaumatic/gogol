                 ...
                 Core.QueryParam "uploadType" Core.Text Core.:>
                   Core.QueryParam "upload_protocol" Core.Text Core.:>
                     Core.QueryParam "alt" Core.AltJSON Core.:>
                       Core.ReqBody '[Core.JSON] InboundSsoAssignment Core.:>
...
newCloudIdentityInboundSsoAssignmentsPatch name payload
  = CloudIdentityInboundSsoAssignmentsPatch{xgafv = Core.Nothing,
                                            accessToken = Core.Nothing, callback = Core.Nothing,
                                            name = name, payload = payload,
                                            updateMask = Core.Nothing, uploadType = Core.Nothing,
                                            uploadProtocol = Core.Nothing}
instance Core.GoogleRequest CloudIdentityInboundSsoAssignmentsPatch
         where
        type Rs CloudIdentityInboundSsoAssignmentsPatch = Operation
        type Scopes CloudIdentityInboundSsoAssignmentsPatch =
             '[CloudIdentity'Inboundsso, CloudPlatform'FullControl]
        requestClient CloudIdentityInboundSsoAssignmentsPatch{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy CloudIdentityInboundSsoAssignmentsPatchResource)
                      Core.mempty
