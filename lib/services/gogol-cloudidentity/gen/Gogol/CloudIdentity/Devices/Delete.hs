...
newCloudIdentityDevicesDelete name
  = CloudIdentityDevicesDelete{xgafv = Core.Nothing,
                               accessToken = Core.Nothing, callback = Core.Nothing,
                               customer = Core.Nothing, name = name, uploadType = Core.Nothing,
                               uploadProtocol = Core.Nothing}
instance Core.GoogleRequest CloudIdentityDevicesDelete where
        type Rs CloudIdentityDevicesDelete = Operation
        type Scopes CloudIdentityDevicesDelete = '[CloudIdentity'Devices]
        requestClient CloudIdentityDevicesDelete{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy CloudIdentityDevicesDeleteResource)
                      Core.mempty
