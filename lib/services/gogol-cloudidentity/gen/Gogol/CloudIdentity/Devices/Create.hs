                 ...
                 Core.QueryParam "uploadType" Core.Text Core.:>
                   Core.QueryParam "upload_protocol" Core.Text Core.:>
                     Core.QueryParam "alt" Core.AltJSON Core.:>
                       Core.ReqBody '[Core.JSON] GoogleAppsCloudidentityDevicesV1Device
...
newCloudIdentityDevicesCreate payload
  = CloudIdentityDevicesCreate{xgafv = Core.Nothing,
                               accessToken = Core.Nothing, callback = Core.Nothing,
                               customer = Core.Nothing, payload = payload,
                               uploadType = Core.Nothing, uploadProtocol = Core.Nothing}
instance Core.GoogleRequest CloudIdentityDevicesCreate where
        type Rs CloudIdentityDevicesCreate = Operation
        type Scopes CloudIdentityDevicesCreate = '[CloudIdentity'Devices]
        requestClient CloudIdentityDevicesCreate{..}
          = go xgafv accessToken callback customer uploadType uploadProtocol
              (Core.Just Core.AltJSON)
              payload
              cloudIdentityService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy CloudIdentityDevicesCreateResource)
                      Core.mempty
