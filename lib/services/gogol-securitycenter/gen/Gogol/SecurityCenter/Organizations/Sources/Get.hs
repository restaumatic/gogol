...
newSecurityCenterOrganizationsSourcesGet name
  = SecurityCenterOrganizationsSourcesGet{xgafv = Core.Nothing,
                                          accessToken = Core.Nothing, callback = Core.Nothing,
                                          name = name, uploadType = Core.Nothing,
                                          uploadProtocol = Core.Nothing}
instance Core.GoogleRequest SecurityCenterOrganizationsSourcesGet
         where
        type Rs SecurityCenterOrganizationsSourcesGet = Source
        type Scopes SecurityCenterOrganizationsSourcesGet =
             '[CloudPlatform'FullControl]
        requestClient SecurityCenterOrganizationsSourcesGet{..}
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy ::
                         Core.Proxy SecurityCenterOrganizationsSourcesGetResource)
                      Core.mempty
