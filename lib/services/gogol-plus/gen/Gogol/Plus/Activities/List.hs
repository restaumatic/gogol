       ...
       "v1" Core.:>
         "people" Core.:>
           Core.Capture "userId" Core.Text Core.:>
             "activities" Core.:>
               Core.Capture "collection" ActivitiesListCollection Core.:>
                 Core.QueryParam "maxResults" Core.Word32 Core.:>
                   Core.QueryParam "pageToken" Core.Text Core.:>
                     Core.QueryParam "alt" Core.AltJSON Core.:>
                       Core.Get '[Core.JSON] ActivityFeed
...
newPlusActivitiesList collection userId
  = PlusActivitiesList{collection = collection, maxResults = 20,
                       pageToken = Core.Nothing, userId = userId}
instance Core.GoogleRequest PlusActivitiesList where
        type Rs PlusActivitiesList = ActivityFeed
        type Scopes PlusActivitiesList = '[Plus'Login, Plus'Me]
        requestClient PlusActivitiesList{..}
          = go userId collection (Core.Just maxResults) pageToken
