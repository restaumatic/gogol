           ...
           Core.QueryParam "query" Core.Text Core.:>
             Core.QueryParam "language" Core.Text Core.:>
               Core.QueryParam "maxResults" Core.Word32 Core.:>
                 Core.QueryParam "orderBy" ActivitiesSearchOrderBy Core.:>
...
newPlusActivitiesSearch query
  = PlusActivitiesSearch{language = "en-US", maxResults = 10,
                         orderBy = ActivitiesSearchOrderBy_Recent, pageToken = Core.Nothing,
                         query = query}
instance Core.GoogleRequest PlusActivitiesSearch where
        ...
        requestClient PlusActivitiesSearch{..}
          = go (Core.Just query) (Core.Just language) (Core.Just maxResults)
              (Core.Just orderBy)
              pageToken
