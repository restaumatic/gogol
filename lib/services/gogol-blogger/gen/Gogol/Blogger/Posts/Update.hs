...
-- |
-- Module      : Gogol.Blogger.Posts.Update
...
newBloggerPostsUpdate blogId payload postId
  = BloggerPostsUpdate{xgafv = Core.Nothing,
                       accessToken = Core.Nothing, blogId = blogId,
                       callback = Core.Nothing, fetchBody = Core.True,
                       fetchImages = Core.Nothing, maxComments = Core.Nothing,
                       payload = payload, postId = postId, publish = Core.Nothing,
                       revert = Core.Nothing, uploadType = Core.Nothing,
                       uploadProtocol = Core.Nothing}
instance Core.GoogleRequest BloggerPostsUpdate where
        type Rs BloggerPostsUpdate = Post'
        type Scopes BloggerPostsUpdate = '[Blogger'FullControl]
        requestClient BloggerPostsUpdate{..}
          = go blogId postId xgafv accessToken callback (Core.Just fetchBody)
              fetchImages
              maxComments
              publish
              revert
