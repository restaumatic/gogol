...
newBloggerPostsInsert blogId payload
  = BloggerPostsInsert{xgafv = Core.Nothing,
                       accessToken = Core.Nothing, blogId = blogId,
                       callback = Core.Nothing, fetchBody = Core.True,
                       fetchImages = Core.Nothing, isDraft = Core.Nothing,
...
instance Core.GoogleRequest BloggerPostsInsert where
        ...
        type Scopes BloggerPostsInsert = '[Blogger'FullControl]
        requestClient BloggerPostsInsert{..}
          = go blogId xgafv accessToken callback (Core.Just fetchBody)
              fetchImages
              isDraft
              uploadType
              uploadProtocol
