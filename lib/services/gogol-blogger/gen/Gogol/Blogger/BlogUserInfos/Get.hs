...
newBloggerBlogUserInfosGet blogId userId
  = BloggerBlogUserInfosGet{xgafv = Core.Nothing,
                            accessToken = Core.Nothing, blogId = blogId,
                            callback = Core.Nothing, maxPosts = Core.Nothing,
...
instance Core.GoogleRequest BloggerBlogUserInfosGet where
        ...
        requestClient BloggerBlogUserInfosGet{..}
          = go userId blogId xgafv accessToken callback maxPosts uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              bloggerService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy BloggerBlogUserInfosGetResource)
                      Core.mempty
