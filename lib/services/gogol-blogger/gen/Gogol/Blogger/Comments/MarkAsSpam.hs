...
newBloggerCommentsMarkAsSpam blogId commentId postId
  = BloggerCommentsMarkAsSpam{xgafv = Core.Nothing,
                              accessToken = Core.Nothing, blogId = blogId,
                              callback = Core.Nothing, commentId = commentId, postId = postId,
...
instance Core.GoogleRequest BloggerCommentsMarkAsSpam where
        ...
        type Scopes BloggerCommentsMarkAsSpam = '[Blogger'FullControl]
        requestClient BloggerCommentsMarkAsSpam{..}
          = go blogId postId commentId xgafv accessToken callback uploadType
              uploadProtocol
              (Core.Just Core.AltJSON)
              bloggerService
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy BloggerCommentsMarkAsSpamResource)
                      Core.mempty
